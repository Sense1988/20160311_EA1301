...
static int createConnection(void) {
    if(g_bwcsfd < 0) {
        ...
        if(g_bwcsfd >= 0) {
            ...
            {
                LOGE("connect to BT task failed, %s, %d", strerror(errno), errno);
                close(g_bwcsfd);
                g_bwcsfd = -1;
            }
        }else {
            LOGE("createConnection failed, %s, %d", strerror(errno), errno);
        }
    }
    return g_bwcsfd;
}
...
static int sendCmd(unsigned int cmd, unsigned int resp, ilm_struct *ilm, unsigned int *size) {
    ...
    *size = sizeof(ilm_struct) - MAX_ILM_BUFFER_SIZE + *size;
    LOGI("sendCmd cmd=%lu, size=%u", ilm->msg_id, *size);
    do{
    ...
    if(ret >= 0) {
        if (resp > 0 ) {
            ...
            if(ret > 0)
            {
                ...
                if( ilm->msg_id != resp)
                {
                    LOGE("unexpected resp=%lu, expected=%u", ilm->msg_id, resp);
                    ret = -1;
                }
                else
                {
                    *size = ret;
                    LOGI("Recv resp, size=%d", ret);
                }
            }else {
                LOGE("select failed, ret=%d, %s, %d", ret, strerror(errno), errno);
                ret = -1;
            }
        }else {
            LOGI("No response for this command");
        }
    }else {
        LOGE("send failed, %s, %d", strerror(errno), errno);
    }
    return ret;
}
...
int bwcs_query_bt_state(unsigned char *state) {
    ...
    if(!state) {
        LOGE("Invalid parameter : %p", state);
        return -1;
    }

    LOGI("bwcs_query_bt_state(%p)", state);
    ...
    if(result >= 0 && result == CAL_ILM_SIZE(wmt_bt_query_state_cnf_struct)) {
        ...
        *state = cnf->state;
        LOGI("bt state = %d", *state);
    } else {
        LOGE("bwcs_query_bt_state failed. result=%d, expected size=%d", result, sizeof(wmt_bt_query_state_cnf_struct));
    }
    return result;
}
...
int bwcs_set_afh(unsigned int freq, unsigned char *chnl_map, unsigned char hb) {
    ...
    LOGI("bwcs_set_afh(%p, %d)", chnl_map, hb);
    if(chnl_map == NULL) {
        LOGE("[ERR] chnl_map is NULL");
        return -1;
    }
    req->afh = KAL_TRUE;
    req->pta = KAL_FALSE;
...
int bwcs_set_pta(unsigned char action) {
    ...
    LOGI("bwcs_set_pta(action=%d)", action);
    req->afh = KAL_FALSE;
    req->pta = KAL_TRUE;
...
int bwcs_query_rssi(unsigned int conn_handle, int *rssi) {
    ...
    if(!rssi) {
        LOGE("Invalid parameter : rssi=null");
        return -1;
    }

    LOGI("bwcs_query_rssi(conn_handle=%d)", conn_handle);
    ...
    if(result >= 0 && result == CAL_ILM_SIZE(wmt_bt_query_rssi_ind_struct)) {
        ...
        if(ind->is_valid == KAL_TRUE) {
            *rssi = ind->bt_rssi;
            ...
            result = -1;
            LOGW("[WARN] is_valid is false");
        }
    }else {
        LOGE("[ERR] bwcs_query_rssi failed. result=%d, expected size=%d", result, sizeof(wmt_bt_query_rssi_ind_struct));
    }
    return result;
}
...
int bwcs_set_rx_range(unsigned char lower_boundary, unsigned char higher_boundary) {
    ...
    LOGI("bwcs_set_rx_range(lower=%d, higher=%d)", lower_boundary, higher_boundary);
...
int bwcs_set_default_tx_power(unsigned char power) {
    ...
    LOGI("bwcs_set_default_tx_power(power=%d)", power);
...
int bwcs_update_conn_tx_power(unsigned int conn_handle, unsigned char power) {
    ...
    int result;
    ilm_struct ilm;
    LOGI("bwcs_update_conn_tx_power(conn_handle=%d, power=%d)", conn_handle, power);
