...
void SM_HandleDiscDelayTimeout(EvmTimer *Timer)
{
    ...
    if (session->smDiscCount == 0 ||
        (HCC(con)[idx].conFlags & HCI_DATA_REQ || HCC(con)[idx].packetsSent > 0) && session->smDiscCount < 5)
    {
...
static void SM_EndSessionFSM(const BtEvent *event)
{
    ...
    case SM_GC_STATE_DISC:
        ...
        if (session->bdc->link->state == BDS_CONNECTED && session->type == SM_PAIR_TYPE_BOND)
        {
    ...
    case SM_GC_STATE_RESET_CNTX:
    {
        ...
        case SM_SESSION_NULL:
            ...
            //MeFreeRemoteDevice(session->bdc->link);
            Report(("bdc->under_used=0x%x", session->bdc->under_used));
            session->bdc->link = NULL;
...
static SmPairFailReason SM_SetPairInfo(SmSession *session, const BtEvent *event)
{
    ...
    if ((event->p.smPairResponse.authRequirements & 0x1) == 0 || MEC(bondable) == FALSE)
    {
        OS_Report("Bondable: local %d, remote %d", MEC(bondable), (event->p.smPairResponse.authRequirements & 0x1));
...
BtStatus SM_Bond(BtDeviceContext *bdc, SmPairAttribute *attr)
{
    ...
    status = ME_LE_CreateLink(&session->smHandler, &bdc->addr, NULL, &remDev);
 ...
 *
 * DESCRIPTION
 *  Phase 1 is defined in Security Manager Spec 5.3 Message Sequence Chart
 *  Including procedures below:
 *
 *  - Bonding procedures
 *    1. Send PAIR REQ
 *    2. Receive PAIR RSP
 *    3. Decide pair model and notify UI
 *
 *  - Pairing procedures
 *    1. Receive PAIR REQ
 *    2. Send PAIR RSP
...
static void SM_Phase1FSM(const BtEvent *event)
{
    ...
    BtRemoteDevice *remDev = SM_GetRemDev(event);
    ...
    while (status == BT_STATUS_SUCCESS)
    {
        ...
        case SM_SESSION_INIT:
            ...
            if (session->type == SM_PAIR_TYPE_BOND)
            {
                ...
                if (btbmNameReq(&BMController.nameTok, session->bdc, btbmHandlerPinCodeAskNameQuery) == BT_STATUS_PENDING)
                {
                    status = BT_STATUS_PENDING;
 ...
 *
 * DESCRIPTION
 *  Phase 2 is defined in Security Manager Spec 5.3 Message Sequence Chart
 *  Including procedures below:
 *
 *  - Bonding procedures
 *    1. Generate 128bit Mrand
 *    2. Calculate 128bit Mconfirm
 *    3. Send Mconfirm
 *    4. Send Mrand after receiving Sconfirm
 *    5. Verify Sconfirm after receiving Srand
 *    6. Calculate STK
 *    7. Save the bond
 *    8. Encrypt Link
 *
 *  - Pairing procedures
 *    1. Generate 128bit Srand
 *    2. Calculate 128bit Sconfirm
 *    3. Send Sconfirm after receiving Mconfirm
 *    4. Verify Mconfirm after receiving Mrand
 *    5. Send Srand after Mconfirm verification succeed
 *    6. Calculate STK
 *    7. Save the bond
 *    8. Encrypt Link by replying LTK
 *
 * PARAMETERS
 *  event     [IN]  
 *
 ...
 *
 * DESCRIPTION
 *  Phase 3 is defined in Security Manager Spec 5.3 Message Sequence Chart
 *  Including procedures below:
 *
 *  - Bonding procedures
 *    1. Receive Slave's LTK/EDIV, Rand/IRK, Identify Address Info/CSRK
 *    2. Generate and distribute LTK
 *    3. Generate and distribute EDIV, Rand
 *    4. If support Privacy, send IRK and Identity Address Info
 *    5. Send CSRK
 *
 *  - Pairing procedures
 *    1. Generate and distribute LTK
 *    2. Generate and distribute EDIV, Rand
 *    3. If support Privacy, send IRK and Identity Address Info
 *    4. Send CSRK
 *
 * PARAMETERS
 *  event     [IN]  
