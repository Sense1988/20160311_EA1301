...
void L2Cap_SendRR(L2CAP_Channel *Channel, U8 index)
{
    ...
    Report(("L2Cap: L2CAP Send RR from:%d recvSeq::%d", index, Channel->recvSeq));
#endif
    if(Channel->inLinkMode == LINK_MODE_ENHANCED_RETRANSMISSION)
    {
...
void L2Cap_SendRNR(L2CAP_Channel *Channel)
{
    Report(("L2Cap: L2CAP Send RNR:0x%x", Channel));
...
void L2Cap_SendSREJ(L2CAP_Channel *Channel, U16 ReqSeq)
{
    Report(("L2Cap: L2CAP Send SREJ:0x%x reqseq:0x%x", Channel, ReqSeq));
    if(Channel->inLinkMode == LINK_MODE_ENHANCED_RETRANSMISSION)
    {
...
{
#if __BT_30_DEBUG__ == XA_ENABLED
    Report(("L2Cap: L2Cap_SendREJ Channel->enhancedRejSent:%d",Channel->enhancedRejSent));
...
{
#if __BT_30_DEBUG__ == XA_ENABLED
    Report(("L2Cap: L2cap send ack Wait_F:%d busy:%d sendFlag:%d", Channel->enhancedWait_F, Channel->enhancedLocalBusy, sendFlag));
#endif
