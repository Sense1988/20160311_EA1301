...
void BTBMGapBondingDevice(bt_bm_bonding_req_struct *btbm_adp_input_msg)
{
    ...
    {
        status = BtbmAllocateFreeDevices((MTKBMC(remote_addr)));
        ...
        bdc = DS_FindDevice(&(MTKBMC(remote_addr)));
    }
    ...
    */
    /* if (bdc->link == 0 || bdc->link->state <= BDS_CONNECTED) */
    {
        ...
        if (bdc->devType == BT_DEV_TYPE_LE)
        {
            ...
            attr.bond = (ME_IsBondable() ? SM_BONDING_YES : SM_BONDING_NO);
        ...
        /*
        if (ME_CreateLink(&linkbmHandler, &(MTKBMC(remote_addr)), &(bdc->psi), &(bdc->link)) == BT_STATUS_NO_RESOURCES)
        {
            btbmSendBondingDeviceCfm(BTBM_ADP_FAILED, tempAddr);
            btbmUtilPairingProcedureClear();
            return;
    ...
    {
        bt_trace(TRACE_GROUP_1, BTBM_CREATE_CONNECTION, 0x02);
        btbmSendBondingDeviceCfm(BTBM_ADP_FAIL_NOT_ALLOW, tempAddr);  //MAUI_01327197
    }
    */
}
 ...
 *  BTBMGetPairedDevice
 * DESCRIPTION
 *  This function is used to get all the paired device info
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void BTBMGetPairedDevice(bt_bm_get_paired_device_req_struct *btbm_adp_input_msg)
{
    BMCallbackParms parms;
    BtDeviceRecord rec;
    int idx = btbm_adp_input_msg->index;
    int i;
    btbm_bd_addr_t* addr;
    
    Report(("BTBMGetPairedDevice, idx=%d, no=%d", btbm_adp_input_msg->index, btbm_adp_input_msg->no));
    OS_MemSet((U8 *)&parms, 0x0, sizeof(BMCallbackParms));
    parms.p.GetPairedDeviceIndication.index = btbm_adp_input_msg->index;

    if (btbm_adp_input_msg->index + btbm_adp_input_msg->no > BTBM_ADP_MAX_PAIRED_LIST_NO)
    {
        btbm_adp_input_msg->no = BTBM_ADP_MAX_PAIRED_LIST_NO - btbm_adp_input_msg->index;
    }

    for (i = 0; i < btbm_adp_input_msg->no; i++)
    {
        if (DDB_EnumDeviceRecords(idx, &rec) == BT_STATUS_SUCCESS)
        {
            addr = &parms.p.GetPairedDeviceIndication.bd_addr;
            parms.p.GetPairedDeviceIndication.index = idx;

            memset(parms.p.GetPairedDeviceIndication.name, 0, BTBM_ADP_MAX_NAME_LEN);
            CONVERT_ARRAY2BDADDR(addr, rec.bdAddr.addr);
            OS_StrnCpy(parms.p.GetPairedDeviceIndication.name, rec.name, BTBM_ADP_MAX_NAME_LEN);
            
            Report(("Found Record : name=%s, addr=%x:%x:%x:%x:%x:%x", rec.name, rec.bdAddr.addr[0], rec.bdAddr.addr[1], rec.bdAddr.addr[2], rec.bdAddr.addr[3], rec.bdAddr.addr[4], rec.bdAddr.addr[5]));
            
            parms.event = BTBM_EVENT_GET_PAIRED_DEVICE_INDICATION;
            MTKBMC(bmCB)(&parms);
            idx++;
        }
        else
        {
            parms.p.GetPairedDeviceResult.result = BTBM_ADP_SUCCESS;
            parms.p.GetPairedDeviceResult.no = i;
            parms.event = BTBM_EVENT_GET_PAIRED_DEVICE_RESULT;
            MTKBMC(bmCB)(&parms);
            return;
        }
    }
 ...
 *  void
 *****************************************************************************/
U8 SSP_MITM_Mode=0;     // Default : MITM Not Required
U8 SSP_Bonding_Mode=1;  // Default : Dedicate Bonding
U8 SSP_IO_Capability=BT_SSP_IO_CAPABILITY;
void BTBMGapSetSSPMitmMode(bt_bm_set_ssp_mitm_req_struct *btbm_adp_input_msg)
{
    BMCallbackParms parms;
    SSP_MITM_Mode = btbm_adp_input_msg->mode;
    
    parms.event = BTBM_EVENT_SET_SSP_MITM_RESULT;
    parms.p.result = 1;
    MTKBMC(bmCB)(&parms);
}
 ...
 *  void
 *****************************************************************************/
void BTBMGapSetSSPBondingMode(bt_bm_set_ssp_bonding_req_struct *btbm_adp_input_msg)
{
    BMCallbackParms parms;
    SSP_Bonding_Mode = btbm_adp_input_msg->mode;

    parms.event = BTBM_EVENT_SET_SSP_BONDING_RESULT;
    parms.p.result = 1;
    MTKBMC(bmCB)(&parms);
}
 ...
 *  void
 *****************************************************************************/
void BTBMGapSetSSPIoCapability(bt_bm_set_ssp_io_capability_req_struct *btbm_adp_input_msg)
{
    BMCallbackParms parms;
    SSP_IO_Capability = btbm_adp_input_msg->mode;

    parms.event = BTBM_EVENT_SET_SSP_IO_CAPABILITY_RESULT;
    parms.p.result = 1;
    MTKBMC(bmCB)(&parms);
}
...
void btbmHandlerPinCodeRequest(const BtEvent *Event)
{
    ...
    else
    {
        ...
        if (bdc == 0)
        {
            ...
            status = BtbmAllocateFreeDevices(Event->p.remDev->bdAddr);
            if (status == BT_STATUS_FAILED)
            {
                SEC_SetPinByAddr(Event->p.remDev->bdAddr, 0x00, 0x00, BPT_SAVE_NOT_TRUSTED);
...
void btbmHandleIoCapabilityRequest(BtRemoteDevice *remDev)
{
    ...
    switch (MTKBMC(bm_opactivity))
    {
        ...
        case BM_OP_NONE:
            ...
            {
                bt_trace(BT_TRACE_G1_PROTOCOL, BT_BTBM_BDC_IS_ZERO);
                status = BtbmAllocateFreeDevices(remDev->bdAddr);
                if (status == BT_STATUS_FAILED)
                {
        ...
        case BM_OP_PAIRING:
            ...
            if (ME_IsBondable() == FALSE)
            {
                /* Under non-bondable mode, only accept pairing with no_bonding requirement */
                bonding_mode = BT_SSP_MITM_REQUIRED_NO_BONDING;
                    ...
                    MTKBMC(SSPDeviceInfo).authRequirements != BT_SSP_MITM_REQUIRED_NO_BONDING)
                {
                 ...
                 /* Both set as no_bonding, should delete bond information after link disconnect */
                remDev->pairingType = BPT_NOT_SAVED;
            }

            #ifdef __PRJ_MT7650_SONY_BDP__
            if ( SSP_MITM_Mode == 0 )   // MITM Not Required
            {
                if ( SSP_Bonding_Mode == 0 )    // No Bonding
                {
                    bonding_mode = BT_SSP_MITM_NOT_REQUIRED_NO_BONDING;
                    Report(("SSP MITM & Bonding Setting : BT_SSP_MITM_NOT_REQUIRED_NO_BONDING"));
                }
                else if ( SSP_Bonding_Mode == 1 )   // Dedicated Bonding
                {
                    bonding_mode = BT_SSP_MITM_NOT_REQUIRED_DEDICATED_BONDING;
                    Report(("SSP MITM & Bonding Setting : BT_SSP_MITM_NOT_REQUIRED_DEDICATED_BONDING"));
                }
                else    // General BOnding
                {
                    bonding_mode = BT_SSP_MITM_NOT_REQUIRED_GENERAL_BONDING;
                    Report(("SSP MITM & Bonding Setting : BT_SSP_MITM_NOT_REQUIRED_GENERAL_BONDING"));
                }
            }
            else    // MITM Required
            {
                ...
                {
                    bonding_mode = BT_SSP_MITM_REQUIRED_NO_BONDING;
                    Report(("SSP MITM & Bonding Setting : BT_SSP_MITM_REQUIRED_NO_BONDING"));
                }
                else if ( SSP_Bonding_Mode == 1 )   // Dedicated Bonding
                {
                    bonding_mode = BT_SSP_MITM_REQUIRED_DEDICATED_BONDING;
                    Report(("SSP MITM & Bonding Setting : BT_SSP_MITM_REQUIRED_DEDICATED_BONDING"));
                }
                else    // General BOnding
                {
                    bonding_mode = BT_SSP_MITM_REQUIRED_GENERAL_BONDING;
                    Report(("SSP MITM & Bonding Setting : BT_SSP_MITM_REQUIRED_GENERAL_BONDING"));
                }
            }
            #endif
            break;
                ...
                U8 *key)
{
    ...
    CONVERT_BDADDRSRC2ARRAY(BdAddr.addr, &bd_addr);
