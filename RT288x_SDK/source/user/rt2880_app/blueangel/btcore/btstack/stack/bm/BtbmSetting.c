...
void BTBMGapWriteScanEnabled(bt_bm_write_scanenable_mode_req_struct *btbm_adp_input_msg)
{
    ...
    {
        ME_LE_SetAccessible((BtAccessibleMode) btbm_adp_input_msg->mode, NULL);
    ...
    if (MEC(localNameLen) > 0)
    {
        ME_LE_SetAccessible(btbm_adp_input_msg->mode, NULL);  /* Wait for name config */
    }
#endif
#endif
    ...
    {
        Assert(MTKBMC(bm_opactivity) == BM_OP_NONE);
        ME_SetAccessibleModeNC((BtAccessibleMode) btbm_adp_input_msg->mode);
        parms.p.result = BTBM_ADP_SUCCESS;
    }
    else
    {
...
void BTBMGapWriteBondable(bt_bm_write_bondable_mode_req_struct *btbm_adp_input_msg)
{
    BMCallbackParms parms;
    ME_SetBondableMode(btbm_adp_input_msg->bondable);
    parms.event = BTBM_EVENT_WRITE_BONDABLE_MODE_RESULT;
    parms.p.result = BTBM_ADP_SUCCESS;
    MTKBMC(bmCB)(&parms);
}
...
void BTBMLocalWriteScanEnabled(U8 mode)
{
    ...
    Report(("BTBMLocalWriteScanEnabled:%02x,%02x,%02x,", mode, scan_enabled_mode, MTKBMC(bm_opactivity)));
 ...
 * DESCRIPTION
 *  Get current Bluetooth properties
 * PARAMETERS
...
void BTBMGapGetWholeProperty(void)
{
    ...
    if ( MTKBMC(bmCB) == 0 )
    {
        Report(("BTBMGapGetWholeProperty() invoked before BlueAngel been enabled !!!\n"));
        return;
    }
    
    parms.event = BTBM_EVENT_READ_PROPERTY_RESULT;
    CONVERT_ARRAY2BDADDR(&parms.p.ReadPropertyResult.bd_addr, MEC(bdaddr).addr);
    parms.p.ReadPropertyResult.cod = MEC(classOfDevice);
    parms.p.ReadPropertyResult.powered = BTCoreVerifySysInitState();
    parms.p.ReadPropertyResult.discoverable = ((MEC(accModeExpValue) & BAM_DISCOVERABLE_ONLY) ? TRUE : FALSE);
    parms.p.ReadPropertyResult.connectable = ((MEC(accModeExpValue) & BAM_CONNECTABLE_ONLY) ? TRUE : FALSE);
    parms.p.ReadPropertyResult.pairable = MEC(bondable);
    parms.p.ReadPropertyResult.disctimeout = 40000; /* ms */
    parms.p.ReadPropertyResult.pairtimeout = 200000; /* ms */
    if (BMController.bm_opactivity == BM_OP_INQUIRY || btbmUtilIsInquiryNameRequestOngoing())
    {
        parms.p.ReadPropertyResult.discovering = TRUE;
    }
    else
    {
        parms.p.ReadPropertyResult.discovering = FALSE;
    }
    parms.p.ReadPropertyResult.name = (char *)MTKBMC(bm_setting).local_name;
    MTKBMC(bmCB)(&parms);
    return;
}
#endif
