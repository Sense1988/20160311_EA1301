...
extern BtStatus BT_ATT_ForGattExchangeMTURequest(BtRemoteDevice *link);
extern BtStatus BT_ATT_SendReadByGroupTypeRequest(BtATTReadByGroupTypeRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendFindByTypeValueRequest(BtATTFindByTypeValueRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendReadByTypeRequest(BtATTReadByTypeRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendReadRequest(BtATTReadRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendFindInfoRequest(BtATTFindInfoRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendReadBlobRequest(BtATTReadBlobRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendReadMultipleRequest(BtATTReadMultipleRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendWriteRequest(BtATTWriteRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendPrepareWriteRequest(BtATTPrepareWriteRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendPrepareWriteRequest(BtATTPrepareWriteRequestStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendHandleValueNotification(BtATTValueNotificationStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendWriteCommand(BtATTWriteCommandStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendSignedWriteCommand(BtATTSignedWriteCommandStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendHandleValueIndication(BtATTValueIndicationStruct *ptr, BtRemoteDevice *link);
extern BtStatus BT_ATT_SendExecuteWriteRequest(BtATTExecuteWriteRequestStruct *ptr, BtRemoteDevice *link);


extern void GATT_HandleOperation(BtRemoteDevice *link);


void GattSetOpTypeNull(BtOperation *op)
{
    op->opType = 0;
}
...
void GattHandleExchangeMTUOp(U8 event, BtRemoteDevice *link)
{
    ...
    switch (event)
    {
        ...
        case BT_GATT_OP_ERROR:
            ...
            GattSetOpTypeNull(conn->gattRequestCurOp);
            break;
    }
	conn->gattRequestCurOp = 0;
...
void GattHandleFindIncludeServiceOp(U8 event, BtRemoteDevice *link)
{
	...
	//BtATTReadResponseStruct *ptr1 = NULL;
		...
		case BT_GATT_OP_END:
			...
			else if(ptr->length == 0x15)
			{
				...
				while (remain_length !=0)
				{
					...
					gatt_event.charProperty = ptr->handleDataList[offset+2];
					gatt_event.charValueAttributeHandle = LEtoHost16(ptr->handleDataList+3+offset);
					gatt_event.charUUID126 = ptr->handleDataList+5+offset;
					gatt_event.type = 0x10;
					Report(("[GATT]gatt_event.charProperty:%d",gatt_event.charProperty));
					Report(("[GATT]gatt_event.charValueAttributeHandle:%d",gatt_event.charValueAttributeHandle)); 			
					Report(("[GATT]gatt_event.charUUID:%d",gatt_event.charUUID)); 			
		...
		case BT_GATT_OP_END:
			...
			else if(ptr->length == 0x15)
			{
				...
				while (remain_length !=0)
				{
					...
					gatt_event.charProperty = ptr->handleDataList[offset+2];
					gatt_event.charValueAttributeHandle = LEtoHost16(ptr->handleDataList+3+offset);
					gatt_event.charUUID126 = (U8 *)(ptr->handleDataList+5+offset);
					gatt_event.type = 0x10;
					if(AttUuidCmp(ptr->handleDataList+5+offset, 0x02, tempUUID, parms->type) == TRUE)
					{
						Report(("[GATT]gatt_event.charProperty:%d",gatt_event.charProperty));
...
void GattHandleReliableWriteOp(U8 event, BtRemoteDevice *link)
{
    ...
    switch (event)
    {
		...
		case BT_GATT_OP_END:
            ...
            {
                ptr = (BtATTPrepareWriteResponseStruct*)conn->attEvent->parms;
                if((ptr->valueOffset != parms->offset) ||
                    (OS_MemCmp(ptr->partAttributeValue ,ptr->partAttributeValueLen,
                        parms->data+parms->offset, ptr->partAttributeValueLen) == FALSE))
                {
                    /* Flush old data   */
...
void GattHandleNotificationOp(U8 event, BtRemoteDevice *link)
{
    ...
    switch (event)
    {
        ...
        case BT_GATT_OP_END:
            ...
            GattSetOpTypeNull(conn->gattNotificationCurOp);
            op->apCb( op, &conn->gattEvent); 		
...
void GattHandleIndicationsOp(U8 event, BtRemoteDevice *link)
{
    ...
    switch (event)
    {
		...
		case BT_GATT_OP_LINK_DISCONNECTED:
            ...
            gatt_event.flag = BT_GATT_EVENT_RESULT_DISCONNECT; /* Report event error*/
            GattSetOpTypeNull(conn->gattIndicationCurOp);
