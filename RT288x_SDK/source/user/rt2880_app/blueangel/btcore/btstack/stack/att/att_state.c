...
void BT_ATT_HandleTxDone(BtRemoteDevice *link, BtPacket *pkt)
{
    ...
    switch(pkt->data[0])
    {
        ...
        case BT_ATT_OPCODE_SIGNED_WRITE_COMMAND:
            BT_ATT_ReportEvent(BT_ATT_EVENT_NOTIFIED_PACKET_HANDLED, &ATTC(conn)[con_id], pkt->llpContext);
...
BtStatus AttClientRequest(BtRemoteDevice *link)
{
    ...
    if(BT_ATT_GetConId(link) == MAX_ATT_CON_NO)
    {
        ...
        if(ME_GetDeviceType(&link->bdAddr) == BT_DEV_TYPE_LE)
            ...
            ATTC(conn)[i].secRecord.level = BSL_AUTHORIZATION_IN | BSL_ENCRYPTION_IN | BSL_ENCRYPTION_OUT;
...
void AttSecCallback(const BtEvent *Event)
{
    ...
    if (Event->p.secToken->incoming == FALSE)
    {
        if (Event->eType == BTEVENT_ACCESS_APPROVED)
        {
            ...
            {
                L2CAP_LinkConnectedByCid(Event->p.secToken->remDev->bdAddr, 0x0004);
                return;
            }
            else
            {
