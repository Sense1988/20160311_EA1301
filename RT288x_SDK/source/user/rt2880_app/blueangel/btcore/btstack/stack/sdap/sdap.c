...
extern void BTBMAdpAttributeSearchComplete(kal_uint8 result, btbm_bd_addr_t addr, kal_uint16 len, kal_uint8 *data);
#else
extern void BTBMAdpAttributeSearchComplete(kal_uint8 result, btbm_bd_addr_t addr);
#endif
...
U8 SdapUUID128MultipleRfcommGroupServiceSearchAttribReq[] = 
{
    ...
    SDP_UUID_128BIT(SC_SYNCML_BT_SERVER_128),   /* UUID-128 bits for SyncML BT Server in Big Endian */
    SDP_UUID_16BIT(PROT_L2CAP),     /* L2CAP UUID in Big Endian */
    SDP_UUID_16BIT(PROT_RFCOMM),    /* UUID for RFCOMM in Big Endian */
...
#define SC_USER_DEFINED_128 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00
U8 SdapUserDefineServiceSearchAttribReq[] = 
{
    /*
     * First parameter is the search pattern in data element format. It
     * is a list of 3 UUIDs. 
     */
    SDP_ATTRIB_HEADER_8BIT(23),

    SDP_UUID_128BIT(SC_USER_DEFINED_128), 
    SDP_UUID_16BIT(PROT_L2CAP),     /* L2CAP UUID in Big Endian */
    SDP_UUID_16BIT(PROT_RFCOMM),    /* UUID for RFCOMM in Big Endian */
...
BtStatus SDAP_ServiceSearchMultipleAttribute(sdap_service_search_multi_attribute_struct search_pattern)
{
    ...
    if (SDAP(progress) == SDAP_PROGRESS_NONE)
    {
        ...
        if (A2DP_GetSdpQueryCount() == 1)
        {
	     ...
	     if (search_pattern.uuid == SC_AV_REMOTE_CONTROL_TARGET)
	     {
	     		if (SDAP(sdap_current_multi_req_index) == SDAP(sdap_multi_req_index))
	            {
	                SDAP(sdap_current_multi_req_index) = 0;
	            ...
	            else
	            {
	                /* In this case: Existing one pending sdp request from other profile, reject this 2nd a2dp sdp request */
	                //bt_trace(TRACE_GROUP_1, BT_LOG_SDAP_REJECT_SECOND_A2DP_SDP_DUE_TO_ONE_PENDING);
	                             Report(("AvrcpSdpQuery  BT_STATUS_FAILED"));
					return BT_STATUS_FAILED;
	            }
	     }
...
#ifdef __PRJ_MT7650_SONY_BDP__
            case SC_USER_DEFINED:
                SDAP(multiple_attribute_group) = SdapUserDefineServiceSearchAttribReq;
                SDAP(multiple_attribute_group_size) = sizeof(SdapUserDefineServiceSearchAttribReq);
                sqt->uuid = SC_USER_DEFINED;
                SDAP(attribute_offset) = 29;
                uuid_offset = 3;
                break;
#endif
...
#ifdef __BT_SUPPORT_SYNCML__        
        if (sqt->uuid != SC_SYNCML_BT_SERVER && sqt->uuid != SC_SYNCML_BT_CLIENT)
        {
...
#else
#ifdef __PRJ_MT7650_SONY_BDP__
        if ( SC_USER_DEFINED == uuid )
        {
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid1);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid2);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid3);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid4);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid5);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid6);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid7);
            uuid_offset+=2;
            StoreBE16(SDAP(multiple_attribute_group)+uuid_offset,search_pattern.uuid8);
            uuid_offset+=2;
        }
        else
#endif
...
BtStatus SDAP_ServiceSearchAll(sdap_service_search_all_struct search_pattern)
{
    ...
    if (SDAP(progress) == SDAP_PROGRESS_NONE)
    {
		...
		OS_Report("Failed to queue sdap service search all request.");
		return BT_STATUS_FAILED;
	}
#endif
    else
...
static void query_sdap_service_search_all_callback(const BtEvent *Event)
{
    ...
    switch (Event->eType)
    {
        ...
        case SDEVENT_QUERY_ERR:
            /* CONTINUE, SDAP operation query error */
        case SDEVENT_QUERY_FAILED:
            if (Event->p.token->rm == NULL) //klocwork
                break;
            if (Event->p.token->rm != 0)
            {
                Event->p.token->rm->discFlag = TRUE;
...
#ifdef __SDAP_REQUEST_QUEUE__
	OS_Report("sdap_process_next_request");
	SdapQueuedRequest *req = sdapDequeueRequestSameAddr(&remDev->bdAddr);
	if (req == NULL) {
		OS_Report("No SDAP request for the same device.");
	...
	if (req != NULL) {
		...
		switch (req->req_type) {
			...
			case SDAP_REQUEST_MULTIPLE_ATTRIBUTE:
				SDAP_ServiceSearchMultipleAttribute(req->params.multi_attr);
				return;
			default:
				break;
		}
	} else {
		OS_Report("No any queued sdap request.");
	}
...
static SdapQueuedRequest *sdapGetAvailableRequestStruct()
{
	...
	if (ret == NULL) {
		OS_Report("No available sdap request struct.");
	}
	return ret;
}
...
static void sdapQueuedRequestTimeout(EvmTimer *timer)
{
	...
	OS_Report("sdapQueuedRequestTimeout(), req_type: %d", req->req_type);
	switch (req->req_type) {
		case SDAP_REQUEST_ALL:
...
static BtStatus sdapEnqueueRequestAll(sdap_service_search_all_struct search_pattern)
{
	...
	OS_Report("sdapEnqueueRequestAll()");
	if (req != NULL) {
...
static BtStatus sdapEnqueueRequestMultipleAttribute(sdap_service_search_multi_attribute_struct search_pattern)
{
	...
	OS_Report("sdapEnqueueRequestMultipleAttribute()");
	if (req != NULL) {
...
static SdapQueuedRequest *sdapDequeueRequestFromHead()
{
	...
	if (!IsListEmpty(&SDAP(sdap_request_queue))) {
		...
		OS_Report("Get queued sdap request, type: %d", ret->req_type);
...
static void sdapRemoveRequestFromQueue(SdapQueuedRequest *req)
{
	...
	OS_Report("Sdap failed to remove the request from queue");
}
...
static void sdapClearRequestSameAddr(BD_ADDR *bdaddr)
{
			...
			cur_req = next_req) 
    {
		if (cur_req && OS_MemCmp(bdaddr->addr, 6, cur_req->bdAddr.addr, 6) == TRUE) 
		{
			...
			switch (cur_req->req_type == SDAP_REQUEST_ALL) 
			{
				case SDAP_REQUEST_ALL:
