...
#ifdef ANDROID
#define bt_ext_log(fmt, ...) LOGI(fmt,## __VA_ARGS__)
#else
#define bt_ext_log(fmt, ...) OS_Report(fmt,## __VA_ARGS__)
#endif
static pthread_mutex_t avrcpsendcmd_mutex = PTHREAD_MUTEX_INITIALIZER;

extern U8 BTBMProfileAllocateConnIndex(U32 uuid, BD_ADDR addr);
extern BOOL BTBMProfileReleaseConnIndex(U8 index);

extern void AvrcpHandleActivateCnf(U8 chnl_num, U16 result);
extern void AvrcpHandleDeactivateCnf(U8 chnl_num, U16 result);
extern void AvrcpHandleConnectCnf(U8 chnl_num, U16 result, bt_avrcp_dev_addr_struct *dev_addr, U8 connection_id, U16 browse_channel, U16 ctFeature,U16 tgFeature, U16 tgVersion);
...
extern void AvrcpHandleBrowseDisconnectCnf(U8 chnl_num, U16 reslult);
...
extern void AvrcpHandleCommandFrameInd(U8 chnl_num, U16 result, U8 seq_id, U8 profile_id, U8 c_type, U8 subunit_type, U8 subunit_id, U16 data_len, U8 *frame_data);
...
AvrcpCallList  avrcpCallList;

BOOL AddAvrcpCallList(U8 c_type, U8 subunit_type, U8 subunit_id, U16 data_len, U8* frame_data)
{
       pthread_mutex_lock (&avrcpsendcmd_mutex);
       bt_ext_log("[AVRCP] AddAvrcpCallList:c_type=0x%x,subunit_type=0x%x,subunit_id=0x%x,data_len=%d,frame_data[0]=0x%x,frame_data[1]=0x%x\n",c_type,subunit_type,subunit_id,data_len, frame_data[0],frame_data[1]);
       if (avrcpCallList.fullflag == 1)
       {
       	bt_ext_log("[AVRCP] AddAvrcpCallList 111:\n");
		pthread_mutex_unlock (&avrcpsendcmd_mutex);	
		return FALSE;
       }
       	
	if (avrcpCallList.calllist[avrcpCallList.rearposition].useflag == 1)
	{
		//i++;
		bt_ext_log("[AVRCP] AddAvrcpCallList 222:\n");
		pthread_mutex_unlock (&avrcpsendcmd_mutex);	
		return FALSE;
	}
	else
	{
        bt_ext_log("[AVRCP] AddAvrcpCallList 333:\n");
        avrcpCallList.calllist[avrcpCallList.rearposition].cmdFrame.ctype = c_type;
		avrcpCallList.calllist[avrcpCallList.rearposition].cmdFrame.subunitType = subunit_type;
		avrcpCallList.calllist[avrcpCallList.rearposition].cmdFrame.subunitId = subunit_id;
		avrcpCallList.calllist[avrcpCallList.rearposition].cmdFrame.opcode = frame_data[0];;
		avrcpCallList.calllist[avrcpCallList.rearposition].cmdFrame.operandLen = data_len - 1;
	
		OS_MemCopy((U8 *)&(avrcpCallList.calllist[avrcpCallList.rearposition].frame_data[0]), (const U8 *) &frame_data[1], data_len - 1);
		avrcpCallList.calllist[avrcpCallList.rearposition].cmdFrame.operands = (U8 *)&(avrcpCallList.calllist[avrcpCallList.rearposition].frame_data[0]);

		avrcpCallList.calllist[avrcpCallList.rearposition].useflag = 1;

             bt_ext_log("[AVRCP] AddAvrcpCallList 444:\n");
		if ((avrcpCallList.rearposition + 1 % BT_AVRCP_CALL_MAX) ==avrcpCallList.frontposition)
		{bt_ext_log("[AVRCP] AddAvrcpCallList 555:\n");
                        avrcpCallList.fullflag = 1;
			avrcpCallList.rearposition =(avrcpCallList.rearposition + 1)%BT_AVRCP_CALL_MAX;
		}
		else
		{bt_ext_log("[AVRCP] AddAvrcpCallList 666:\n");
			avrcpCallList.rearposition =(avrcpCallList.rearposition + 1)%BT_AVRCP_CALL_MAX;
		}
		avrcpCallList.emptyflag = 0;

	}
	pthread_mutex_unlock (&avrcpsendcmd_mutex);	
			
	bt_ext_log("[AVRCP] AddAvrcpCallList 777:\n");
	return TRUE;
}


BOOL RemoveAvrcpCallList(/*avrcp_adp_context *cntx_ptr,U8 * frame_data*/)
{
          pthread_mutex_lock (&avrcpsendcmd_mutex);	
          bt_ext_log("[AVRCP] RemoveAvrcpCallList 111:\n");
          if (avrcpCallList.emptyflag == 1)
          {
          	bt_ext_log("[AVRCP] RemoveAvrcpCallList 222:\n");
			pthread_mutex_unlock (&avrcpsendcmd_mutex);	
			return FALSE;
          }
		  	
	   //if ((avrcpCallList.rearposition  ==avrcpCallList.frontposition)  && (avrcpCallList.fullflag !=1))
	   	
	   	//return FALSE;
         bt_ext_log("[AVRCP] RemoveAvrcpCallList 333:\n");
       avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.ctype = 0;
	avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.subunitType = 0;
	avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.subunitId = 0;
	avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.opcode = 0;
	avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.operandLen = 0;
       bt_ext_log("[AVRCP] RemoveAvrcpCallList 4444:\n");
	memset(&(avrcpCallList.calllist[avrcpCallList.frontposition].frame_data[0]),0,BT_AVRCP_MAX_CMD_FRAME_SIZE);
	
	avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.operands = NULL;
       bt_ext_log("[AVRCP] RemoveAvrcpCallList 555:\n");
	avrcpCallList.calllist[avrcpCallList.frontposition].useflag = 0;

      if (avrcpCallList.fullflag == 1)
	  	avrcpCallList.fullflag = 0;

bt_ext_log("[AVRCP] RemoveAvrcpCallList 6666:\n");
      if ((avrcpCallList.frontposition  == avrcpCallList.rearposition) ||( (avrcpCallList.frontposition + 1)%BT_AVRCP_CALL_MAX ==avrcpCallList.rearposition))
	{bt_ext_log("[AVRCP] RemoveAvrcpCallList 777:\n");
              avrcpCallList.emptyflag = 1;
		//avrcpCallList.rearposition =(avrcpCallList.rearposition + 1)%10;
		avrcpCallList.frontposition = (avrcpCallList.frontposition + 1)%BT_AVRCP_CALL_MAX;
	}
	else
	{bt_ext_log("[AVRCP] RemoveAvrcpCallList 888:\n");
		avrcpCallList.frontposition = (avrcpCallList.frontposition + 1)%BT_AVRCP_CALL_MAX;
	}
bt_ext_log("[AVRCP] RemoveAvrcpCallList 999:\n");
       pthread_mutex_unlock (&avrcpsendcmd_mutex);	
	return TRUE;
...
BOOL AvrcpCallIsEmpty()
{
	return (avrcpCallList.emptyflag == 1)?TRUE:FALSE;
}

void GetAvrcpCallData(avrcp_adp_context **p_cntx_ptr)
{     
       pthread_mutex_lock (&avrcpsendcmd_mutex);	
       bt_ext_log("[AVRCP] RemoveAvrcpCallList GetAvrcpCallData1111:\n");
	(*p_cntx_ptr)->cmdFrame.ctype = avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.ctype;
	bt_ext_log("[AVRCP] RemoveAvrcpCallList GetAvrcpCallData222:\n");
	(*p_cntx_ptr)->cmdFrame.subunitType = avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.subunitType;
	(*p_cntx_ptr)->cmdFrame.subunitId = avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.subunitId;
	(*p_cntx_ptr)->cmdFrame.opcode = avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.opcode;
	(*p_cntx_ptr)->cmdFrame.operandLen = avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.operandLen;
       (*p_cntx_ptr)->cmdFrame.operands = avrcpCallList.calllist[avrcpCallList.frontposition].cmdFrame.operands;
	bt_ext_log("[AVRCP] RemoveAvrcpCallList GetAvrcpCallData777:\n");
	pthread_mutex_unlock (&avrcpsendcmd_mutex);	
}
 ...
 *  void
 *****************************************************************************/
void AVRCP_UitlActivateReq(U8 chnl_num, U8 local_role)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    kal_trace( BT_TRACE_G8_AVRCP, BT_AVRCP_UTIL_ACTIVATE_REQ, chnl_num, local_role);
     ...
     bt_ext_log("[AVRCP] AVRCP_UitlActivateReq:\n");
    if( BTCoreVerifySysInitState() == FALSE )
    {
        AvrcpHandleActivateCnf(chnl_num, BT_AVRCP_RESULT_BAD_STATE);
        return;
    }  
    ...
    if (status == BT_STATUS_SUCCESS)
    {
        ...
        AvrcpHandleActivateCnf(chnl_num, BT_AVRCP_RESULT_SUCCESS);
    }
    else
    {
        AvrcpHandleActivateCnf(chnl_num, BT_AVRCP_RESULT_ERROR);
    }
}
...
void AVRCP_UitlDeactivateReq(U8 chnl_num)
{
    ...
    #ifndef __PRJ_MT7650_SONY_BDP__ // Don't auto disconnect since it will cause ASSERT later.
...
void AVRCP_UitlConnectReq(U8 chnl_num, U8 local_role, bt_avrcp_dev_addr_struct* avrcp_device_addr)
{
    ...
    avrcp_adp_context *cntx_ptr = NULL;
    U32 uuid_role = 0;
    bt_ext_log("[AVRCP] AVRCP_UitlConnectReq:\n");
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    {
        AvrcpHandleConnectCnf(chnl_num, BT_AVRCP_RESULT_BAD_STATE, avrcp_device_addr, 0, 0, 0, 0, 0);
        return;
    }
    ...
    if (status == BT_STATUS_SUCCESS)
    {
        ...
        AvrcpHandleConnectCnf(chnl_num, BT_AVRCP_RESULT_SUCCESS, avrcp_device_addr, cntx_ptr->connect_id,
            ...
            cntx_ptr->rcp_channel.remoteTGSdp.tgVersion
            );
    }
    /* Connecting*/
    ...
    {
        AvrcpHandleConnectCnf(chnl_num, BT_AVRCP_RESULT_ERROR, avrcp_device_addr, 0, 0, 0, 0, 0);
    }
...
void AVRCP_UitlConnectIndRes(U8 chnl_num, BOOL accept_conn, BOOL pending_conn)
#else
...
#endif
{
    ...
    if (cntx_ptr->state == BT_AVRCP_STATE_ACCEPTING_CONNECT)
    {
        ...
        if ( pending_conn )
        {
            status = AVRCP_ConnectRspPending(&cntx_ptr->rcp_channel);
        }
        else
        {
#endif
...
void AVRCP_UitlCmdFrameReq(U8 chnl_num, U8 c_type, U8 subunit_type, U8 subunit_id, U16 data_len, U8* frame_data)
{
...
#else
    //Report(("A2dpAvdtpEventCallback, event:%d",event));
    //bt_ext_log("[AVRCP] AVRCP_UitlCmdFrameReq: %d\n", msg->stream_handle);
    bt_ext_log("[AVRCP] AVRCP_UitlCmdFrameReq:\n");
    AddAvrcpCallList(c_type, subunit_type,subunit_id, data_len, frame_data);
    ...
    if (status == BT_STATUS_SUCCESS)
    {
    	OS_Report("[AVRCP] AVRCP_UitlCmdFrameReq  ### ");
        AvrcpHandleCommandFrameCnf(
            chnl_num,
 ...
 *  void
 *****************************************************************************/
void AVRCP_UitlCmdFrameRspInd(U8 chnl_num, U8 seq_id, U8 c_type,  U8 subunit_type, U8 subunit_id, U16 data_len, U8* frame_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void AVRCP_UitlBrowseDisconnectReq(U8 chnl_num)
{
    ...
    if ( !AVRCP_IsBrowseExisted(&cntx_ptr->rcp_channel) )
    {
        ...
        AvrcpHandleBrowseDisconnectCnf(chnl_num, BT_AVRCP_RESULT_SUCCESS);
        return;
    }
	...
	cntx_ptr->rcp_channel.bwsDiscFlag = TRUE;
    // wait for event back
}
 ...
 *  void
 *****************************************************************************/
void AVRCP_UtilSendAVRCPNotImplement(AvrcpChnlInfo *chnl_info, U8 datalen, U8 *data){
    AvrcpQueuePacketFrame *qPacket;
	BtStatus status = BT_STATUS_FAILED;
    avrcp_adp_context *cntx_ptr = NULL;
...
void AVRCP_UtilAVRCPGetPlayerValueTextRsp(bt_avrcp_get_playerapp_valuetext_rsp_struct *rsp){
	...
	if( FALSE == rsp->reject ){
        ...
        // spec 6.5.5 page 54
        for( i = 0; i< buffer[0] && i < BT_AVRCP_MAX_ATTRIBUTE_VALUE_NUM && ((buffer_len+4+rsp->value_string_len[i]) < sizeof(buffer) ) ; i++ ){
...
void AVRCP_UtilAVRCPCmdRegisterNotificationRsp_PlaySettingChange(bt_avrcp_register_notification_rsp_struct *rsp){
    ...
    data[len++] = rsp->count;
    for(i =0 ; i < rsp->count && (len+2) < sizeof(data) ; i++ ){
...
/// Make sure the buffer size is less then max length of l2cap payload
	...
	if( FALSE == rsp->reject ){
		...
		while( i < rsp->counter && remote_mtu > ( (U32)buffer_len + 14 + rsp->players[i].name_len ) ) {
			...
			if ( buffer_len >= BT_AVRCP_MAX_RSP_FRAME_SIZE ) break;
			// add it to buffer
        ...
        for( i = 0; i < rsp->counter && FALSE == stop; i++){
            ...
            switch( rsp->items[i].item_type ){
                case 0x01: // play item
                {
					...
					if( (31 + rsp->items[i].name_length) > BT_AVRCP_MAX_RSP_FRAME_SIZE ){
						stop = TRUE;
						break;
					}
					if ( buffer_len < BT_AVRCP_MAX_RSP_FRAME_SIZE ) //klocwork warning
                ...
                case 0x03: // element ietm
                {
					...
					if ( attribute_count_offset < BT_AVRCP_MAX_RSP_FRAME_SIZE ) //klocwork warning
...
/// Browse cmd
	...
	if( FALSE == rsp->reject ){
		...
		for( i = 0; i < rsp->folder_depth && i < BT_AVRCP_MAX_FOLDER_DEPTH_NUM ; i++ ){
            ...
            if(  (buffer_len+2+rsp->foldername_len[i]) > remote_mtu ){
                // too much data for buffer
                break;
			}
		...
		if( i > 0 && i != rsp->folder_depth){
		    if ( size_offset < BT_AVRCP_MAX_RSP_FRAME_SIZE ) //klocwork warning
...
{	
	Report(("[BT] [AVRCP] AVRCP_UtilAVRCPSendCmd_Register_Nofitication_Req"));
	U8 *buffer = g_avrcp_util_marshall_buffer;
	U16 buffer_len = 0;
	U8 opcode = AVRCP_SPECIFIC_OPCODE_REGISTER_NOFITICATION;
	AvrcpChnlInfo myChnlInfo;
	...
	if( BT_STATUS_PENDING != AVRCP_UtilAVRCPCmdBufferRsp( &myChnlInfo, AVCTP_CTYPE_NOTIFY, opcode, buffer_len, buffer) ){
		Report(("[BT] [AVRCP] AVRCP_UtilAVRCPSendCmd_Register_Nofitication_Req fail !"));
...
#ifdef __PRJ_MT7650_SONY_BDP__
static U8 PTS_AVRCP_RCR_BV_03_C = 0;
#endif
...
//    BtStatus status;
    ...
    if (cntx_ptr->state == BT_AVRCP_STATE_CONNECTING)
    {
		...
		kal_trace( BT_TRACE_G8_AVRCP, BT_AVRCP_SDP_TG_INFO, cntx_ptr->rcp_channel.remoteCTSdp.ctAvailable,
		    ...
		    cntx_ptr->rcp_channel.remoteTGSdp.tgFeatureFlag,
		    cntx_ptr->rcp_channel.remoteTGSdp.tgSupportBrowse);
		if( cntx_ptr->rcp_channel.remoteTGSdp.tgAvailable != FALSE && 0x00 != cntx_ptr->rcp_channel.remoteTGSdp.tgSupportBrowse ){
...
static void AvrcpUtilCallbackDisconnect(AvrcpChannel *chnl, AvrcpCallbackParms *Parms)
{
    ...
    if (cntx_ptr->state == BT_AVRCP_STATE_CONNECTING)
    {
        ...
        AVRCP_CONVERT_ARRAY2BDADDR(&device_addr,Parms->p.remDev->bdAddr.addr);
        AvrcpHandleConnectCnf(chnl_num, BT_AVRCP_RESULT_ERROR, &device_addr, 0, 0, 0, 0, 0);
    }
...
static void AvrcpBrowseUtilCallbackDisconnect(AvrcpChannel *chnl, AvrcpCallbackParms *Parms)
{
	...
	if( cntx_ptr->rcp_channel.bwsDiscFlag ){
    	AvrcpHandleBrowseDisconnectCnf(chnl_num, BT_AVRCP_RESULT_SUCCESS);
	}else{
		AvrcpHandleBrowseDisconnectInd(chnl_num);
	}
...
#ifdef __PRJ_MT7650_SONY_BDP__
                if ( PTS_AVRCP_RCR_BV_03_C )
                {
                    PTS_AVRCP_RCR_BV_03_C = 0;
                    send_data[4] = AVRCP_SPECIFIC_OPCODE_ABORT_CONTINUING;
                }
#endif
				AVRCP_UitlCmdFrameReq(chnl_info->chnl_num, AVRCP_CTYPE_CONTROL, 0x07, 0x48, sizeof(send_data), send_data);
            }
		...
		case  AVRCP_SPECIFIC_OPCODE_REGISTER_NOFITICATION:
		    {
				...
				if( parm_len >= 1 && ctype == AVCTP_RESPONSE_REJECTED ){
					Report(("[BT] [AVRCP] AVRCP_SPECIFIC_OPCODE_REGISTER_NOFITICATION, AVCTP_RESPONSE_REJECTED"));
					pcnf->result = BT_AVRCP_RESULT_ERROR;
					pcnf->error_code = parms[0];
				}else if( parm_len >= 1 ){
					Report(("[BT] [AVRCP] AVRCP_SPECIFIC_OPCODE_REGISTER_NOFITICATION, BT_AVRCP_RESULT_SUCCESS"));
        		    pcnf->result = BT_AVRCP_RESULT_SUCCESS;
        		    AvrcpHandleCBAVRCPRsp_RegisterNoti(chnl_info, result, pcnf, parm_len, parms);
		        }else{
		        	Report(("[BT] [AVRCP] AVRCP_SPECIFIC_OPCODE_REGISTER_NOFITICATION, BT_AVRCP_ERRCODE_INVALID_PARAM"));
		            pcnf->result = BT_AVRCP_RESULT_ERROR;
...
void AvrcpHandleCBAVCResponse(AvrcpChnlInfo *chnl_info, AvctpRspFrame *avctpRsp, U16 operandLen, U8 *operands){
    ...
    switch(avctpRsp->opcode){
        ...
        case AVRCP_OPCODE_PASS_THROUGH: // pass through data, pass to old frameCmdId
			{   
#if 0
                bt_ext_log("[AVRCP] AvrcpHandleCBAVCResponse 111:\n");
                avrcp_adp_context *cntx_ptr = NULL;
                cntx_ptr = &avrcp_cntx[chnl_info->chnl_num];		
                //AddAvrcpCallList(c_type, subunit_type,subunit_id, data_len, frame_data);
                bt_ext_log("[AVRCP] AvrcpHandleCBAVCResponse 111:chnl_info->chnl_num=%d\n",chnl_info->chnl_num);
			    if (avrcpCallList.emptyflag != 1)
			    {
			    	bt_ext_log("[AVRCP] AvrcpHandleCBAVCResponse 222:\n");
			    	RemoveAvrcpCallList();
			    }
			    if (avrcpCallList.emptyflag != 1)
			    {
                    bt_ext_log("[AVRCP] AvrcpHandleCBAVCResponse 333:\n");
					GetAvrcpCallData(&cntx_ptr);
					AVRCP_SendCommand(&cntx_ptr->rcp_channel, &cntx_ptr->cmdFrame);
			    }
                bt_ext_log("[AVRCP] AvrcpHandleCBAVCResponse 444:\n");
#endif
				AvrcpHandleCommandFrameCnf(
					chnl_info->chnl_num,
					BT_AVRCP_RESULT_SUCCESS,
