...
#elif defined(BTMTK_ON_LINUX)
	btmtk_wstrcpy((WCHAR*)bpp_private_path, (WCHAR*)BPP_BTMTK_PARENT_PATH);
#else
...
//mtk03036 20100406 end	
	...
	btmtk_wstrcat((WCHAR*)bpp_private_path, (WCHAR*)BPP_BTMTK_ROOT_PATH);
	...
	if ((fh = bpp_fs_open(bpp_private_path, BTMTK_FS_READ_ONLY)) < 0)
	{
		...
		/* create "\\profile" path */	
		btmtk_wstrcat((WCHAR*)bpp_private_path, (WCHAR*)BPP_BTMTK_PROFILE_PATH);
		ret = bpp_fs_createdir(bpp_private_path);
	}
	else
	{
                    ...
                    U32 line)
{
    ...
    if (pmem_node)
    {
        ...
        strncpy((S8*) pmem_node->file, pfilename, BPP_DEBUG_MAX_FILE_NAME);
...
static kal_bool bpp_debug_x_add_fh_node(bpp_debug_x_info_struct *pbpp_debug_x_info, BTMTK_FS_HANDLE fh, S8 *pfile, U32 line)
{
    ...
    if (pfh_node && fh > 0)
    {
        ...
        strncpy(pfh_node->file, pfilename, BPP_DEBUG_MAX_FILE_NAME);
