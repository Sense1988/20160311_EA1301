...
extern void A2dpHandleStreamOpenCnf(U16 result, 
                                                ...
                                                U8 *dev_name, U8 name_len, bt_a2dp_audio_cap_struct *config);
...
extern void A2dpHandleStreamDataInd(U8 stream_handle,GavdpCallbackParms *info);
extern void A2dpHandleStreamSetConfigCnf(U16 result, U8 stream_handle);
#ifdef __PRJ_MT7650_SONY_BDP__
extern void A2dpHandleSecondStreamInd(bt_addr_struct *first_device_addr, bt_addr_struct *second_device_addr);
extern void A2dpHandleAvdtpConnectedInd(bt_addr_struct *device_addr);
extern void A2dpHandleAvdtpDisconnectedInd(bt_addr_struct *device_addr);
#endif
...
static void A2dpSdpQueryCallback(SdpQueryToken *sqt, U8 result, U8 attr_idx, U8 *attr_val)
{
    ...
    if (bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_cnf == BT_A2DP_WAIT_CNF_SDP_QUERY)
    {
        ...
        case BT_STATUS_NOSERVICES:
            ...
            if(sqt->uuid == SC_AUDIO_DISTRIBUTION)
            {
                ...
                {
                   //bt_a2dp_ctx.sqt.a2dp_sdp_query_count =0;
                   A2dpSetSdpQueryCount(0);
                   goto exit;
                }
            }
 ...
 exit:        
                ...
                A2dpHandleStreamOpenCnf(BT_A2DP_RESULT_TIMEOUT, stream_handler, &device_addr, NULL, 0, NULL);
            }
            break;
        }
    }
...
static BtStatus A2dpSdpQuery(U8 stream_id)
{
...
#ifdef __PRJ_MT7650_SONY_BDP__  // work around for Xperia P (Android 4.0.4) IOT issue.
    usleep(50000);
#endif
    ...
    search_pattern.uuid = SC_AUDIO_SOURCE;
...
StreamEndPoint * A2dpGetStreamEndpoint(void)
{
    ...
    for (i = 0; i < MAX_A2DP_SEP_NUM; i++)
    {
        ...
        if ( (0 == bt_a2dp_ctx.local_sep_List[i].info.inUse) && 
             (bt_a2dp_ctx.default_local_role == bt_a2dp_ctx.local_sep_List[i].info.streamType) )
#else
        if (0 == bt_a2dp_ctx.local_sep_List[i].info.inUse)
#endif
        {
...
static void A2dpContinueOpenReq(bt_addr_struct *device_addr)
{
    ...
    if ( stream_handler < MAX_A2DP_DEV_NUM)
    {
        if (bt_a2dp_ctx.a2dp_device_list[stream_handler].local_sep->stream_state >= GAVDP_STATE_OPEN)
        {
            ...
            A2dpHandleStreamOpenCnf(BT_A2DP_RESULT_OK, stream_handler, device_addr, NULL, 0, NULL);
            return;
        }
        A2dpReleaseStreamHandler(stream_handler);
        A2dpHandleStreamOpenCnf(BT_A2DP_RESULT_FATAL_ERROR, stream_handler, device_addr, NULL, 0, NULL);
        return;
    }
...
void A2DP_StreamOpenReq(bt_addr_struct *device_addr)
{
    ...
    {
        A2dpHandleStreamOpenCnf( BT_A2DP_RESULT_PANIC, UNKNOWN_STREAM_HANDLE, device_addr, NULL, 0, NULL);
        return;
    }
...
static void A2dpStreamOpenCnf(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    ...
    else
    {
        ...
        switch (info->error)
        {
            case GAVDP_ERR_BAD_SEP:
                result = BT_A2DP_RESULT_BAD_SEP;
                break;
            case GAVDP_ERR_BAD_STATE:
                result = BT_A2DP_RESULT_BAD_STATE;
                break;
            case GAVDP_ERR_INVALID_CAPABILITY:
                result = BT_A2DP_RESULT_INVALID_CAPABILITY;
                break;
            case GAVDP_ERR_NO_RESOURCE :
                result = BT_A2DP_RESULT_NO_RESOURCE;
                break;
            case GAVDP_ERR_PANIC :
                result = BT_A2DP_RESULT_PANIC;
                break;
            case GAVDP_ERR_FATAL :
                result = BT_A2DP_RESULT_FATAL_ERROR;
                break;
            case GAVDP_ERR_SAME_DEV :
                result = BT_A2DP_RESULT_SAME_DEV;
                break;
            case GAVDP_ERR_CONN_IN :
                result = BT_A2DP_RESULT_CONN_IN;
                break;
            default:
                result = BT_A2DP_RESULT_TIMEOUT;
                break;
    ...
    {	
        ...
        if(deviceName != 0)
            A2dpHandleStreamOpenCnf(result, stream_handler, &device_addr, (U8 *)deviceName->dev_name, deviceName->dev_name_len, &current_config);
        else
            A2dpHandleStreamOpenCnf(result, stream_handler, &device_addr, NULL, 0, &current_config);
    }
    else
    {
        // TODO: error handling, ignore it ?
        A2dpHandleStreamOpenCnf(BT_A2DP_RESULT_FATAL_ERROR, stream_handler, &device_addr, NULL, 0, NULL);
    }
}
...
static U8 A2dpStreamOpenInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    ...
    if (stream_handler == UNKNOWN_STREAM_HANDLE)
    {
            ...
            bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_cnf <= BT_A2DP_WAIT_CNF_SDP_QUERY)
        {
            ...
            if(deviceName != 0)
                A2dpHandleStreamOpenCnf(BT_A2DP_RESULT_OK, stream_handler, &device_addr, (U8 *)deviceName->dev_name, deviceName->dev_name_len, &current_config);
            else
                A2dpHandleStreamOpenCnf(BT_A2DP_RESULT_OK, stream_handler, &device_addr, NULL, 0, &current_config);
        ...
        (status == BT_STATUS_PENDING || status == BT_STATUS_SUCCESS))
    {
        ...
        if(deviceName != 0)
            A2dpHandleStreamOpenInd(stream_handler, &device_addr, (U8 *)deviceName->dev_name, deviceName->dev_name_len, &current_config);
        else
            A2dpHandleStreamOpenInd(stream_handler, &device_addr, NULL, 0, &current_config);
    }
    else
    {
...
void A2DP_StreamOpenRes(U16 result, U8 stream_handle)
{
    ...
    ASSERT(stream_handle < MAX_A2DP_DEV_NUM);
...
//    BtStatus status;
    ...
    if ( FALSE == A2DP_NgeoConfig(&tmp_sep->local_cfg_codec, &tmp_sep->remote_cap_info[0]) )
    {
...
static void A2dpStreamReconfigInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
        ...
        bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_cnf == BT_A2DP_WAIT_CNF_NONE)
    {
        ...
        A2dpHandleStreamReconfigInd(stream_handler, (A2dpCodec *)&sep->remote_cfg_codec);
    }
    else
    {
...
void A2DP_StreamStartReq(U8 stream_handle)
{
    ...
    if (bt_a2dp_ctx.a2dp_device_list[stream_handle].wait_res == BT_A2DP_WAIT_RES_START)
    {
        ...
        // We want to send "stream start". Remote device also send "stream start" at the same time.
        // Skip our action, don't send.
        return;
#endif
...
static void A2dpStreamStartInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    ...
    if ( stream_handler < MAX_A2DP_DEV_NUM )
    {
        ...
        if ( (bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_res == BT_A2DP_WAIT_RES_NONE && 
               bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_cnf == BT_A2DP_WAIT_CNF_NONE) || 
             (bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_res == BT_A2DP_WAIT_RES_NONE && 
               bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_cnf == BT_A2DP_WAIT_CNF_START) )
#else
        if (bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_res == BT_A2DP_WAIT_RES_NONE && 
            bt_a2dp_ctx.a2dp_device_list[stream_handler].wait_cnf == BT_A2DP_WAIT_CNF_NONE)
#endif
...
static void A2dpStreamCloseCnf(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    ...
    if (wait_cnf == BT_A2DP_WAIT_CNF_CLOSE)
    {
        A2dpHandleStreamCloseCnf( result, stream_handler);
    }
    else if (wait_res == BT_A2DP_WAIT_RES_CLOSE)
    {
        A2dpCmgrLinkClose(stream_handler);
        return;
    }
...
static void A2dpStreamSentDataCallback(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ( info && info->p.data )    // Klockwork warning.
...
void A2DP_SendStreamData(U8 stream_handle, U8 *data, U32 len, U32 sample_count)
{
    ...
    for (i = 0; i < MAX_A2DP_DEV_NUM; i++)
    {
        ...
        {
#if defined(__PRJ_MT7650_SONY_BDP__) && defined(__CONTENT_PROTECTION_SUPPORT__)
            {
                StreamEndPoint *sep;
                U8 stream_handle = i;
                ...
                {
                    if ( A2DP_GetStreamBuffer(i, len+1, &buffer) != BT_A2DP_RESULT_OK )
                        return;
                ...
                {
                    if ( A2DP_GetStreamBuffer(i, len, &buffer) != BT_A2DP_RESULT_OK )
                        return;
 ...
 *  void
 *****************************************************************************/
static void A2dpStreamDataInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    A2dpHandleStreamDataInd(sep->stream.locStrmId, info);
}

static void A2dpStreamSetConfigCnf(StreamEndPoint *sep, GavdpCallbackParms *info)
{
	U8 stream_handler = UNKNOWN_STREAM_HANDLE;
	U16 result = BT_A2DP_RESULT_BAD_STATE;
	stream_handler = A2dpFindStreamIDbySep(sep);
	OS_Report("[A2DP] A2dpStreamSetConfigCnf, stream_handler:%d", stream_handler);
	 if (stream_handler < MAX_A2DP_DEV_NUM)
    {
        if (info->error == GAVDP_ERR_NO_ERR){
        	result = BT_A2DP_RESULT_OK;
		...
		A2dpHandleStreamSetConfigCnf(result, stream_handler);
    }
}

#ifdef __PRJ_MT7650_SONY_BDP__
static void A2dpSecondStreamInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    bt_addr_struct first_device_addr;
    bt_addr_struct second_device_addr;

    bt_a2dp_get_bt_device_addr(&(sep->second_conn_bd_sddr), &second_device_addr);
    bt_a2dp_get_bt_device_addr(&(sep->remote_bd_addr), &first_device_addr);
    A2dpHandleSecondStreamInd(&first_device_addr, &second_device_addr);
}

static void A2dpAvdtpConnectedInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    bt_addr_struct device_addr;
    
    bt_a2dp_get_bt_device_addr(&(sep->remote_bd_addr), &device_addr);
    A2dpHandleAvdtpConnectedInd(&device_addr);
}

static void A2dpAvdtpDisconnectedInd(StreamEndPoint *sep, GavdpCallbackParms *info)
{
    bt_addr_struct device_addr;
    
    bt_a2dp_get_bt_device_addr(&(sep->remote_bd_addr), &device_addr);
    A2dpHandleAvdtpDisconnectedInd(&device_addr);
}
    ...
    switch (event)
    {
        ...
        case GAVDP_EVENT_STREAM_ESTABLISH_IND:
            ...
            switch (device_type)
            {
                ...
                default:
                    if(info->error == GAVDP_ERR_NO_ERR) {
                        stream_handle = A2dpFindStreamIDbySep(sep);
                        if ( stream_handle < MAX_A2DP_DEV_NUM )
                        {
		...
		case GAVDP_EVENT_CHANGE_PARAMETERS_CHANGED:
			{	
				bt_a2dp_audio_cap_struct current_config;
				bt_adp_a2dp_unpack_audio_cap((A2dpCodec *) &sep->current_cfg_codec, &current_config);
				A2dpHandleConfigChange(&current_config);
			}
			break;
		case GAVDP_EVENT_STREAM_SET_CONFIG_CNF:
			{
				A2dpStreamSetConfigCnf(sep, info);
			}
			break;
#ifdef __PRJ_MT7650_SONY_BDP__
        case GAVDP_EVENT_SECOND_CONNECTION_IND:
            A2dpSecondStreamInd(sep, info);
            break;
        case GDVDP_EVENT_AVDTP_CONNECTED_IND:
            A2dpAvdtpConnectedInd(sep, info);
            break;
        case GDVDP_EVENT_AVDTP_DISCONNETED_IND:
            A2dpAvdtpDisconnectedInd(sep, info);
            break;
#endif
        default:
            break;
...
//    BtStatus status;
...
#ifdef __PRJ_MT7650_SONY_BDP__
    bt_a2dp_ctx.default_local_role = local_role;
    bt_a2dp_ctx.sdp_record_snk.num = SDPDB_GetLocalServiceAttribute( SC_AUDIO_SINK, &bt_a2dp_ctx.sdp_record_snk.attribs, &local_service_id );
    SDP_AddRecord(&bt_a2dp_ctx.sdp_record_snk);
    bt_a2dp_ctx.sdp_record_src.num = SDPDB_GetLocalServiceAttribute( SC_AUDIO_SOURCE, &bt_a2dp_ctx.sdp_record_src.attribs, &local_service_id );
    SDP_AddRecord(&bt_a2dp_ctx.sdp_record_src);
    ...
    for (i = 0; ((i < sep_num) && (i < MAX_A2DP_SEP_NUM)); i++)
    {
        ...
        //content protection
        bt_a2dp_ctx.local_sep_List[i].content_protection[0].data = get_ctrl_buffer(10);
        bt_a2dp_ctx.local_sep_List[i].content_protection[1].data = get_ctrl_buffer(10);		
        bt_a2dp_ctx.local_sep_List[i].current_cfg_cp.data= get_ctrl_buffer(10);
        for ( j = 0 ; j < MAX_NUM_REMOTE_CP ; j++ )
            bt_a2dp_ctx.local_sep_List[i].remote_cp_info[j].data = get_ctrl_buffer(10);
        ...
        bt_a2dp_ctx.local_sep_List[i].content_protection[0].cpType = AVDTP_CP_TYPE_SCMS_T;
        bt_a2dp_ctx.local_sep_List[i].content_protection[0].dataLen = 0;        
#endif		
...
#ifdef __PRJ_MT7650_SONY_BDP__
        if (BT_A2DP_SOURCE == local_sep_list[i].sep_type)
#else
        if (BT_A2DP_SOURCE == local_role)
#endif
...
#ifdef __PRJ_MT7650_SONY_BDP__
        GAVDP_Init_RegisterGavdpCallback(A2dpGavdpEventCallback);
#endif
...
//    BtStatus status;
    ...
    for (i = 0; i < MAX_A2DP_SEP_NUM; i++)
    {
        if ( BT_STATUS_SUCCESS == GAVDP_DeRegister(&bt_a2dp_ctx.local_sep_List[i]))
        {
            ...
            //content protection
            if (bt_a2dp_ctx.local_sep_List[i].content_protection[0].data)
                free_ctrl_buffer(bt_a2dp_ctx.local_sep_List[i].content_protection[0].data);
            if (bt_a2dp_ctx.local_sep_List[i].content_protection[1].data)
                free_ctrl_buffer(bt_a2dp_ctx.local_sep_List[i].content_protection[1].data);		

            if (bt_a2dp_ctx.local_sep_List[i].current_cfg_cp.data)
                free_ctrl_buffer(bt_a2dp_ctx.local_sep_List[i].current_cfg_cp.data);
            for ( j = 0 ; j < MAX_NUM_REMOTE_CP ; j++ )
                if (bt_a2dp_ctx.local_sep_List[i].remote_cp_info[j].data)
                    free_ctrl_buffer(bt_a2dp_ctx.local_sep_List[i].remote_cp_info[j].data);
    ...
    /* remove SDP record */
#ifdef __PRJ_MT7650_SONY_BDP__
    SDP_RemoveRecord(&bt_a2dp_ctx.sdp_record_src);
    SDP_RemoveRecord(&bt_a2dp_ctx.sdp_record_snk);
#else
...
#ifdef __PRJ_MT7650_SONY_BDP__
void A2DP_AAC_Enable(U8 aac_enable)
{
    GAVDP_SetAacEnable(aac_enable);
}

void A2DP_SetLocalRole(U8 local_role)
{
    bt_a2dp_ctx.default_local_role = local_role;
}
#endif
...
void A2DP_StreamChannelOpenReq(U8 stream_handle)
{
	StreamEndPoint *sep;
	OS_Report("[INFO] A2DP_StreamChannelOpenReq, stream_handler:%d", stream_handle);
	ASSERT(stream_handle < MAX_A2DP_DEV_NUM);
    ASSERT(NULL != bt_a2dp_ctx.a2dp_device_list[stream_handle].local_sep);
	sep = bt_a2dp_ctx.a2dp_device_list[stream_handle].local_sep;
