    ...
    U8 *dev_name, U8 name_len, bt_a2dp_audio_cap_struct *config)
{
	...
	if (config)
        memcpy(&parms.p.A2DPEventOpenStreamCnf.config, config, sizeof(bt_a2dp_audio_cap_struct));
	else
		memset((U8 *)&parms.p.A2DPEventOpenStreamCnf.config, 0, sizeof(bt_a2dp_audio_cap_struct));
    ...
    U8 *dev_name, U8 name_len, bt_a2dp_audio_cap_struct *config)
{
	...
	if (config)
		memcpy(&parms.p.A2DPEventOpenStreamInd.config, config, sizeof(bt_a2dp_audio_cap_struct));
	else
		memset((U8 *)&parms.p.A2DPEventOpenStreamInd.config, 0, sizeof(bt_a2dp_audio_cap_struct));
 ...
 *  void
 *****************************************************************************/
void A2dpHandleStreamDataInd(U8 stream_handle, GavdpCallbackParms *info)
{
    BTA2DPCallbackParms parms;
    //Report(("[INFO] Remote device send media data to local  in A2dpHandleStreamDataInd "));
#ifdef __PRINT_A2DP_DATA_LOG__ 
    Report(("[INFO] Remote device send media data to local  in A2dpHandleStreamDataInd info->p.data=0x%x,info->len=%d",*(info->p.data),info->len));
#endif    
    parms.event = BTA2DP_EVENT_STREAM_DATA_IND;
    parms.p.A2DPEventDataStreamInd.stream_handle = stream_handle;
    parms.p.A2DPEventDataStreamInd.buf = info->p.data;
    parms.p.A2DPEventDataStreamInd.buf_size = info->len;
    bt_a2dp_ctx.a2dpCBfunc(&parms);
    return;
}

void A2dpHandleStreamSetConfigCnf(U16 result, U8 stream_handle)
{
	BTA2DPCallbackParms parms;
	Report(("[INFO] A2dpHandleStreamSetConfigCnf, stream_handle:%d", stream_handle));
	parms.event = BTA2DP_EVENT_SET_CONFIGURATION_CNF;
	parms.p.A2DPEventSetConfigurationCnf.stream_handle = stream_handle;
	parms.result = result;
	bt_a2dp_ctx.a2dpCBfunc(&parms);
...
void A2dpHandleSecondStreamInd(bt_addr_struct *first_device_addr, bt_addr_struct *second_device_addr)
{
    BTA2DPCallbackParms parms;

    parms.event = BTA2DP_EVENT_SECOND_STREAM_IND;
    parms.p.A2DPEventSecondStreamInd.first_device_addr = first_device_addr;
    parms.p.A2DPEventSecondStreamInd.second_device_addr = second_device_addr;
    bt_a2dp_ctx.a2dpCBfunc(&parms);
}

void A2dpHandleAvdtpConnectedInd(bt_addr_struct *device_addr)
{
    BTA2DPCallbackParms parms;

    parms.event = BTA2DP_EVENT_AVDTP_CONNECTED_IND;
    parms.p.A2DPEventAvdtpConnectedInd.device_addr = device_addr;
    bt_a2dp_ctx.a2dpCBfunc(&parms);
}

void A2dpHandleAvdtpDisconnectedInd(bt_addr_struct *device_addr)
{
    BTA2DPCallbackParms parms;

    parms.event = BTA2DP_EVENT_AVDTP_DISCONNECTED_IND;
    parms.p.A2DPEventAvdtpDisconnectedInd.device_addr = device_addr;
    bt_a2dp_ctx.a2dpCBfunc(&parms);
}
