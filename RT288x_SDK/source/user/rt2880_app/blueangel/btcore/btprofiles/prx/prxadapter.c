...
extern BOOL OS_MemCmp(const U8 *buffer1, U16 len1, const U8 *buffer2, U16 len2);
...
static void prx_tps_serivce_Timeout(void *para);
...
PrxMTransport *g_prx_used_monitorarray[PRX_MAX_REPORTER_NUM];
PrxMTransport *g_prx_latest_monitor = NULL;
...
void prxmClientCmgrConnectionCallback(CmgrHandler *Handler, CmgrEvent Event,
	...
	case CMEVENT_DATA_LINK_DIS:
		...
		if( g_prx_latest_monitor == pChnl ){
			g_prx_latest_monitor = NULL; // has been removed
		}
...
#if 0
BOOL release_monitor(PrxMTransport *monitor){
	return FALSE;
}
 ...
 *
 */
BOOL assign_monitor_by_remdev(PrxMTransport *monitor, BtRemoteDevice *dev){
	U8 i;
...
void prxGattAPCallBack(void *userdata, BtGattEvent *event)
{
    ...
    switch(event->eType)
    {
		...
		case BT_GATT_OPERATOR_WRITE_WITHOUT_RESPONSE:
			...
			break;
		default:
			Report(("[PRX:][GATT][WRN] unknow eType:%d", event->eType));
			break;
    }
}
...
void ProximityCommonGattCallback(void *handler, BtGattEvent *event)
{
	...
	switch (event->eType)
	{
		case BT_GATT_APP_EVENT_CONNECTED:
			...
			}else if( g_prx_latest_monitor == NULL ){
			    Report(("[PRX:][WRN] no monitor ignore it!"));			
			}else{
	    	    pCntx = (PrxMContext *) ContainingRecord( g_prx_latest_monitor, PrxMContext, chnl);
    			prxm_notifyConnectResult(pCntx, TRUE, 0);
			}
#endif			
			break;
...
U8 ProximityImmAlertLevel(U8 type, BtRemoteDevice *link, U16 *len, U8 **raw)
{
    U8 i=0;
...
U8 BtPrxLocalTxPowerDatabase(U8 type, BtRemoteDevice *link, U16 *len, U8 **raw)
{
...
U8 ProximityLinkLossAlert(U8 type, BtRemoteDevice *link, U16 *len, U8 **raw)
{
    U8 i=0;
    U8 status = BT_STATUS_FAILED;
#if defined(__BT_PRXR_PROFILE__)
