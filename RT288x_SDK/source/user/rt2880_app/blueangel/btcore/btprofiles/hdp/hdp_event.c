...
void hdp_event_mdl_opend_cnf(McapCallbackParms  *parms)
{
	...
	if (HDP_MDEP_ID_ECHO_TEST == parms->mdl->mdepId)
	{
		...
		{
			hdp_set_channel_state(channel, HDP_CHANNEL_STATE_CONNECTED);
			hdp_send_data(channel->mcapmdl->l2ChannelId, HDP_ECHO_TEST_PACKET, HDP_ECHO_TEST_PACKET_LEN);
...
void hdp_event_mdl_config_ind(McapCallbackParms  *parms)
{
	...
	//check config
	localtype = hdp_get_local_config(bdAddr, instance->channelType);		
	...
	channel = hdp_util_find_channel_by_mdl_id(bdAddr, parms->mdl->mdlId);
	if (NULL == channel)
	{
...
void hdp_event_mdl_config_cnf(McapCallbackParms  *parms)
{
	...
	if (BT_STATUS_SUCCESS == status)
	{
		...
		localtype = hdp_get_local_config(bdAddr, channel->type);
...
void hdp_event_packet_handle_ind(McapCallbackParms  *parms)
{
	...
	if (HDP_MDEP_ID_ECHO_TEST == parms->mdl->mdepId)
	{
		...
		else
		{
			...
			hdp_send_data(parms->mdl->l2ChannelId ,parms->rxPkt->data, parms->rxPkt->dataLen);
		}
