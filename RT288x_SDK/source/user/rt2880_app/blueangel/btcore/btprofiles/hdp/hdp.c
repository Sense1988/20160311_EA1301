...
BtStatus hdp_mcl_connect(HdpChannel *channel)
{
	...
	if ((NULL == mcl->mcapMcl) ||
			(mcl->mcapMcl->mclState != MCAP_MCL_STATE_CONNECTED))
	{
...
//	instance = hdp_find_instance(id);		
	...
	mode = hdp_util_convert_type2mode(type);
...
{	
	...
	while (BT_STATUS_PENDING != ret) {		
		...
		channel = hdp_find_channel_by_l2capId(txPkt->channelId);
		if (NULL == channel)
		{
...
{
	BtStatus ret;
	HdpInstance * instance = hdp_find_available_instance();
	MdepId id = hdp_assign_mdep_id();
		
	OS_Report("[HDP]hdp_register");
	if (NULL == config || NULL == instance || id <= 0) 
	{
		OS_Report("[HDP]Err, config is null? %d, available instance is null? %d, id is %d",
					(NULL == config), (NULL == instance), id);
...
BtStatus hdp_deregister(MdepId id) 
{
	...
	else
	{
		...
		//Notes: if disconnect cnf has not been received but MMI request to deregister instance, MAYBE no channece to clear channel.
...
BtStatus hdp_open_channel(MdepId id, BD_ADDR *bdaddr, HdpChannelType type)
{
	...
	OS_Report("[HDP]hdp_open_channel:id %d, addr 0x%x:0x%x:0x%x:0x%x:0x%x:0x%x, type is %d", 
				id, bdaddr->addr[0],bdaddr->addr[1],bdaddr->addr[2],bdaddr->addr[3],bdaddr->addr[4],bdaddr->addr[5],type);

/*	localtype = hdp_get_local_config(bdaddr, type);
	if (localtype != type)
	{
		OS_Report("[HDP]Err, channel type mismatch: localtype is %d", type);
		ret = BT_STATUS_INVALID_PARM;
		goto exit;
	}*/

	HdpInstance *instance = hdp_find_instance(id);	
	channel = hdp_create_channel(instance, bdaddr, type);
...
/*
	...
	mclHeader = GetHeadList(&instance->mdl);
	nextMdl = mclHeader;
	do {
		nextMcl = (McapMdl *)GetNextNode(&nextMcl->node);

		mdlHeader = GetHeadList(&nextMcl->mcapMdlList);
		nextMdl = mdlHeader;
		do {
			nextMdl = (McapMdl *)GetNextNode(&mdlHeader->node);
			MCAP_CloseMdl(nextMcl, nextMdl);	
			nextMdl->currentOp = HDP_TASK_DISCONNECT_MDL;
		} while (nextMdl != mdlHeader);
	} while (nextMcl != mclHeader);
...
BtStatus hdp_send_data(U16 l2capId, void* data, U32 len)
{
	...
	HdpChannel* channel = NULL;
	U16 offset = 0;
	OS_Report("[HDP]hdp_send_data:channel id %d, data len %d", l2capId, len);
	...
	channel = hdp_find_channel_by_l2capId(l2capId);
	...
	while(offset < len)
	{
		...
		dataPkt = hdp_data_packat_new(l2capId, data+offset, sublen);
		if (NULL == data)
		{
