...
kal_int16 jsr82_session_PutBytes(bt_jsr82_put_bytes_req_struct *ptr)
{
    ...
    kal_int16 freespace_dataLen_in_TXBuf = 0;
     ...
     if (BT_JSR82_SESSION_IDLE == BTJ82SEAP(session)[ptr->index].status)
     {
	  ...
	  parms.p.put_bytes_cnf.index = ptr->index;
	  parms.p.put_bytes_cnf.l2cap_id = ptr->l2cap_id;
	  parms.p.put_bytes_cnf.ps_type = ptr->ps_type;
	  parms.p.put_bytes_cnf.length = 0;
	  parms.event = JSR82_EVENT_PUT_BYTES_CNF;
	  parms.dstMod = MOD_BT;
	  JSR82CB(&parms);
     	  return 0;
     }
    ...
    writeLength = RING_WriteData(&(BTJ82SEAP(session)[ptr->index].WriteRingBuf), (char*)BTJ82SEAP(session)[ptr->index].deliverPutBufPtr, ptr->length);
#else
   ...
   if (writeLength < ptr->length)
    ...
    parms.p.put_bytes_cnf.l2cap_id = ptr->l2cap_id;
    parms.p.put_bytes_cnf.ps_type = ptr->ps_type;
    parms.p.put_bytes_cnf.length = writeLength;
    parms.event = JSR82_EVENT_PUT_BYTES_CNF;
    parms.dstMod = MOD_BT;
    JSR82CB(&parms);
...
kal_int16 jsr82_session_GetBytes(bt_jsr82_get_bytes_req_struct *ptr)
{
    ...
    parms.p.put_bytes_cnf.l2cap_id = ptr->l2cap_id;
    parms.p.put_bytes_cnf.ps_type = ptr->ps_type;
    parms.p.put_bytes_cnf.length = DataRead;
    parms.event = JSR82_EVENT_GET_BYTES_CNF;
    parms.dstMod = MOD_BT;
    JSR82CB(&parms);
            ...
            kal_uint8 buf_type)
{
    ...
    parms.p.put_bytes_cnf.index = session_inx;
    parms.p.put_bytes_cnf.l2cap_id = subsession_id;
    parms.p.put_bytes_cnf.ps_type = session_type;
    parms.event = JSR82_EVENT_GET_AVAILABLE_DATA_LENGTH_CNF;
    parms.dstMod = MOD_BT;
    
    if (buf_type == JBT_SESSION_RX_BUF_TYPE)
    {
        /* RX Buffer */
	 length = RING_DataLen(&(BTJ82SEAP(session)[session_inx].ReadRingBuf));
	 parms.p.put_bytes_cnf.length = length;
	 JSR82CB(&parms);
        return length;
    }
    else if (buf_type == JBT_SESSION_TX_BUF_TYPE)
    {
        /* TX Buffer */
	 length = RING_DataLen(&(BTJ82SEAP(session)[session_inx].WriteRingBuf));
	 parms.p.put_bytes_cnf.length = length;
	 JSR82CB(&parms);
        return length;
    }
