 ...
 * [ALPS00238652] [ICS][CTS Verifier][Lenovo75][CTS4.0.3_r1] Bluetooth insecure server and client - Fail (A69 V.S. Galaxy nexus)
 * .
 *
...
void BTJSR82_SessionApInit(void)
{
    ...
    {
        BTJ82SEAP(session)[i].con_id = i;
        InitializeListHead(&BTJ82SEAP(session)[i].ApTxQueue);
        InsertTailList(&BTJ82SEAP(session)[i].ApTxQueue, &BTJ82SEAP(session)[i].txPacket.node);
    }
...
void BTJSR82_DeInitAll(void)
{
    ...
    bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_APPSDPDBDEACTIVATE);
#ifdef __JSR82_MAUI__
...
void bt_jsr82_HandleJSR82Deactivate(void)
{
    ...
    /* De-register jsr82globalHandler first from GlobalHandler:2007-1215 */
    ME_UnregisterGlobalHandler(&jsr82globalHandler);
    btmtk_os_memset((U8*) & bt_jsr82_session_ap_context, 0, sizeof(BT_JSR82_SESSION_AP_CONTEXT_struct_t));

    /* The following are codes of BTJSR82_SessionApInit() */
    for (i = 0; i < JSR82_MAX_SESSION_NO; i++)
    {
        BTJ82SEAP(session)[i].con_id = i;
        InitializeListHead(&BTJ82SEAP(session)[i].ApTxQueue);
    ...
    BTJ82SEAP(rfcomm_channel_usage_table) = 0;
    BTJ82SEAP(jsr82_disconnect_all_acl).func = 0;
    EVM_ResetTimer(&(BTJ82SEAP(jsr82_con_req_timer)));
    EVM_ResetTimer(&(BTJ82SEAP(jsr82_sdp_req_timer)));
    jsr82LinkHandler.callback = jsr82_DummyLinkCallback;
    BTJ82SEAP(jsr82_disconnect_all_acl).func = 0;
    EVM_ResetTimer(&(BTJ82SEAP(jsr82_con_req_timer)));
    EVM_ResetTimer(&(BTJ82SEAP(jsr82_sdp_req_timer)));
...
void bt_jsr82_HandleSessionApDisconnectAllAcl(void)
{
    ...
    if (BTJ82SEAP(jsr82_con_req_timer).func != 0)
    {
        ...
        bt_jsr82_SessionApConnectCfn(
            ...
            ptr->mtu,
            ptr->channel,
            0,  /* resulut is failed, index=0 and it is meaningless */
            ptr->identify,
            0,
            JSR82_SESSION_CONNECT_CLIENT_FAILED);
#endif
...
void BTJSR82_RfCallbackFunc(RfChannel *chan, RfCallbackParms *parms)
{
    ...
    /* Only RFCOMM Client side, it will set Channel->remDev when RF_OpenClientChannel is invoked :2007-1125 */
    /* parms->ptrs.remDev: It is only valid when receiving the events: RFEVENT_OPEN_IND, RFEVENT_OPEN, RFEVENT_CLOSED_IND and RFEVENT_CLOSE */
    ...
    if (chan->remDev == NULL)
    {
        bt_trace(
            ...
            parms->event,
            parms->ptrs.remDev);
        if (parms->event == RFEVENT_CLOSE_IND || parms->event == RFEVENT_CLOSED || parms->event == RFEVENT_OPEN_IND ||
            parms->event == RFEVENT_OPEN)
        {
            if (parms->ptrs.remDev != NULL)
            {
                chan->remDev = parms->ptrs.remDev;
            }
            else
            {
                Assert(0);
    ...
    switch (parms->event)
    {
        ...
        case RFEVENT_CLOSED:
            if (ptr1->client_server == BT_JSR82_SESSION_CLIENT)
            {
                ...
                {
                    /*
                     * SecHandleAuthenticateComplete() with errCode!=0
                     * SecProcessAllSecurityTokens()
                     * token->id(&MEC(btEvent)) with BTEVENT_ACCESS_DENIED
                     * RfSecCallback() with BT_STATUS_RESTRICTED and RFEVENT_CLOSED 
                     * RfAppCallback(): Channel->callback()
                     * BTJSR82_RfCallbackFunc
                     */
            ...
            if (ptr1->channel_state == BT_JSR82_CHANNEL_STATE_IDLE)
            {
                if (ptr1->client_server == BT_JSR82_SESSION_CLIENT)
                {
                    ...
                    bt_jsr82_SessionApConnectCfn(
                        ...
                        JSR82_SESSION_CONNECT_CLIENT_FAILED);
                    */
		      cbparms.p.connect_cnf.bd_addr = (U8*) ptr1->bd_addr;
		      cbparms.p.connect_cnf.ps_type = ptr1->ps_type;
		      cbparms.p.connect_cnf.mtu = ptr->rfc.maxFrameSize;
		      cbparms.p.connect_cnf.in_mtu = 0;
		      cbparms.p.connect_cnf.channel = ptr1->channel;
		      cbparms.p.connect_cnf.index = 0;
		      cbparms.p.connect_cnf.identify = ptr1->identify;
		      cbparms.p.connect_cnf.l2cap_id = 0;
		      cbparms.p.connect_cnf.result = JSR82_SESSION_CONNECT_CLIENT_FAILED;
		      cbparms.event = JSR82_EVENT_CONNECT_CNF;
		      cbparms.dstMod = MOD_MMI;
		      JSR82CB(&cbparms);
            ...
            else
            {
                ...
                //bt_jsr82_SessionApCloseInd(JSR82_SESSION_PS_RFCOMM, ptr1->con_id, 0, 0);
		  cbparms.p.disconnect_ind.ps_type = JSR82_SESSION_PS_RFCOMM;
		  cbparms.p.disconnect_ind.con_id = ptr1->con_id;
		  cbparms.p.disconnect_ind.identify = 0;
		  cbparms.p.disconnect_ind.l2cap_id = 0;
		  cbparms.event = JSR82_EVENT_DISCONNECT_IND;
		  cbparms.dstMod = MOD_MMI;
		  JSR82CB(&cbparms);
		  ...
		  // MTU (of remote device) should be updated for future data transmission
                ...
                if (ptr1->client_server == BT_JSR82_SESSION_CLIENT)
                {
                    ...
                    bt_jsr82_SessionApConnectCfn(
                        ...
                        ptr->rfc.maxFrameSize,
                        ptr1->channel,
                        ptr1->con_id,   /* Add by mtk01411: 2007-0916 */
                        ptr1->identify,
                        0,
                        rc);
#endif
		      ...
		      cbparms.p.connect_cnf.ps_type = ptr1->ps_type;
		      cbparms.p.connect_cnf.mtu = ptr->rfc.maxFrameSize;
		      cbparms.p.connect_cnf.in_mtu = ptr1->mtu;
		      cbparms.p.connect_cnf.channel = ptr1->channel;
		      cbparms.p.connect_cnf.index = ptr1->con_id;
		      cbparms.p.connect_cnf.identify = ptr1->identify;
		      cbparms.p.connect_cnf.l2cap_id = 0;
		      cbparms.p.connect_cnf.result = rc;
		      cbparms.event = JSR82_EVENT_CONNECT_CNF;
		      cbparms.dstMod = MOD_MMI;
		      JSR82CB(&cbparms);
                }
                else
                {
                    ...
                    CMGR_CreateDataLink(&(ptr->cmgrHandler), &(chan->remDev->bdAddr));
                    /* Add by mtk01411:2007-0916 */
                #if 0
                    if(index == JSR82_MAX_SESSION_NO)
                    {
                        rc = JSR82_SESSION_CONNECT_CLIENT_FAILED;
                        /* index is set to zero and it is invalid due to the rc (result code) is failure */
                        index = 0;
                    }
                    else
                        rc = JSR82_SESSION_CONNECT_IND_CONNECTED; 
                #endif /* 0 */ 
                    rc = JSR82_SESSION_CONNECT_IND_CONNECTED;
#if 0
                    bt_jsr82_SessionApConnectInd(
                        parms->ptrs.remDev->bdAddr.addr,
                        JSR82_SESSION_PS_RFCOMM,
                        RF_FrameSize(chan),
                        ptr1->channel,
                        ptr1->con_id,   /* Add by mtk01411:2007-0916 */
                        ...
                        rc);
#endif
		      cbparms.p.connect_ind.bd_addr = parms->ptrs.remDev->bdAddr.addr;
		      cbparms.p.connect_ind.ps_type = JSR82_SESSION_PS_RFCOMM;
		      cbparms.p.connect_ind.mtu = RF_FrameSize(chan);
		      cbparms.p.connect_ind.in_mtu = ptr1->mtu;
		      cbparms.p.connect_ind.channel = ptr1->channel;
		      cbparms.p.connect_ind.index = ptr1->con_id;
		      cbparms.p.connect_ind.identify = ptr1->identify;
		      cbparms.p.connect_ind.l2cap_id = 0;
		      cbparms.p.connect_ind.result = rc;
		      cbparms.event = JSR82_EVENT_CONNECT_IND;
		      cbparms.dstMod = MOD_MMI;
		      JSR82CB(&cbparms);
                }
            ...
            break;
            /*
             * Note by mtk01411: 2007-0916
             * If JSR82 supports the setting port related configuration (e.g, ModemStatus,portSetting,LineStatus...etc.), then:
             * RFEVENT_PORT_NEG_IND (RPN Req: len>1), 
             * RFEVENT_PORT_NEG_CNF (RPN Cnf), 
             * RFEVENT_PORT_STATUS_IND (RPN Req: len <=1; ask port settings), 
             * RFEVENT_MODEM_STATUS_IND (MSC Req)
             */
            ...
            if (BTJ82SEAP(session)[index].client_server == BT_JSR82_SESSION_CLIENT)
            {
                    ...
                    (ptr->l2capCtx.l2cap_con_state == BT_JSR82_CHANNEL_STATE_CREATE_CHANNEL))
                {
                    ...
                    bt_jsr82_SessionApConnectCfn(
                        ...
                        JSR82_SESSION_CONNECT_CLIENT_FAILED);
#endif
		      parms.p.connect_cnf.bd_addr = (U8*) ptr1->bd_addr;
		      parms.p.connect_cnf.ps_type = ptr1->ps_type;
		      parms.p.connect_cnf.mtu = 0;
		      parms.p.connect_cnf.mtu = 0;
		      parms.p.connect_cnf.channel = ptr1->channel;
		      parms.p.connect_cnf.index = ptr1->con_id;
		      parms.p.connect_cnf.identify = ptr1->identify;
		      parms.p.connect_cnf.l2cap_id = L2ChannelId;
		      parms.p.connect_cnf.result = JSR82_SESSION_CONNECT_CLIENT_FAILED;
		      parms.event = JSR82_EVENT_CONNECT_CNF;
		      parms.dstMod = MOD_MMI;
		      JSR82CB(&parms);
                ...
                else
                {
                    ...
                    //bt_jsr82_SessionApCloseInd(JSR82_SESSION_PS_L2CAP, ptr1->con_id, 0, L2ChannelId);
		      parms.p.disconnect_ind.ps_type = JSR82_SESSION_PS_L2CAP;
		      parms.p.disconnect_ind.con_id = ptr1->con_id;
		      parms.p.disconnect_ind.identify = 0;
		      parms.p.disconnect_ind.l2cap_id = L2ChannelId;
		      parms.event = JSR82_EVENT_DISCONNECT_IND;
		      parms.dstMod = MOD_MMI;
		      JSR82CB(&parms);
                }
            ...
            if (info->status == BT_STATUS_SUCCESS)
            {
                ...
                if (channel->authorization.incoming == FALSE)
                {
                    ...
                    bt_jsr82_SessionApConnectCfn(
                        ...
                        ptr1->identify,
                        L2ChannelId,
                        JSR82_SESSION_CONNECT_CLIENT_SUCCESS);
#endif
		      parms.p.connect_cnf.bd_addr = (U8*) info->aclLink->bdAddr.addr;
		      parms.p.connect_cnf.ps_type = JSR82_SESSION_PS_L2CAP;
		      parms.p.connect_cnf.mtu = channel->outMtu;
		      parms.p.connect_cnf.in_mtu = ptr1->mtu;
		      parms.p.connect_cnf.channel = ptr1->channel;
		      parms.p.connect_cnf.index = index;
		      parms.p.connect_cnf.identify = ptr1->identify;
		      parms.p.connect_cnf.l2cap_id = L2ChannelId;
		      parms.p.connect_cnf.result = JSR82_SESSION_CONNECT_CLIENT_SUCCESS;
		      parms.event = JSR82_EVENT_CONNECT_CNF;
		      parms.dstMod = MOD_MMI;
		      JSR82CB(&parms);
                }
                else
                {
                    ...
                    bt_jsr82_SessionApConnectInd(
                        ...
                        ptr1->identify,
                        L2ChannelId,
                        JSR82_SESSION_CONNECT_IND_CONNECTED);
                    */
		      parms.p.connect_ind.bd_addr = info->aclLink->bdAddr.addr;
		      parms.p.connect_ind.ps_type = JSR82_SESSION_PS_L2CAP;
		      parms.p.connect_ind.mtu = channel->outMtu;
		      parms.p.connect_ind.in_mtu = ptr1->mtu;
		      parms.p.connect_ind.channel = ptr1->channel;
		      parms.p.connect_ind.index = ptr1->con_id;
		      parms.p.connect_ind.identify = ptr1->identify;
		      parms.p.connect_ind.l2cap_id = L2ChannelId;
		      parms.p.connect_ind.result = JSR82_SESSION_CONNECT_IND_CONNECTED;
		      parms.event = JSR82_EVENT_CONNECT_IND;
		      parms.dstMod = MOD_MMI;
		      JSR82CB(&parms);
                }
...
void bt_jsr82_CmgrClientCallback(CmgrHandler *Handler, CmgrEvent Event, BtStatus Status)
{
    ...
    if (!StrnCmp(Handler->remDev->bdAddr.addr, 6, Handler->bdc->addr.addr, 6))
    {
    ...
    bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_CLI_CMGR_CALLBACK_CON_ID_EVENT_STATUS, ptr1->con_id, Event, ptr1->status);
    ...
    switch (Event)
    {
        case CMEVENT_DATA_LINK_CON_CNF:
            if (Status == BT_STATUS_SUCCESS)
            {
                ...
                {
                    bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_LINK_CON_CNF_TRY_OPEN_RFCHNL);
		    int index;
		    for (index = 0; index < JSR82_MAX_SESSION_NO; index++)
    		    {
        	        if (BTJ82SEAP(session)[index].status == BT_JSR82_SESSION_ACTIVE && 
			    BTJ82SEAP(session)[index].channel == ptr1->channel)
        		{
	            ...
	            if (index < JSR82_MAX_SESSION_NO)
	            {
			 if (BTJ82SEAP(session)[index].security_value == 0)
			 {
				 ret_Status = RF_OpenJSR82ClientChannel(Handler->remDev, (U8) (ptr1->channel), &ptr->rfc, 1, 0);
			 }
			 else 
			 {
				 ret_Status = RF_OpenJSR82ClientChannel(Handler->remDev, (U8) (ptr1->channel), &ptr->rfc, 1, 1);
			 }
			 } else 
			 {
			     OS_Report("invalid channel:%d", ptr1->channel);
			     break;
			 }
            ...
            if (ret_Status != BT_STATUS_PENDING)
            {
		      ...
		      parms.p.connect_cnf.bd_addr = (U8*) ptr1->bd_addr;
		      parms.p.connect_cnf.ps_type = ptr1->ps_type;
		      parms.p.connect_cnf.mtu = ptr1->mtu;
		      parms.p.connect_cnf.in_mtu = 0;
		      parms.p.connect_cnf.channel = ptr1->channel;
		      parms.p.connect_cnf.index = 0;
		      parms.p.connect_cnf.identify = ptr1->identify;
...
void bt_jsr82_HandleSessionApEnableService(bt_jsr82_enable_service_req_struct *ptr)
{
    ...
    if (index == JSR82_MAX_SESSION_NO)
    {
        ...
        //bt_jsr82_SessionApEnableServiceCfn(JSR82_SESSION_REGISTRARION_FAILED, ptr->ps_type, 0x00, 0x00, ptr->identify);
        parms.p.enable_service_cnf.result = JSR82_SESSION_REGISTRARION_FAILED;
        parms.p.enable_service_cnf.ps_type = ptr->ps_type;
        parms.p.enable_service_cnf.channel = 0x00;
        parms.p.enable_service_cnf.index = 0x00;
    ...
    if (BTJ82SEAP(session)[index].ps_type == JSR82_SESSION_PS_RFCOMM)
    {
        ...
        if (BTJ82SEAP(session)[index].channel == JSR82_NO_AVAILABLE_SESSION)
        {
            ...
            bt_jsr82_SessionApEnableServiceCfn(
                JSR82_SESSION_REGISTRARION_FAILED,
                ptr->ps_type,
                0x00,
                0x00,
                ptr->identify);
...
//    #else /* defined (__BT_2_1_SIMPLE_PAIRING__) */ 
	...
	if (security_value == 0x00)
        {
            status = RF_JSR82RegisterServerChannel(rfcomm_channel, &srv_chn, 1, 0);
	} 
	else 
	{
	    status = RF_JSR82RegisterServerChannel(rfcomm_channel, &srv_chn, 1, 1);
	}
        bt_trace(
            BT_TRACE_JSR82_GROUP,
    ...
    if (status == BT_STATUS_SUCCESS)
    {
        ...
        /*
        bt_jsr82_SessionApEnableServiceCfn(
            JSR82_SESSION_REGISTRARION_SUCCESS,
            ptr->ps_type,
            (U16) BTJ82SEAP(session)[index].channel,
            index,
            tr->identify);
        */
        parms.p.enable_service_cnf.result = JSR82_SESSION_REGISTRARION_SUCCESS;
        parms.p.enable_service_cnf.ps_type = ptr->ps_type;
        parms.p.enable_service_cnf.channel = (U16) BTJ82SEAP(session)[index].channel;
    ...
    {
        BTJ82SEAP(session)[index].status = BT_JSR82_SESSION_IDLE;
        //bt_jsr82_SessionApEnableServiceCfn(JSR82_SESSION_REGISTRARION_FAILED, ptr->ps_type, 0, 0x00, ptr->identify);
        parms.p.enable_service_cnf.result = JSR82_SESSION_REGISTRARION_FAILED;
        parms.p.enable_service_cnf.ps_type = ptr->ps_type;
        parms.p.enable_service_cnf.channel = 0;
        parms.p.enable_service_cnf.index = 0x00;
        parms.p.enable_service_cnf.identify = ptr->identify;
	 parms.event = JSR82_EVENT_ENABLE_SERVICE_CNF;
	 parms.dstMod = MOD_MMI;
...
void bt_jsr82_HandleSessionApTurnOnService(bt_jsr82_turnon_service_req_struct *ptr)
{
        ...
        (BTJ82SEAP(session)[index].ps_type != ptr->ps_type))
    {
        ...
        //bt_jsr82_SessionApTurnOnServiceCfn(JSR82_SESSION_TURNON_FAILED, ptr->ps_type, index, transaction_id);
        parms.p.turn_on_service_cnf.result = JSR82_SESSION_TURNON_FAILED;
    ...
    else
    {
        ...
        BTJ82SEAP(session)[index].identify = ptr->identify;
        //bt_jsr82_SessionApTurnOnServiceCfn(JSR82_SESSION_TURNON_SUCCESS, ptr->ps_type, index, transaction_id);
        parms.p.turn_on_service_cnf.result = JSR82_SESSION_TURNON_SUCCESS;
        parms.p.turn_on_service_cnf.ps_type = ptr->ps_type;
        parms.p.turn_on_service_cnf.con_id = index;
        parms.p.turn_on_service_cnf.identify = transaction_id;
	 parms.event = JSR82_EVENT_TURN_ON_SERVICE_CNF;
...
void bt_jsr82_HandleSessionApTurnOffService(bt_jsr82_turnoff_service_req_struct *ptr)
{
        ...
        (BTJ82SEAP(session)[index].ps_type != ptr->ps_type))
    {
        ...
        //bt_jsr82_SessionApTurnOffServiceCfn(JSR82_SESSION_TURNOFF_FAILED, ptr->ps_type, index, transaction_id);
        parms.p.turn_off_service_cnf.result = JSR82_SESSION_TURNON_FAILED;
    ...
    else
    {
        ...
        BTJ82SEAP(session)[index].authorize = BT_JSR82_AUTHORIZE_AUTO_REJECT;
        BTJ82SEAP(session)[index].identify = ptr->identify;
        //bt_jsr82_SessionApTurnOffServiceCfn(JSR82_SESSION_TURNOFF_SUCCESS, ptr->ps_type, index, transaction_id);
        parms.p.turn_off_service_cnf.result = JSR82_SESSION_TURNOFF_SUCCESS;
        parms.p.turn_off_service_cnf.ps_type = ptr->ps_type;
        parms.p.turn_off_service_cnf.con_id = index;
        parms.p.turn_off_service_cnf.identify = transaction_id;
	 parms.event = JSR82_EVENT_TURN_OFF_SERVICE_CNF;
...
void bt_jsr82_HandleConReqTimeoutHdlr(EvmTimer *timer)
{
    ...
    bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_CON_REQ_TIMEOUT_HDLR, bt_power_status);
...
void bt_jsr82_HandleSessionApConnectReq(bt_jsr82_connect_req_struct *ptr)
{
    ...
    if (BTJ82SEAP(jsr82_con_req_timer).func == 0)
    {
        BTJ82SEAP(jsr82_con_req_timer).func = bt_jsr82_HandleConReqTimeoutHdlr;
        btmtk_os_memcpy((U8*) & BTJ82SEAP(jsr82_con_req), (U8*) ptr, sizeof(bt_jsr82_connect_req_struct));
        EVM_StartTimer(&BTJ82SEAP(jsr82_con_req_timer), 500);
    }
    else
    {
    	 JSR82CallbackParms parms;
    	 
        bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_CON_REQ_PREVIOUS_ONGOING);
        /* Previous one connection request is ongoing */
#if 0        
        bt_jsr82_SessionApConnectCfn(
            (U8*) ptr->bd_addr,
            ptr->ps_type,
...
//#else /* defined (__BT_2_1_SIMPLE_PAIRING__) */ 
    ...
    if (status == BT_STATUS_SUCCESS)
    {
	    ...
	    if (security_value == 0x00)
    	    {			
    		ret_status = RF_OpenJSR82ClientChannel(cmgrhdl->remDev,
                            (U8) (BTJ82SEAP(session)[index].channel),
                            channel,
                            session_devCredit,
                            0);
	    }
    ...
    else
    {
        ...
        bt_jsr82_SessionApConnectCfn(
            ...
            ptr->identify,
            0,
            JSR82_SESSION_CONNECT_CLIENT_INVALID_PARMS);
#endif
	  parms.p.connect_cnf.bd_addr = (U8*) ptr->bd_addr;
	  parms.p.connect_cnf.ps_type = ptr->ps_type;
	  parms.p.connect_cnf.mtu = mtu;
	  ...
	  parms.p.connect_cnf.identify = ptr->identify;
	  parms.p.connect_cnf.l2cap_id = 0;
	  parms.p.connect_cnf.result = JSR82_SESSION_CONNECT_CLIENT_INVALID_PARMS;
	  parms.event = JSR82_EVENT_CONNECT_CNF;
	  parms.dstMod = MOD_MMI;
	  JSR82CB(&parms);
    }
...
void bt_jsr82_HandleSessionApL2capConnectReq(bt_jsr82_connect_req_struct *ptr)
{
    ...
    CmgrHandler *cmgrhdl = NULL;
    BD_ADDR addr;
    BT_JSR82_SESSION_Buffer_struct_t *buffer = 0;
    U16 l2cap_cid = 0;
    U8 psm_index = 0;
    JSR82CallbackParms parms;
    U16 mtu = 0;
    ...
    bt_jsr82_init_session_buffer((BT_JSR82_SESSION_Buffer_struct_t*) BTJ82SEAP(session)[index].session_buffer);
#ifdef BTMTK_ON_LINUX
    buffer = (BT_JSR82_SESSION_Buffer_struct_t*) (U8*)jsr82_core_malloc_ashm(ptr->memName, index, JSR82_ASHM_TYPE_CHANNEL_BUFFER);
#else
    buffer = (BT_JSR82_SESSION_Buffer_struct_t*) ptr->channel_context;
#endif
    ((BT_JSR82_SESSION_Buffer_struct_t*) (BTJ82SEAP(session)[index].session_buffer))->mainRecord =
        (U8*) & BTJ82SEAP(session)[index];
    BTJ82SEAP(session)[index].ps_type = ptr->ps_type;
...
void bt_jsr82_HandleSessionApTxReq(bt_jsr82_tx_data_req_struct *ptr)
{
    ...
    for (i = 0; i < JSR82_MAX_SESSION_NO; i++)
    {
        if ((BTJ82SEAP(session)[i].status == BT_JSR82_SESSION_ACTIVE) && (ptr->index == BTJ82SEAP(session)[i].con_id))
    ...
    if (BTJ82SEAP(session)[i].ps_type == JSR82_SESSION_PS_RFCOMM)
    {
        ...
        /* Add by mtk01411: 2007-0916 */
        bt_jsr82_sendToRfComm(ptr->index);
        // TODO: Is this check necessary?
        //jsr82_session_CheckReadyToWrite(BTJ82SEAP(session)[i].ps_type, ptr->index, subsession_id, NULL);

    #if 0   /* Disabled by mtk01411: 2007-0916 */
        rfc_size = RF_FrameSize(&((BT_JSR82_SESSION_Buffer_struct_t *)BTJ82SEAP(session)[index].session_buffer)->rfc);
        if(ptr->length > rfc_size)
            bt_jsr82_SessionApTxCfn(ptr->identify, 0, JSR82_SESSION_TX_SIZE_TOO_LARGE);
        if(!IsListEmpty(&BTJ82SEAP(session)[index].ApTxQueue))
        {
            pkt = (BtPacket *)RemoveHeadList(&BTJ82SEAP(session)[index].ApTxQueue);
            /* Send the packet */
            pkt->flags = BTP_FLAG_INUSE;
            pkt->data = ptr->data;
            pkt->dataLen = ptr->length;
            pkt->headerLen = 0;
            pkt->priority_type = BTP_FLAG_HIGH_PRIPORTY; 
            status = RF_SendData(&((BT_JSR82_SESSION_Buffer_struct_t *)BTJ82SEAP(session)[index].session_buffer)->rfc, pkt);
            if(status != BT_STATUS_PENDING)
                bt_jsr82_SessionApTxCfn(ptr->identify, 0, JSR82_SESSION_TX_ERROR);
        }
        else
            bt_jsr82_SessionApTxCfn(ptr->identify, 0, JSR82_SESSION_TX_ERROR);
    #endif /* 0 */ 
    ...
    {
        /* Add by mtk01411: 2007-1104 */
        bt_jsr82_sendToL2Cap(ptr->index, ptr->l2cap_id);
        // TODO: Is this check necessary?
        //jsr82_session_CheckReadyToWrite(BTJ82SEAP(session)[i].ps_type, ptr->index, ptr->l2cap_id, NULL);
    #if 0
        if(!IsListEmpty(&BTJ82SEAP(session)[i].ApTxQueue))
        {
            pkt = (BtPacket *)RemoveHeadList(&BTJ82SEAP(session)[i].ApTxQueue);
            /* Send the packet */
            pkt->flags = BTP_FLAG_INUSE;
            pkt->data = ptr->data;
            pkt->dataLen = ptr->length;
            pkt->headerLen = 0;
            pkt->priority_type = BTP_FLAG_HIGH_PRIPORTY; 
            status = L2CAP_SendData(ptr->l2cap_id, pkt);
            if(status != BT_STATUS_PENDING)
                bt_jsr82_SessionApTxCfn(ptr->index, ptr->l2cap_id, JSR82_SESSION_TX_ERROR);
        }
        else
            bt_jsr82_SessionApTxCfn(ptr->index, ptr->l2cap_id, JSR82_SESSION_TX_ERROR);
    #endif /* 0 */ 
...
void bt_jsr82_HandleSessionApDisableService(bt_jsr82_disable_service_req_struct *ptr)
{
    ...
    for (i = 0; i < JSR82_MAX_SESSION_NO; i++)
    {
        ...
        {
            bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_DISABLE_SERVICE_FIND_JSR82_CHANNEL_x02X, i);
            break;
        }
    }
    if (i == JSR82_MAX_SESSION_NO)
    {
        bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_DISABLE_SERVICE_NO_EMPTY_CHANNEL);
        //bt_jsr82_SessionApDisableServiceCfn(0, ptr->index, ptr->identify, BT_JSR82_SESSION_DISABLED_FAILED);
        parms.p.disable_service_cnf.ps_type = 0;
        parms.p.disable_service_cnf.con_id = ptr->index;
        parms.p.disable_service_cnf.identify = ptr->identify;
        parms.p.disable_service_cnf.result = BT_JSR82_SESSION_DISABLED_FAILED;
    ...
    if (BTJ82SEAP(session)[i].ps_type == JSR82_SESSION_PS_RFCOMM)
    {
        ...
        if (BTJ82SEAP(session)[i].channel_state != BT_JSR82_CHANNEL_STATE_IDLE)
        {
            ...
            bt_jsr82_SessionApDisableServiceCfn(
                ...
                ptr->index,
                ptr->identify,
                BT_JSR82_SESSION_DISABLED_FAILED);
            */
            parms.p.disable_service_cnf.ps_type = JSR82_SESSION_PS_RFCOMM;
            parms.p.disable_service_cnf.con_id = ptr->index;
            parms.p.disable_service_cnf.identify = ptr->identify;
            parms.p.disable_service_cnf.result = BT_JSR82_SESSION_DISABLED_FAILED;
	     parms.event = JSR82_EVENT_DISABLE_SERVICE_CNF;
        ...
        srv_chn.serviceId = (U8) BTJ82SEAP(session)[i].channel;
        status = RF_DeregisterServerChannel(channel, &srv_chn);
        RF_JSR82_DeRegisterServerChannel();
    ...
    else
    {
        ...
        if (temp_ptr->l2capCtx.l2cap_con_state != BT_JSR82_CHANNEL_STATE_IDLE)
        {
            ...
            bt_jsr82_SessionApDisableServiceCfn(
                ...
                BT_JSR82_SESSION_DISABLED_FAILED);
            */
            parms.p.disable_service_cnf.ps_type = JSR82_SESSION_PS_L2CAP;
            parms.p.disable_service_cnf.con_id = ptr->index;
            parms.p.disable_service_cnf.identify = ptr->identify;
            parms.p.disable_service_cnf.result = BT_JSR82_SESSION_DISABLED_FAILED;
	     parms.event = JSR82_EVENT_DISABLE_SERVICE_CNF;
    ...
    if (status == BT_STATUS_SUCCESS)
    {
	 ...
	 jsr82_session_free_RWRingBuf(i);        
        ...
        //bt_jsr82_SessionApDisableServiceCfn(ps_type, ptr->index, ptr->identify, BT_JSR82_SESSION_DISABLED_SUCCESS);
        parms.p.disable_service_cnf.ps_type = ps_type;
        parms.p.disable_service_cnf.con_id = ptr->index;
        parms.p.disable_service_cnf.identify = ptr->identify;
        parms.p.disable_service_cnf.result = BT_JSR82_SESSION_DISABLED_SUCCESS;
	 parms.event = JSR82_EVENT_DISABLE_SERVICE_CNF;
	 parms.dstMod = MOD_MMI;
	 JSR82CB(&parms);
        return;
    ...
    {
        //bt_jsr82_SessionApDisableServiceCfn(ps_type, ptr->index, ptr->identify, BT_JSR82_SESSION_DISABLED_FAILED);
        parms.p.disable_service_cnf.ps_type = ps_type;
        parms.p.disable_service_cnf.con_id = ptr->index;
        parms.p.disable_service_cnf.identify = ptr->identify;
        parms.p.disable_service_cnf.result = BT_JSR82_SESSION_DISABLED_FAILED;
...
#if 0
    if(session_ap_record_ptr->client_server == BT_JSR82_SESSION_CLIENT)
        /* Note: Because CONNECT_CNF is not reported to JBT, the con_id is not kept in session_entry, it must use transaction_id to find correct session_entry : 2007-1103 */
        rx_free_size = jsr82_session_RxFree(JBT_LIST_SEARCH_TYPE_BY_TRANSACTION_ID, JBT_SESSION_TYPE_SPP, subsession_id,session_ap_record_ptr->identify);
    else
        /* For Server role, it must use the chnl_num to find the registered service: Modified from BY_TRANSACTION_ID to BY_BY_CHNL_NUM */  
        rx_free_size = jsr82_session_RxFree(JBT_LIST_SEARCH_TYPE_BY_CHNL_NUM, JBT_SESSION_TYPE_SPP, subsession_id,session_ap_record_ptr->channel);
#endif /* 0 */ 
    ...
    for (i = 0; i < JSR82_MAX_ACL_NO; i++)
    {
        if (BTJ82SEAP(active_link)[i].used == 1)
        {
            if (TRUE == btmtk_os_memcmp(BTJ82SEAP(active_link)[i].link->bdAddr.addr, 6, ptr->bd_addr, 6))
            {
                ...
                {
                    //bt_jsr82_SetACLSecurityCnf(JSR82_SET_ACL_SECURITY_SUCCESS, ptr->transaction_id, ptr->bd_addr);
		      ...
		      JSR82CB(&parms);
                ...
                else
                {
                    ...
                    //bt_jsr82_SetACLSecurityCnf(JSR82_SET_ACL_SECURITY_FAILED, ptr->transaction_id, ptr->bd_addr);
		      parms.p.set_acl_security_cnf.result = JSR82_SET_ACL_SECURITY_FAILED;
		      parms.p.set_acl_security_cnf.transaction_id = ptr->transaction_id;
		      parms.p.set_acl_security_cnf.addr = ptr->bd_addr;
		      parms.event = JSR82_EVENT_SET_ACL_SECURITY_CNF;
...
void bt_jsr82_sendToRfComm(U8 session_inx)
{
    ...
    /* Check if there is available data in JBT_Write_RingBuf */
#if 0
    /* JBT_Check_WriteRingBuf(session_inx, (char **)&pkt->data, &bytes); */
#endif /* 0 */ 
    ...
    /* In error state: eg., POWEROFF state or session is not in connected state: 2007-1115 */
    if ((signed char) rc < 0)
    {
