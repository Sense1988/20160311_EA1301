...
void HdpSdpDB_UpdateFeatures()
{
	...
	else if (NULL != attribute->value)
	{
		OS_Report("[HDP]attribute->value reset: %d", attribute->value);
		hdp_util_free(attribute->value);
		attribute->value = NULL;
	}
	attribute->value = (U8 *)hdp_util_malloc( totalSize);
	OS_Report("[HDP]attribute->value malloc: %d", attribute->value);
	if (NULL == attribute->value)
	{
		OS_Report("[HDP][SDP] fatal error: fail to allocate mempory");
		return;
	}
	attribute->len = totalSize;
	...
	else
	{
		...
		OS_MemCopy(attribute->value, HdpMdepList_Header, sizeof(HdpMdepList_Header));
	}
}
