...
BOOL ftpobs_util_compareFileContent(U32 *filepath, U8 *buff, U32 len){
	...
	U32 pos;
	S32 filesize;
	filesize = btmtk_fs_get_filesize(filepath);
	if( len != filesize ){
		FTPAssert(0);
	}
	
	fh = btmtk_fs_open_ucs2(filepath, BTMTK_FS_READ_ONLY);
	if( fh <= 0 ){
		FTPAssert(0);
	}
...
ObexRespCode FTPOBS_TestMemBuffer_Read(){
	...
	U8 *l_pfilepath = L"c:\\temp\\1\\1k.txt";
	S32 MAXSIZE = FTP_INTERNAL_RW_BUFFER_SIZE ;
...
ObexRespCode FTPOBS_TestMemBuffer(){
	...
	FtpcObStoreHandle handler;
	//U8 *l_pfilepath = L"/mnt/sdcard/test_one_write.txt";
	U8 *l_pfilepath = L"c:\\temp\\1\\1.txt";
	S32 MAXSIZE = FTP_INTERNAL_RW_BUFFER_SIZE ;
	...
	for(pos = 0; pos < MAXSIZE; pos++)
	{
		...
		for( len1 = 1; len1 < (MAXSIZE+2); len1++){
			...
			if( TRUE != ftpobs_util_compareFileContent(l_pfilepath, buff, len1) ){
				...
				OS_Report("[testbuffer] pass pos:%d len:%d\n", pos, len1);
			}
		}
	}
#endif
	...
	for(pos = 0; pos < MAXSIZE; pos++)
	{
		...
		for( len1 = 1; len1 < (MAXSIZE+2); len1++){
			...
			// del the file
			btmtk_fs_delete_ucs2(l_pfilepath);
			handler = FTPOBS_New(NULL, TRUE, l_useMem, 0);
			if( handler == NULL ){
				FTPAssert(0);
			}
			...
			// compare
			if( TRUE != ftpobs_util_compareFileContent(l_pfilepath, buff, len1+len2) ){
				FTPAssert(0);
			}else{
				OS_Report("[testbuffer] pass pos:%d len:%d\n", pos, len1);
			}
...
// start write twice
	...
	for(pos = 0; pos < MAXSIZE; pos++)
	{
		...
		for( len1 = 1; len1 < (MAXSIZE+2); len1++){
			...
			// compare
			if( TRUE != ftpobs_util_compareFileContent(l_pfilepath, buff, len1+len2+len3) ){
				FTPAssert(0);
			}else{
				OS_Report("[testbuffer] pass pos:%d len:%d len2:%d len3:%d\n", pos, len1, len2, len3);
			}
				}
			}
		}
...
ObexRespCode FTPOBS_Close(FtpcObStoreHandle *obshPtr, BT_BOOL bDel)
{
    ...
    if (obsh->ocx != 0) {
        ...
        if( bDel ){
            ...
            if( NULL != obsh->path && BTMTK_FS_OK == btmtk_fs_delete_ucs2( obsh->path ) ){
                rcode = OBRC_SUCCESS;
            }else{
...
ObexRespCode FTPOBS_Open(FtpcObStoreHandle *obshPtr, BT_BOOL writeable)
{
	...
	if( writeable ){
		obsh->ocx = btmtk_fs_open_ucs2(obsh->path, BTMTK_FS_CREATE |BTMTK_FS_CREATE_ALWAYS );
	}else{
		obsh->ocx = btmtk_fs_open_ucs2(obsh->path, BTMTK_FS_READ_ONLY);
	}
...
void FTPOBS_AppendFilePath(FtpcObStoreHandle obsh, const U8 *path, U16 len)
{
...
#if 1
	{
		U8 filepath[128];
		memset(filepath, 0, sizeof(filepath) );
		if( len != 0  && bt_ucs2strlen(obsh->path) == 0 ){
    ...
    if( obsh->flags == 0 ){
		...
		if( obsh->membuffer && obsh->membuffer_len > 0 ){
			...
			if( obsh->membuffer && 0 == obsh->membuffer_len ){
				// read from buffer
				result2 = btmtk_fs_read_ucs2(obsh->ocx, obsh->membuffer, obsh->membuffer_size, &obsh->membuffer_len);
			...
			if( len > (obsh->membuffer_size-obsh->membuffer_len) ){
				...
				if( fs_ret < 0 || written != part1_len){
					...
					if( obsh->membuffer_len > 0 ){
						...
						if( fs_ret < 0 || written != obsh->membuffer_len){
							FTPAssert(0);
						}else{
							obsh->membuffer_len = 0;
						}
...
ObexRespCode FTPOBS_MoveOpenFilePath(FtpcObStoreHandle obsh, const U8 *path, U16 len){
    ...
    GOEP_Report("[FTP] FTPOBS_MoveOpenFilePath obsh:0x%x newpath_len:%d newpath_len:%d test5", obsh, len, bt_ucs2strlen(path)*2 );
    ...
    new_ocx = btmtk_fs_open_ucs2(path, BTMTK_FS_CREATE|BTMTK_FS_CREATE_ALWAYS );
    if( new_ocx < 0 ){
	...
	obsh->ocx = NULL;
	if(obsh->path){
		btmtk_fs_delete_ucs2( obsh->path );
	}
	...
	if( obsh->writeable && obsh->membuffer && 0 !=obsh->membuffer_len ){
		...
		if( written != part1_len || fs_ret < 0 ){
			...
			if(  obsh->membuffer_len > 0){
				...
				if( written != obsh->membuffer_len || fs_ret < 0 ){
					ret = OBRC_FORBIDDEN;
				}else{
					obsh->membuffer_len = 0;
				}
...
void FTPOBS_TestCode(S8 *code, S8 *filepath){
	...
	if( code && 0 == bt_ucs2cmp(code, (const U8 *)L"mtkbttestsd") && 11 == bt_ucs2strlen((const S8 *)code)){
		...
		for( j = 0; j< 4; j++){
			...
			for( i = 0; i< 4; i++){
				...
				pos = read = write = 0;
				fh = btmtk_fs_open_ucs2(filepath, BTMTK_FS_CREATE|BTMTK_FS_CREATE_ALWAYS );
				if( fh < 0 ){
					break;
				}
				// mark time1
