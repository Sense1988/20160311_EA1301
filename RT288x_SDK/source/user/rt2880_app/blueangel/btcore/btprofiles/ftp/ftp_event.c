    ...
    if (server->currOp.handler) 
    {
            ...
            if( NULL == serverApp->goep_buff_ptr ){
                ...
                if(server->currOp.oper == GOEP_OPER_PUSH)
                {
					...
					if( goepApp->object ){
						...
						bt_ucs2ncpy(filepath, FTPOBS_GetFilePath(goepApp->object), sizeof(filepath)/2);
						FTPOBS_Close( &goepApp->object, FALSE );
						goepApp->object = goepApp->object;
					}
					ftps_send_pushfile_rsp_ind_msg(serverApp->goep_conn_id, GOEP_STATUS_SUCCESS, FTPS_TEMP_PUSH_FILEPATH, gFtp.ftps_filepath, server->obs.server.rxOffset);
                }else  if(server->currOp.oper == GOEP_OPER_PULL)
                {
                    // close file
					if( goepApp->object ){
        ...
        }else if(server->currOp.event == GOEP_EVENT_AUTH_RESPONSE)
		{
		    ...
		    if(server->currOp.oper == GOEP_OPER_PUSH)
            {
				...
				if( goepApp->object ){
				    FTPOBS_AppendFilePath( goepApp->object, FTPS_TEMP_PUSH_FILEPATH, bt_ucs2strlen(FTPS_TEMP_PUSH_FILEPATH) *2 ); //TODO: need to move this
				}
			...
			if( serverApp->goep_buff_ptr ){
				...
				if(server->currOp.oper == GOEP_OPER_PUSH)
                {
					...
					if( server->currOp.reason == OBRC_LINK_DISCONNECT ){
						ftps_send_pushfile_rsp_ind_msg(serverApp->goep_conn_id, GOEP_USER_UNKNOW, NULL, NULL);
					}else{
						ftps_send_pushfile_rsp_ind_msg(serverApp->goep_conn_id, GOEP_USER_CANCEL, NULL, NULL);
					}
					if( goepApp->object ){
						FTPOBS_Close( &goepApp->object, TRUE );
						goepApp->object = NULL;
...
void FtpGoepClientCallback(GoepClientEvent *event)
{
...
#endif				
                ftpc_send_connect_cnf_msg(clientApp->req_id, 0xFF, GOEP_STATUS_FAILED, NULL, 0, 0);
    ...
    else if(Event == GOEP_EVENT_COMPLETE)
    {
        ...
        else
        {
            ...
            (Event == GOEP_EVENT_NO_SERVICE_FOUND))
    {
        ftpc_send_connect_cnf_msg(clientApp->req_id, 0xFF, GOEP_STATUS_FAILED, NULL, uuid, uuid_len);
    }
    else if (Event == GOEP_EVENT_SCO_REJECT)
    {
        ftpc_send_connect_cnf_msg(clientApp->req_id, 0xFF, GOEP_STATUS_SCO_REJECT, NULL, uuid, uuid_len);
    }
    else if (Event == GOEP_EVENT_BTCHIP_REJECT)
    {
        ftpc_send_connect_cnf_msg(clientApp->req_id, 0xFF, GOEP_STATUS_BUSY, NULL, uuid, uuid_len);
    }
