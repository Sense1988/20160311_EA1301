...
void ftpc_pushfile_req_hdlr(bt_ftpc_pushfile_req_struct* req){
    ...
    if(client != NULL)
    {
        ...
        if((req->obj_name[0] != 0)||(req->obj_name[1] != 0))
        {
            ...
            bt_ucs2ncpy( (U8*) object.name, req->obj_name, GOEP_MAX_OBJ_NAME/sizeof(U16));
        }
        else
            object.name = NULL;
        ...
        if((rsp_code != OB_STATUS_SUCCESS)&&(rsp_code != OB_STATUS_PENDING))
        {
            ...
            ftpc_send_pushfile_cnf_msg(req->goep_conn_id, rsp_code, 1, uuid, uuid);
            return;
        }
...
void ftps_pushfile_rsp_hdlr(bt_ftps_pushfile_rsp_struct *rsp){
	...
	if( rsp->rsp_code != GOEP_STATUS_SUCCESS ){
		...
		if( NULL == goepApp->object ){
			...
			if( OBRC_SUCCESS != FTPOBS_Open(&goepApp->object, TRUE) ){
		    ...
		    /* move the object */
            if( rsp->filepath_len == 0 || OBRC_SUCCESS != FTPOBS_MoveOpenFilePath(goepApp->object, rsp->filepath, rsp->filepath_len) ){
...
void ftps_pullfile_rsp_hdlr(bt_ftps_pullfile_rsp_struct *rsp){
	...
	if( rsp->rsp_code != GOEP_STATUS_SUCCESS ){
		...
		goepApp->object = FTPOBS_New( &goepApp, FALSE, l_useMem, rsp->goep_conn_id);
		FTPOBS_AppendFilePath( goepApp->object, rsp->filepath, rsp->filepath_len );
		if( OBRC_SUCCESS == FTPOBS_Open( &goepApp->object, FALSE) ){
			// setup object length
			ObexAppHandle = &(GOES(servers)[goepApp->connId].obs);
			goepcons = &GOES(servers)[goepApp->connId];
...
*  None
*****************************************************************************/
int ftp_find_obex_uuid_with_bt_uuid(U32* bt_uuid_p, U8* obex_uuid_str, U8* obex_uuid_len)
{
    ...
    if(client != NULL)
    {
        ...
        if(ftpc_push_req->pkt_type == GOEP_FIRST_PKT || GOEP_SINGLE_PKT == ftpc_push_req->pkt_type)
        {
            ...
            if((ftpc_push_req->obj_name[0] != 0)||(ftpc_push_req->obj_name[1] != 0))
            {
                ...
                /* Transfer to utf-8 first */
                object.name = get_ctrl_buffer(GOEP_MAX_OBJ_NAME);
                bt_ucs2ncpy( (U8 *)object.name, ftpc_push_req->obj_name, GOEP_MAX_OBJ_NAME/sizeof(U16));
            }
            else
            {
                object.name = NULL;
            ...
            {
                object.type = get_ctrl_buffer(GOEP_MAX_MIME_TYPE);
                bt_strncpy((U8 *)object.type, (U8 *)ftpc_push_req->obj_mime_type, GOEP_MAX_MIME_TYPE);
            }
            else
                object.type = NULL;
    ...
    if(client != NULL)
    {
        ...
        {       
            ...
            if((ftp_pull_req->obj_name[0] != 0)||(ftp_pull_req->obj_name[1] != 0))
            {
                /* Transfer to utf-8 first */
                object.name = get_ctrl_buffer(GOEP_MAX_OBJ_NAME);
                bt_ucs2ncpy( (U8*) object.name, ftp_pull_req->obj_name, GOEP_MAX_OBJ_NAME/sizeof(U16));
            }
            else
                object.name = NULL;
...
void ftpc_set_folder_req_hdlr(ftpc_set_folder_req_struct* ftp_set_folder_req) 
{
    ...
    if(client != NULL)
    {
        switch(ftp_set_folder_req->setpath_flag)
        {
            ...
            bt_ucs2ncpy( (U8*)folder.name, (const U8*)ftp_set_folder_req->folder_name, GOEP_MAX_FOLDER_NAME /sizeof(U16));
            folder.reset = FALSE;
...
void ftpc_action_req_hdlr(ftpc_action_req_struct* ftpc_action_req) 
{
    ...
    if(client != NULL)
    {
        ...
        bt_ucs2ncpy( (U8*)action.name, (const U8*)ftpc_action_req->folder_name, GOEP_MAX_FOLDER_NAME /sizeof(U16));
        
        if( OBAP_SETPARAMISSION != action.id ){
            action.dstname = get_ctrl_buffer(GOEP_MAX_FOLDER_NAME + 2);
            bt_ucs2ncpy( (U8*)action.dstname, (const U8*)ftpc_action_req->dst_name, GOEP_MAX_FOLDER_NAME /sizeof(U16));
        }
