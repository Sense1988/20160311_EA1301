...
extern void A2DP_SendStreamData(U8 stream_handle, U8 *data, U32 len, U32 sample_count);
...
BOOL bt_adp_a2dp_unpack_audio_cap(A2dpCodec *src, bt_a2dp_audio_cap_struct *dst)
{
    ...
    dst->sep_type = BT_A2DP_SINK; //Klocwork warning.
#endif
...
void a2dp_adp_message_handler(ilm_struct *ilm_ptr)
{
    ...
    switch (ilm_ptr->msg_id)
    {
        ...
        case MSG_ID_BT_A2DP_ACTIVATE_REQ:
            ...
            A2DP_Activate(((bt_a2dp_activate_req_struct*) msg_p)->default_local_role,
		...
		case MSG_ID_BT_A2DP_STREAM_CHANNEL_OPEN_REQ:
		{
			Report(("[INFO] a2dp_adp_message_handler STREAM_CHANNEL_OPEN_REQ, stream_handler:%d", ((bt_a2dp_stream_channel_open_req_struct*) msg_p)->stream_handle));
			A2DP_StreamChannelOpenReq(((bt_a2dp_stream_channel_open_req_struct*) msg_p)->stream_handle);
		}
			break;

#ifdef __PRJ_MT7650_SONY_BDP__
		case MSG_ID_BT_A2DP_AAC_ENABLE_REQ:
		    A2DP_AAC_Enable(((bt_a2dp_aac_enable_req_struct*) msg_p)->aac_enable);
		    break;
		case MSG_ID_BT_A2DP_SET_LOCAL_ROLE_REQ:
		    A2DP_SetLocalRole(((bt_a2dp_set_local_role_req_struct*) msg_p)->default_local_role);
    ...
    U8 *dev_name, U8 name_len, bt_a2dp_audio_cap_struct *config)
{
	...
	if(config)
		...
		memset((U8 *)&bta2dp_adp_msg->current_config, 0, sizeof(bt_a2dp_audio_cap_struct));
...
void BTA2DPAdpStreamDataInd(U8 stream_handle,U8 *buf,U16 buf_size)
{
    bt_a2dp_stream_data_ind_struct *bta2dp_adp_msg;
    bta2dp_adp_msg = (bt_a2dp_stream_data_ind_struct*) construct_local_para(
                                                            sizeof(bt_a2dp_stream_data_ind_struct),
                                                            TD_UL);

    bta2dp_adp_msg->stream_handle = stream_handle;
    bta2dp_adp_msg->buf_size = buf_size;
    #ifdef __PRJ_MT7650_SONY_BDP__
    memset(bta2dp_adp_msg->buf, 0, 1015);
    memcpy(bta2dp_adp_msg->buf, buf, buf_size);
    #endif
...
void BTA2DPAdpStreamSetConfigCnf(U16 result, U8 stream_handle)
{
	bt_a2dp_stream_config_cnf_struct *bta2dp_adp_msg;
    bta2dp_adp_msg =
        (bt_a2dp_stream_config_cnf_struct*) construct_local_para(
                                                    sizeof(bt_a2dp_stream_config_cnf_struct),
                                                    TD_UL);
	bta2dp_adp_msg->result = result;
	Report(("[INFO] BTA2DPAdpStreamSetConfigCnf, stream_handle:%d", stream_handle));
    bta2dp_adp_msg->stream_handle = stream_handle;
    a2dpa_sendMsg(
...
#ifdef __PRJ_MT7650_SONY_BDP__
void BTA2DPAdpSecondStreamInd(bt_addr_struct *first_device_addr, bt_addr_struct *second_device_addr)
{
    bt_a2dp_second_stream_ind_struct *bta2dp_adp_msg;
    bta2dp_adp_msg = (bt_a2dp_second_stream_ind_struct*) construct_local_para(
                                                    sizeof(bt_a2dp_second_stream_ind_struct),
                                                    TD_UL);

    memcpy(&bta2dp_adp_msg->first_device_addr, first_device_addr, sizeof (bt_addr_struct));
    memcpy(&bta2dp_adp_msg->second_device_addr, second_device_addr, sizeof (bt_addr_struct));
	
    a2dpa_sendMsg(MSG_ID_BT_SECOND_STREAM_IND,
                  MOD_EXTERNAL,
                  BT_APP_SAP,
                  (local_para_struct*) bta2dp_adp_msg,
                  NULL);
...
void BTA2DPAdpAvdtpConnectedInd(bt_addr_struct *device_addr)
{
    bt_a2dp_avdtp_connected_ind_struct *bta2dp_adp_msg;
    bta2dp_adp_msg = (bt_a2dp_avdtp_connected_ind_struct*) construct_local_para(
                                                    sizeof(bt_a2dp_avdtp_connected_ind_struct),
                                                    TD_UL);

    memcpy(&bta2dp_adp_msg->device_addr, device_addr, sizeof (bt_addr_struct));
	
    a2dpa_sendMsg(MSG_ID_BT_AVDTP_CONNECTED_IND,
                  MOD_EXTERNAL,
                  BT_APP_SAP,
                  (local_para_struct*) bta2dp_adp_msg,
                  NULL);
...
void BTA2DPAdpAvdtpDisconnectedInd(bt_addr_struct *device_addr)
{
    bt_a2dp_avdtp_disconnected_ind_struct *bta2dp_adp_msg;
    bta2dp_adp_msg = (bt_a2dp_avdtp_disconnected_ind_struct*) construct_local_para(
                                                    sizeof(bt_a2dp_avdtp_disconnected_ind_struct),
                                                    TD_UL);

    memcpy(&bta2dp_adp_msg->device_addr, device_addr, sizeof (bt_addr_struct));
	
    a2dpa_sendMsg(MSG_ID_BT_AVDTP_DISCONNECTED_IND,
                  MOD_EXTERNAL,
                  BT_APP_SAP,
                  (local_para_struct*) bta2dp_adp_msg,
                  NULL);
