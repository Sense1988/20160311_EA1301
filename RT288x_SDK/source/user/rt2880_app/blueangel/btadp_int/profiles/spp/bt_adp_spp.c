...
void spp_adp_init(void)
{
	...
	SPPDRV_Init();
#ifdef ANDROID
	btmtk_adp_spp_start_ashm_service();
#endif
}
...
{
		OS_Report("spp_throughput_test_start(%d)\n", bServer);
...
void spp_throughput_test_send(BOOL bServer)
{
	...
	U16 u2PeriodCount = 0;
//OS_Report("spp_throughput_test_send(%d)\n", bServer?1:0);
	...
	if(bServer)
  {		
              //OS_Report("SPP server start to send data\n");
...
void btmtk_adp_spp_handle_message(ilm_struct *ilm_ptr)
{
    ...
    switch (ilm_ptr->msg_id)
    {
		...
		case MSG_ID_BT_DUN_CLI_ACTIVATE_REQ:
		{
			DUNHandleActivateReq(SPP_CLIENT_PORT);
			break;
		}

		case MSG_ID_BT_DUN_CLI_CONNECT_REQ:
		{
			btmtk_dun_connect_req_struct *connect_req;
			connect_req = (btmtk_dun_connect_req_struct*) ilm_ptr->local_para_ptr;
            DUNHandleConnectReq(
        ...
        case MSG_ID_BT_SPP_CONNECT_REQ:
        {
            ...
            SPPHandleConnectReq(connect_req->lap, 
                                connect_req->uap, 
                                connect_req->nap, 
                                connect_req->server_chnl_num, 
                                connect_req->uuid
	 ...
	 case MSG_ID_BT_SPP_UART_GET_BYTES_REQ:
        #if defined (BTMTK_ON_WIN32) || defined (BTMTK_ON_WISESDK) /*SPP_PORTING*/ || defined (BTMTK_ON_LINUX)        
