...
void bm_adp_message_handler(ilm_struct *ilm_ptr)
{
    ...
    switch (ilm_ptr->msg_id)
    {
        ...
        case MSG_ID_BT_BM_GET_SCATTERNET_STATE_REQ:
            ...
            break;
        case MSG_ID_BT_BM_WRITE_BONDABLE_MODE_REQ:
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            BTBMGapWriteBondable((bt_bm_write_bondable_mode_req_struct*) ilm_ptr->local_para_ptr);
            break;
    #if BT_JSR82_ENABLED
...
#if defined(__PRJ_MT7650_SONY_BDP__)
        case MSG_ID_BT_BM_GET_PAIRED_DEVICE_REQ:
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            BTBMGetPairedDevice((bt_bm_get_paired_device_req_struct*) ilm_ptr->local_para_ptr);
            break;
        case MSG_ID_BT_BM_READ_PROPERTY_REQ:
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            BTBMGapGetWholeProperty();
            break;
        case MSG_ID_BT_BM_WRITE_SSP_MITM_MODE_REQ:
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            BTBMGapSetSSPMitmMode((bt_bm_set_ssp_mitm_req_struct*) ilm_ptr->local_para_ptr);
            break;
        case MSG_ID_BT_BM_WRITE_SSP_BONDING_MODE_REQ:
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            BTBMGapSetSSPBondingMode((bt_bm_set_ssp_bonding_req_struct*) ilm_ptr->local_para_ptr);
            break;
        case MSG_ID_BT_BM_SET_SSP_IO_CAPABILITY_REQ:
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            BTBMGapSetSSPIoCapability((bt_bm_set_ssp_io_capability_req_struct*) ilm_ptr->local_para_ptr);
            break;
        case MSG_ID_BT_BM_SET_VIRTUAL_SNIFFER_REQ:
        {
            #ifdef ENABLE_VIRTUAL_SNIFFER_LOG
            extern int Virtual_Sniffer_RunTime_Config;
            Virtual_Sniffer_RunTime_Config = ((bt_bm_set_virtual_sniffer_req_struct*)ilm_ptr->local_para_ptr)->enable;
            if ( Virtual_Sniffer_RunTime_Config )
            {
                extern int VS_init(void);
                VS_init();
            }
            else
            {
                extern void VS_deinit(void);
                VS_deinit();
            }
            #endif
        ...
        case MSG_ID_BT_BM_SET_TERMINAL_LOG_REQ:
        {
            #ifdef SAVE_TERMINAL_LOG_TO_FILE
            extern int g_Terminal_Log_RunTime_Config;
            g_Terminal_Log_RunTime_Config = ((bt_bm_set_terminal_log_req_struct*)ilm_ptr->local_para_ptr)->enable;
            #endif
        }
#ifdef BT_READ_RSSI
        ...
        {
            MTKBMAC(direct_rsp_module) = ilm_ptr->src_mod_id;
            bt_read_rssi_req_struct* req = (bt_read_rssi_req_struct*) ilm_ptr->local_para_ptr;
            BTBMReadRSSI(req->lap, req->uap, req->nap);
            break;
        }
#endif  /* BT_READ_RSSI */
 ...
 *  void
 *****************************************************************************/
void BTBMAdpWriteBondabledCfm(kal_uint8 result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_bm_write_bondable_mode_cnf_struct *btbm_adp_msg;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    btbm_adp_msg =
        (bt_bm_write_bondable_mode_cnf_struct*) construct_local_para(
                                                    sizeof(bt_bm_write_bondable_mode_cnf_struct),
                                                    TD_UL);

    btbm_adp_msg->result = result;
    bma_sendMsg(
        MSG_ID_BT_BM_WRITE_BONDABLE_MODE_CNF,
        MTKBMAC(direct_rsp_module),
        BT_APP_SAP,
        (local_para_struct*) btbm_adp_msg,
        NULL);
                                  ...
                                  kal_uint32 cod, kal_uint8 name_len, kal_uint8 *name)
{
...
#if defined(__PRJ_MT7650_SONY_BDP__)
    {
        BD_ADDR rem_addr;
        BtDeviceRecord rec;
        CONVERT_BDADDRSRC2ARRAY(rem_addr.addr, &bd_addr);
        btbm_adp_msg->is_paired = ((DDB_FindRecord(&rem_addr, &rec) == BT_STATUS_SUCCESS) ? TRUE : FALSE);
    }
#endif
    if (is_update)
    {
        ...
        kal_uint32 cod)
{
    ...
    U32 temp_value = 0;
    U32 divid_value = 0;
    local_para_struct *btbm_adp_msg = NULL;
    U8 numeric[6];
    msg_type msg = 0;
 ...
 *  void
 *****************************************************************************/
void BTBMAdpReportReadRSSIResult(U8 errCode, S8 rssi_value, U32 lap, U8 uap, U16 nap)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void BTBMAdpGetPairedDeviceInd(kal_uint8 index, btbm_bd_addr_t *addr, char *name_array)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_bm_get_paired_device_ind_struct *btbm_adp_msg;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    btbm_adp_msg =(bt_bm_get_paired_device_ind_struct*) construct_local_para(sizeof(bt_bm_get_paired_device_ind_struct), TD_UL);
    btbm_adp_msg->index = index;
    
    OS_MemCopy((U8 *)&btbm_adp_msg->bd_addr, (U8 *)addr, sizeof(btbm_bd_addr_t));
    OS_MemCopy((U8 *)btbm_adp_msg->name, (U8 *)name_array, BTBM_ADP_MAX_NAME_LEN);
    bma_sendMsg(
        MSG_ID_BT_BM_GET_PAIRED_DEVICE_IND,
        MTKBMAC(direct_rsp_module),
        BT_APP_SAP,
        (local_para_struct*) btbm_adp_msg,
        NULL);
 ...
 *  void
 *****************************************************************************/
void BTBMAdpGetPairedDeviceResult(kal_uint8 result, kal_uint8 no)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_bm_get_paired_device_cnf_struct *btbm_adp_msg;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    btbm_adp_msg =
        (bt_bm_get_paired_device_cnf_struct*)
        construct_local_para(sizeof(bt_bm_get_paired_device_cnf_struct), TD_UL);
    btbm_adp_msg->result = result;
    btbm_adp_msg->total_no = no;
    Assert(no <= BTBM_ADP_MAX_PAIRED_LIST_NO);
    bma_sendMsg(
        MSG_ID_BT_BM_GET_PAIRED_DEVICE_CNF,
        MTKBMAC(direct_rsp_module),
        BT_APP_SAP,
        (local_para_struct*) btbm_adp_msg,
        NULL);
...
void BTBMAdpReadPropertyResult(	btbm_bd_addr_t bd_addr,
                                ...
                                char *name /* Name of device */)
{
    ...
    btbm_adp_msg->bd_addr = bd_addr;
    btbm_adp_msg->cod = cod;
    btbm_adp_msg->powered = powered;
    btbm_adp_msg->discoverable = discoverable;
    btbm_adp_msg->connectable = connectable;
    btbm_adp_msg->pairable = pairable;
    btbm_adp_msg->disctimeout = disctimeout;
    btbm_adp_msg->pairtimeout = pairtimeout;
    btbm_adp_msg->discovering = discovering;
    OS_StrnCpy(btbm_adp_msg->name, name, BTBM_ADP_MAX_NAME_LEN);
...
void BTBMAdpSetSSPMitmResult(kal_uint8 result)
{
    bt_bm_set_ssp_mitm_cnf_struct* btbm_adp_msg;
    btbm_adp_msg = (bt_bm_set_ssp_mitm_cnf_struct*) construct_local_para(sizeof(bt_bm_set_ssp_mitm_cnf_struct), TD_UL);
    btbm_adp_msg->result = 1;
    bma_sendMsg(
        MSG_ID_BT_BM_WRITE_SSP_MITM_MODE_CNF,
        MTKBMAC(direct_rsp_module),
        BT_APP_SAP,
        (local_para_struct*) btbm_adp_msg,
        NULL);
...
void BTBMAdpSetSSPBondingResult(kal_uint8 result)
{
    bt_bm_set_ssp_bonding_cnf_struct* btbm_adp_msg;
    btbm_adp_msg = (bt_bm_set_ssp_bonding_cnf_struct*) construct_local_para(sizeof(bt_bm_set_ssp_bonding_cnf_struct), TD_UL);

    btbm_adp_msg->result = 1;
    bma_sendMsg(
        MSG_ID_BT_BM_WRITE_SSP_BONDING_MODE_CNF,
        MTKBMAC(direct_rsp_module),
        BT_APP_SAP,
        (local_para_struct*) btbm_adp_msg,
        NULL);
...
void BTBMAdpSetSSPIoCapabilitiesResult(kal_uint8 result)
{
    bt_bm_set_ssp_io_capabilities_cnf_struct* btbm_adp_msg;
    btbm_adp_msg = (bt_bm_set_ssp_io_capabilities_cnf_struct*) construct_local_para(sizeof(bt_bm_set_ssp_io_capabilities_cnf_struct), TD_UL);

    btbm_adp_msg->result = 1;
    bma_sendMsg(
        MSG_ID_BT_BM_WRITE_SSP_IO_CAPABILITIES_CNF,
        MTKBMAC(direct_rsp_module),
        BT_APP_SAP,
        (local_para_struct*) btbm_adp_msg,
        NULL);
