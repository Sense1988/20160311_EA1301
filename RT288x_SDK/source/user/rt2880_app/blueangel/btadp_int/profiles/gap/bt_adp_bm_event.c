...
void bm_app_callback(BMCallbackParms *Parms)
{
    ...
    switch (Parms->event)
    {
        ...
        case BTBM_EVENT_WRITE_SCAN_MODE_RESULT:
            ...
            break;
        case BTBM_EVENT_WRITE_BONDABLE_MODE_RESULT:
            BTBMAdpWriteBondabledCfm(Parms->p.result);
            break;
        ...
        case BTBM_EVENT_GET_PAIRED_DEVICE_INDICATION:
            BTBMAdpGetPairedDeviceInd(Parms->p.GetPairedDeviceIndication.index,
                                      &Parms->p.GetPairedDeviceIndication.bd_addr,
                                      Parms->p.GetPairedDeviceIndication.name);
            break;
        case BTBM_EVENT_GET_PAIRED_DEVICE_RESULT:
            BTBMAdpGetPairedDeviceResult(Parms->p.GetPairedDeviceResult.result,
                                         Parms->p.GetPairedDeviceResult.no);
            break;
        case BTBM_EVENT_READ_PROPERTY_RESULT:
            BTBMAdpReadPropertyResult(Parms->p.ReadPropertyResult.bd_addr,
                                      Parms->p.ReadPropertyResult.cod,
                                      Parms->p.ReadPropertyResult.powered,
                                      Parms->p.ReadPropertyResult.discoverable,
                                      Parms->p.ReadPropertyResult.connectable,
                                      Parms->p.ReadPropertyResult.pairable,
                                      Parms->p.ReadPropertyResult.disctimeout,
                                      Parms->p.ReadPropertyResult.pairtimeout,
                                      Parms->p.ReadPropertyResult.discovering,
                                      Parms->p.ReadPropertyResult.name);
            break;
        case BTBM_EVENT_SET_SSP_MITM_RESULT:
            BTBMAdpSetSSPMitmResult(Parms->p.result);
            break;
        case BTBM_EVENT_SET_SSP_BONDING_RESULT:
            BTBMAdpSetSSPBondingResult(Parms->p.result);
            break;
        case BTBM_EVENT_SET_SSP_IO_CAPABILITY_RESULT:
            BTBMAdpSetSSPIoCapabilitiesResult(Parms->p.result);
            break;            
#endif
    }
}
