...
{
    bt_jsr82_enable_service_cnf_struct *ptr;
...
void bt_jsr82_SessionApConnectInd(JSR82CallbackParms *Parms)
{
    ...
    ptr = (bt_jsr82_connect_ind_struct*) construct_local_para(sizeof(bt_jsr82_connect_ind_struct), TD_UL);
    {
        ...
        temp = sizeof(bt_jsr82_connect_ind_struct);
        bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_SESSIONAPCONNECTIND_SIZEx02X, temp);
    }
...
void bt_jsr82_SessionApConnectCfn(JSR82CallbackParms *Parms)
{
    ...
    ptr = (bt_jsr82_connect_cnf_struct*) construct_local_para(sizeof(bt_jsr82_connect_cnf_struct), TD_UL);
    {
        ...
        temp = sizeof(bt_jsr82_connect_cnf_struct);
        bt_trace(BT_TRACE_JSR82_GROUP, BT_JSR82_SESSIONAPCONNECTCFN_SIZEx02X, temp);
    }
...
void bt_jsr82_SessionApRxInd(JSR82CallbackParms *Parms)
{
     ...
     * The bt_jsr82_rx_data_ind_struct should not include a pointer but a real array 
     *   because different task may have different memory space in other OS.
    */
    ptr = (bt_jsr82_rx_data_ind_struct*) construct_local_para(sizeof(bt_jsr82_rx_data_ind_struct), TD_UL);
    ptr->index = Parms->p.rx_data_ind.con_id;
    ptr->length = Parms->p.rx_data_ind.length;
    ptr->data = Parms->p.rx_data_ind.data;
    //btmtk_os_memcpy();
    ptr->l2cap_id = Parms->p.rx_data_ind.l2cap_id;
    JSR82_sendMsg(MSG_ID_BT_JSR82_RX_DATA_IND, MOD_MMI, (local_para_struct*) ptr, NULL);
    return;
}
...
void bt_jsr82_SessionApRxReadyInd(JSR82CallbackParms *Parms)
{
    ...
    ptr = (bt_jsr82_rx_ready_ind_struct*) construct_local_para(sizeof(bt_jsr82_rx_ready_ind_struct), TD_UL);
    ptr->index = Parms->p.rx_ready_ind.session_inx;
    ptr->length = Parms->p.rx_ready_ind.length;
    ptr->l2cap_id = Parms->p.rx_ready_ind.l2cap_id;
    ptr->ps_type = Parms->p.rx_ready_ind.ps_type;
    JSR82_sendMsg(MSG_ID_BT_JSR82_RX_READY_IND, MOD_MMI, (local_para_struct*) ptr, NULL);
}
...
void bt_jsr82_SessionApTxReadyInd(JSR82CallbackParms *Parms)
{
    ...
    ptr = (bt_jsr82_tx_ready_ind_struct*) construct_local_para(sizeof(bt_jsr82_tx_ready_ind_struct), TD_UL);
    ...
    JSR82_sendMsg(MSG_ID_BT_JSR82_TX_READY_IND, MOD_MMI, (local_para_struct*) ptr, NULL);
}
...
{
    bt_jsr82_data_available_ind_struct *ptr;

    ptr =
        (bt_jsr82_data_available_ind_struct*) construct_local_para(
                                                    sizeof(bt_jsr82_data_available_ind_struct),
                                                    TD_UL);
    ptr->index = Parms->p.data_available_ind.index;
    ptr->ps_type = Parms->p.data_available_ind.ps_type;
    ptr->l2cap_id = Parms->p.data_available_ind.l2cap_id;
    ptr->length = Parms->p.data_available_ind.length;
#ifndef BTMTK_ON_LINUX
    ptr->buffAddr = Parms->p.data_available_ind.buffer;
#endif
    JSR82_sendMsg(MSG_ID_BT_JSR82_DATA_AVAILABLE_IND, MOD_MMI, (local_para_struct*) ptr, NULL);
}
