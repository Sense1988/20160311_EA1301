...
void btmtk_adp_oppc_start_ashm_service()
{
	// start the ashm server(android service) with given name and size
#ifdef __BT_GOEPC_AUTO_ACK__
    btmtk_ashm_start_service(OPPC_ASHM_NAME, OPPC_MAX_OBEX_PACKET_LENGTH*2);
#else
	btmtk_ashm_start_service(OPPC_ASHM_NAME, OPPC_MAX_OBEX_PACKET_LENGTH);
#endif
...
// will be called at profile init
void btmtk_adp_opps_start_ashm_service()
{
	// start the ashm server(android service) with given name and size
#ifdef __BT_GOEPS_AUTO_ACK__
    btmtk_ashm_start_service(OPPS_ASHM_NAME, OPPS_MAX_OBEX_PACKET_LENGTH*2);
#else
	btmtk_ashm_start_service(OPPS_ASHM_NAME, OPPS_MAX_OBEX_PACKET_LENGTH);
#endif
