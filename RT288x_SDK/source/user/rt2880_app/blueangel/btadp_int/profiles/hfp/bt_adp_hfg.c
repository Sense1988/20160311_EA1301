...

{
...

	{
		...
		OS_Report("Runout of response queue\r\n");
...

{
...

	{
...

		{
			...
			hfg_deregister(&pChannel->channel);

			pChannel = (HfgAdpChannel*)GetHeadList(&pChannel->node);
...

		{
		    ...
		    status = hfg_register(&pChannel->channel, hfg_app_callback, pChannel->bHeadset);

			Assert(BT_STATUS_SUCCESS == status);
...

{
...

    {
		...
		adp_channel->bHeadset = msg_p->bHeadset;

            //OS_MemSet((U8*)adp_channel->channel, 0, sizeof(HfgChannel));
		   ...
		   adp_channel->timer.context = (void*)adp_channel;
                ...
                //bt_setProfileAddress(PROFILE_HFG, &msg_p->addr, msg_p->addrlen);
...

{
    ...
    //bt_removeProfileAddress(PROFILE_HFG);
...

{
...

    {
	 ...
	 EVM_StartTimer(&adp_channel->timer, HFG_CONNECT_TIMEOUT);
...

{
...

    {
	 ...
	 bt_prompt_trace(MOD_BT, "[HFG][Info] HFG connection accept timer started ");

         */

	 EVM_StartTimer(&adp_channel->timer, 8000);
...
void hfg_adp_connect_req_rsp_hdler(ilm_struct *ilm_ptr)

{

    BtStatus status = BT_STATUS_NOT_FOUND;

    bt_hfg_connect_req_res_struct *msg_p;

    bt_hfg_disconnected_ind_struct *cnf_msg_p;
    ...
    msg_p = (bt_hfg_connect_req_res_struct*)ilm_ptr->local_para_ptr;
    ...
    cnf_msg_p = (bt_hfg_connect_cnf_struct*) construct_local_para(sizeof(bt_hfg_connect_cnf_struct), TD_UL);
...

    {
        ...
        status = hfg_create_service_link(adp_channel->channel, &msg_p->bt_addr);
...
void hfg_adp_send_result_req_hdler(ilm_struct *ilm_ptr)

{

    BtStatus status = BT_STATUS_NOT_FOUND;

    bt_hfg_send_result_req_struct *msg_p;
    ...
    msg_p = (bt_hfg_send_result_req_struct*)ilm_ptr->local_para_ptr;
...

    {
        ...
        len = msg_p->parmLen + msg_p->data_len;

        if(len != 0)

        {

            ptr = get_ctrl_buffer(len);

            if(ptr != NULL)

            {

                OS_MemCopy((U8*)ptr, (const U8*)msg_p->, len+1);

                msg_p->call.number = ptr;

                response = hfg_alloc_rsp(adp_channel);

                if(response != NULL)

                    status = hfg_send_call_list_rsp(adp_channel->channel, &msg_p->call, response, msg_p->bFinal);
 ...
 *  hfg_adp_supported_indicator_req_hdler
...
void hfg_adp_supported_indicator_req_hdler(ilm_struct *ilm_ptr)

{

    BtStatus status = BT_STATUS_NOT_FOUND;

    bt_hfg_supported_indicator_req_struct *msg_p;
...

    {
        ...
        if(response != NULL)
            ...
            status = hfg_send_ring(adp_channel, response);
...

    {
...

        {
            ...
            hfg_send_channel_msg(adp_channel, MSG_ID_BT_HFG_RESULT_COMPLETE_IND, (local_para_struct*)cnf_msg_p);        
...

        {
            ...
            hfg_sendMsg(MSG_ID_BT_HFG_RESULT_COMPLETE_IND, ilm_ptr->src_mod_id, BT_APP_SAP, (local_para_struct*)cnf_msg_p, NULL);
...

    {
...

        {
...

            {
                ...
                //msg_p->subscriber.number = number;
...

{
...

    {
...

        {
            ...
            DBG_PRINT_AT(("[HFG][AT] transmit at : %s", msg_p->atresult));
...

{
...

    {
		...
		case HFG_SMS_TYPE_CBM:
			...
			//list.msg.cbm.length = msg_p->msg.cbm.length;
...

{
...

    {
		...
		case HFG_SMS_TYPE_CBM:
			...
			//readMsg.msg.cbm.length = msg_p->msg.cbm.length;
...

{
...

	{
		...
		adp_channel->channel.state = HFG_SUBSTATE_SLC1;
...

{
...

    {
        ...
        case MSG_ID_BT_HFG_CONNECT_REQ_RES:

            hfg_adp_connect_ind_rsp_hdler(ilm_ptr);
        ...
        case MSG_ID_BT_HFG_AT_RESULT_REQ:
        ...
        case MSG_ID_BT_HFG_OK_REQ:                                       /* OK */

            hfg_adp_ok_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_ERROR_REQ:                                    /* ERROR or +CME ERROR : <err> */

            hfg_adp_error_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_NO_CARRIER_REQ:                           /* NO CARRIER */

            hfg_adp_no_carrier_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_BUSY_REQ:                                     /* BUSY */

            hfg_adp_busy_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_NO_ANSWER_REQ:                            /* NO ANSWER */

            hfg_adp_no_answer_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_DELAYED_REQ:                                  /* DELAYED */

            hfg_adp_delayed_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_BLACKLISTED_REQ:                          /* BLACKLISTED */

            hfg_adp_blacklisted_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_RING_REQ:                                     /* RING */

            hfg_adp_ring_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_CALL_WAITING_REQ:                     /* +CCWA:<number>,<type> */

            hfg_adp_call_waiting_req_hdler(ilm_ptr);

            break;

        #if 0 /* Compiled time determined and response by HFG */

        case MSG_ID_BT_HFG_SUPPORTED_INDICATOR_REQ:        /* +CIND: (<descr>,(list of supported<ind>s)) [,(<descr>,(list of supported<ind>s))[,...]] */

            hfg_adp_supported_indicator_req_hdler(ilm_ptr);

            break;

        #endif

        case MSG_ID_BT_HFG_READ_INDICATOR_REQ:                   /* +CIND: <ind>[,<ind>[,...]] */

            hfg_adp_read_indicator_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_CALL_LIST_REQ:                             /* [+CLCC: <id1>,<dir>,<stat>,<mode>,<mpty>[,<number>,<type>] */

            hfg_adp_call_list_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_CALL_ID_REQ:                                /* +CLIP:<number>,<type> */

            hfg_adp_call_id_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_INDICATOR_REPORT_REQ:             /* +CIEV: <ind>,<value> */

            hfg_adp_indicator_report_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_SUBSCRIBER_NUMBER_REQ:           /* +CNUM: [<alpha>],<number>, <type>,[<speed>] ,<service> */
        ...
        case MSG_ID_BT_HFG_VOICE_TAG_REQ:                           /* +BINP: <Phone number> */

            hfg_adp_voice_tag_req_hdler(ilm_ptr);

            break;

        case MSG_ID_BT_HFG_VOICE_REC_REQ:                            /* +BVRA: <vrect> */
        ...
        case MSG_ID_BT_HFG_RESPONSE_HOLD_REQ:                     /* +BTRH: <n> */
