...
void hfg_app_callback(HfgChannel *Channel, HfgCallbackParms *Parms)
{
    ...
    {
        bt_hfg_callback_ind_struct *ind_msg_p;
        ind_msg_p = (bt_hfg_callback_ind_struct*) construct_local_para(sizeof(bt_hfg_callback_ind_struct), TD_UL);
        ind_msg_p->pContext = (void*)adp_channel;
        hfg_send_channel_msg(adp_channel, MSG_ID_BT_HFG_CALLBACK_IND, (local_para_struct*)ind_msg_p);
        break;
    }
#else    
    case HFG_EVENT_SERVICE_CONNECT_REQ:
    ...
    case HFG_EVENT_AUTH_REQ:
    {
        ...
        ind_msg_p->pContext = (void*)adp_channel;
        ind_msg_p->user_context = adp_channel->userContext;
        //OS_MemCopy( (U8*)&ind_msg_p->bt_addr, (const U8*)&Parms->p.remDev->bdAddr, sizeof(BD_ADDR));
    ...
    case HFG_EVENT_CALL_HOLD:
    {
		...
		/*
        ind_msg_p->action = Parms->p.hold->action;
        ind_msg_p->index = Parms->p.hold->index;
		*/
    ...
    case HFG_EVENT_AT_COMMAND_DATA:
    {
	 ...
	 ind_msg_p->datasize = len;
        ...
        DBG_PRINT_AT(("[HFG][AT] recv at : %s", ind_msg_p->atcmd));
	...
	***************************************************************************/
    ...
    case HFG_EVENT_RESPONSE_COMPLETE:
    {
        ...
        //if (!IsNodeOnList(&adp_channel->rsp_queue, &Parms->p.response->node)) 
		if (!IsNodeOnList(&adp_channel->rsp_queue, &response->node)) 
        {
            ...
            //RemoveEntryList(&Parms->p.response->node);
            //switch(Parms->p.response->type)
