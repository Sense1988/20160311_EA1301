...
extern "C" {
    ...
    void btmtk_adp_ftpc_start_ashm_service() {
		char ashm[] = FTPC_ASHM_NAME;
		// start the ashm server(android service) with given name and size
		btmtk_ashm_start_service(ashm, FTPC_MEM_BUFF_SIZE);
    ...
    // will be called at profile init
    void btmtk_adp_ftps_start_ashm_service() {
		char ashm[] = FTPS_ASHM_NAME;
		// start the ashm server(android service) with given name and size
		btmtk_ashm_start_service(ashm, FTPS_MEM_BUFF_SIZE);
