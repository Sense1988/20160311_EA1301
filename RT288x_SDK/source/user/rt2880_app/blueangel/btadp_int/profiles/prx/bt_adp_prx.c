...
PRXMInternalMapping g_prxmInternal[PRX_MAX_MONITOR_NUM] = {0};
...
void send_prxm_disconnect_ind(U8 index, U8 rspcode) {
    ...
    freePRXMInternalIndex(ind->index);
...
void prxm_connect_req_hndl(ilm_struct *ilm_ptr) {
	...
	g_prxmapp[internalIndex].id = req->index;
...
void prxm_disconnect_req_hndl(ilm_struct *ilm_ptr) {
	...
	if (internalIndex >= PRX_MAX_MONITOR_NUM) {
		send_prxm_disconnect_ind(req->index, BT_STATUS_NOT_FOUND);
		return;
	}
