...
static void xml_free_buffer(XML_PARSER_STRUCT *parser, void *p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ( p == NULL ) return;    //klocwork warning
...
//    FHANDLE fh;
	...
	if( FALSE != btmtk_fs_is_dir_exist_ucs2((const U16*)CONST_XML_FOLDER) )
    {
    ...
    else
    {
	    ...
	    ret = btmtk_fs_create_dir_ucs2((const U16*)CONST_XML_FOLDER );
...
static kal_int32 xml_process_doctype(XML_PARSER_STRUCT *parser, kal_int32 start, kal_int32 length)
{
    ...
    if (name != NULL)
    {
        ...
        if (parser->doct_name == NULL)
            return XML_RESULT_OUT_OF_MEMORY;
        strncpy(parser->doct_name, name, strlen(name));
...
static kal_int32 xml_process_data(XML_PARSER_STRUCT *parser, kal_int32 length)
{
    ...
    btmtk_fs_close_ucs2(handle);    //klocwork warning.
    return XML_RESULT_OK;
...
static kal_int32 xml_process_entity(XML_PARSER_STRUCT *parser, kal_char *start, kal_int32 length)
{
    ...
    if (name != NULL)
    {
        ...
        while ((ptr1 = xml_find_min_entity_mark(ptr1, value + strlen(value))) != NULL)
        {
            ...
            memset(rest, 0, strlen(ptr2) + 1);
            strncpy(rest, ptr2, strlen(ptr2));
            
            strncat(value, tmp, strlen(tmp));
            strncat(value, rest, strlen(rest));
        ...
        if (value != NULL)
        {
            ...
            {
                free_buffer(parser, entity_name);
                free_buffer(parser, value);
                return XML_RESULT_OUT_OF_MEMORY;
            }
            strncpy(entity_name, name, strlen(name));
            strncpy(entity_value, value, strlen(value));
...
static kal_int32 xml_filter_comment_pi(kal_char *data, const kal_char *filter_start, const kal_char *filter_end)
{
    ...
    while (len > 0)
    {
        ...
        if (ptr1 != NULL)
        {
            ...
            if (ptr2 != NULL)
            {
                strncpy(ptr1, ptr2 + temp, strlen(ptr2 + temp));
                                    ...
                                    kal_char *end)
{
    ...
    else
    {
        ...
        {
            strncat(data, entity_value, strlen(entity_value));
        }
        else
                                ...
                                kal_int32 length)
{
    ...
    strncat(dest, original, strlen(original));
    ...
    while ((start = xml_find_min_entity_mark(start, dest + strlen(dest))) != NULL)
    {
        ...
        memset(rest, 0, strlen(end)+1);
        strncpy(rest, end, strlen(end));

        strncat(dest, tmp, strlen(tmp));
        strncat(dest, rest, strlen(rest));
        free_buffer(parser, rest);
        tmp[0] = 0;
                ...
                kal_int32 len_data)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint32 len_dest=0;
    int len;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ( dest == NULL ) //Klocwork warning.
        return;
    ...
    else
    {
        ...
        {
            *handle = btmtk_fs_open_ucs2((const U16*)L"d:\\temp\\ftp\\xml_temp", BTMTK_FS_READ_WRITE | BTMTK_FS_CREATE_ALWAYS);
...
static void xml_set_error(XML_PARSER_STRUCT *parser, kal_int32 error, kal_char *err_str)
{
    ...
    if ((parser->err_str = get_buffer(parser, strlen(err_str) + 1)) != NULL)
        strncpy(parser->err_str, err_str, strlen(err_str));
    parser->error = error;
...
static kal_char *xml_replace_entity(XML_PARSER_STRUCT *parser, kal_char *start, kal_int32 length)
{
    ...
    if ( target == NULL )   // Klocwork warning.
        return NULL;
...
static kal_int32 xml_push_start_tag(XML_PARSER_STRUCT *parser, kal_char *tagname)
{
    ...
    if (namebuffer == NULL)
        return XML_RESULT_OUT_OF_MEMORY;
    strncpy(namebuffer, tagname, strlen(tagname));
