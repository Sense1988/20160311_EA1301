...
{
#ifdef ANDROID
    LOGI("[DUMP] tst_buffer=%p, TST_RB_SIZE=0x%X, TST_RB_SIZE_MASK=0x%X", tst_buffer, TST_RB_SIZE, TST_RB_SIZE_MASK);
    LOGI("[DUMP] tst_cur_pos=%d, tst_last_pos=%d, tst_write_pos=%d", tst_cur_pos, tst_last_pos, tst_write_pos);
    LOGI("[DUMP] tst_lost_trace=%d, tst_lost_promitive=%d", tst_lost_trace, tst_lost_primitive);
    LOGI("[DUMP] wakeup_fun=%p, send_fun=%p", wakeup_fun, send_fun);
    LOGI("[DUMP] g_tstWaitFlag=%d", g_tstWaitFlag);
    //LOGI("[DUMP] g_tstSendFlag=%d", g_tstSendFlag);
    LOGI("[DUMP] g_nMaxSize=%d, g_nMinSize=%d", g_nMaxSize, g_nMinSize);
    LOGI("[DUMP] g_tstStarted=%d", g_tstStarted);
#endif
}
