...
/* HF & HS TRACE START */
    TRC_MSG(BT_HF_API_INIT, "[HF][API] HF_Init") 
    TRC_MSG(BT_HF_API_REGISTER, "[HF][API] HF_Register(pChannel=0x%08X, callback=0x%08X, bHeadset=%d)")  
    TRC_MSG(BT_HF_API_DEREGISTER, "[HF][API] HF_Deregister(pChannel=0x%08X)")    
    TRC_MSG(BT_HF_API_CREATE_SERVICE_LINK, "[HF][API] HF_CreateServiceLink(pChannel=0x%08X, Addr=0x%02X%02X%02X%02X%02X%02X)")
    TRC_MSG(BT_HF_API_DISCONNECT_SERVICE_LINK, "[HF][API] HF_DisconnectServiceLink(pChannel=0x%08X) : state=0x%02X")     
    TRC_MSG(BT_HF_API_CREATE_AUDIO_LINK, "[HF][API] HF_CreateAudioLink(pChannel=0x%08X) : state=0x%02X, IsAudioUp=%d")   
    TRC_MSG(BT_HF_API_DISCONNECT_AUDIO_LINK, "[HF][API] HF_DisconnectAudioLink(pChannel=0x%08X) : state=0x%02X, IsAudioUp=%d")    
    TRC_MSG(BT_HF_API_ACCEPT_CONNECT, "[HF][API] HF_AcceptConnect(pChannel=0x%08X)")    
    TRC_MSG(BT_HF_API_REJECT_CONNECT, "[HF][API] HF_RejectConnect(pChannel=0x%08X)")    
    TRC_MSG(BT_HF_API_SEND_DATA, "[HF][API] HF_SendData(pChannel=0x%08X, pData=0x%08X, len=0x%04X)")
    TRC_MSG(BT_HF_API_INIT_RET, "[HF][API] HF_Init : return TRUE") 
    TRC_MSG(BT_HF_API_REGISTER_RET, "[HF][API] HF_Register : return status=%d") 
    TRC_MSG(BT_HF_API_DEREGISTER_RET, "[HF][API] HF_Deregister : return status=%d") 
    TRC_MSG(BT_HF_API_CREATE_SERVICE_LINK_RET, "[HF][API] HF_CreateServiceLink : return status=%d")
    TRC_MSG(BT_HF_API_DISCONNECT_SERVICE_LINK_RET, "[HF][API] HF_DisconnectServiceLink : return status=%d")
    TRC_MSG(BT_HF_API_CREATE_AUDIO_LINK_RET, "[HF][API] HF_CreateAudioLink : return status=%d")
    TRC_MSG(BT_HF_API_DISCONNECT_AUDIO_LINK_RET, "[HF][API] HF_DisconnectAudioLink : return status=%d")    
    TRC_MSG(BT_HF_API_ACCEPT_CONNECT_RET, "[HF][API] HF_AcceptConnect : return status=%d") 
    TRC_MSG(BT_HF_API_REJECT_CONNECT_RET, "[HF][API] HF_RejectConnect : return status=%d")  
    TRC_MSG(BT_HF_API_SEND_DATA_RET, "[HF][API] HF_SendData : return status=%d")      
    TRC_MSG(BT_HF_CBK_SDP_QUERY, "[HF][CBK] hfSdpQueryCallback(sqt=0x%08X, result=%d, attribute_index=%d, attribute_value=0x%08X)")
    TRC_MSG(BT_HF_CBK_CMGR_CALLBACK, "[HF][CBK] HfCmgrCallback : pChannel=0x%08X, event=%d, status=%d")
    TRC_MSG(BT_HF_CBK_RF_CALLBACK, "[HF][CBK] HfRfCallback : pChannel=0x%08X, event=%d, status=%d")
    TRC_MSG(BT_HF_EVENT_SDP_PROTOCOL, "[HF][EVENT] SDP callback : AID_PROTOCOL_DESC_LIST, server channel=0x%02X")
    TRC_MSG(BT_HF_EVENT_SDP_PROFILE, "[HF][EVENT] SDP callback : AID_BT_PROFILE_DESC_LIST, version=0x%04X")
    TRC_MSG(BT_HF_EVENT_SDP_NETWORK, "[HF][EVENT] SDP callback : AID_EXTERNAL_NETWORK, AG network=0x%02X")
    TRC_MSG(BT_HF_EVENT_SDP_FEATURES, "[HF][EVENT] SDP callback : AID_SUPPORTED_FEATURES, AG features=0x%04X")
    TRC_MSG(BT_HF_EVENT_SDP_CONNECTION_FAILED, "[HF][EVENT] SDP callback : BT_STATUS_CONNECTION_FAILED")
    TRC_MSG(BT_HF_EVENT_SDP_FAILED, "[HF][EVENT] SDP callback : BT_STATUS_FAILED")
    TRC_MSG(BT_HF_EVENT_SDP_NOSERVICES, "[HF][EVENT] SDP callback : BT_STATUS_NOSERVICES")
    TRC_MSG(BT_HF_EVENT_SDP_UNKNOWN_RESULT, "[HF][EVENT] SDP callback : unknown result : %d")
    TRC_MSG(BT_HF_EVENT_CMGR_DATA_LINK_CON_CNF, "[HF][EVENT] CMGR callback : CMEVENT_DATA_LINK_CON_CNF, status=%d")
    TRC_MSG(BT_HF_EVENT_CMGR_DATA_LINK_DIS, "[HF][EVENT] CMGR callback : CMEVENT_DATA_LINK_DIS, status=%dd")
    TRC_MSG(BT_HF_EVENT_CMGR_AUDIO_LINK_CON, "[HF][EVENT] CMGR callback : CMEVENT_AUDIO_LINK_CON, status=%d")
    TRC_MSG(BT_HF_EVENT_CMGR_AUDIO_LINK_DIS, "[HF][EVENT] CMGR callback : CMEVENT_AUDIO_LINK_DIS, status=%d") 
    TRC_MSG(BT_HF_EVENT_RF_OPEN_IND, "[HF][EVENT] RFCOMM callback : RFEVENT_OPEN_IND")
    TRC_MSG(BT_HF_EVENT_RF_OPEN, "[HF][EVENT] RFCOMM callback : RFEVENT_OPEN")
    TRC_MSG(BT_HF_EVENT_RF_CLOSED, "[HF][EVENT] RFCOMM callback : RFEVENT_CLOSED")
    TRC_MSG(BT_HF_EVENT_RF_DATA_IND, "[HF][EVENT] RFCOMM callback : RFEVENT_DATA_IND")    
    TRC_MSG(BT_HF_EVENT_RF_PACKET_HANDLED, "[HF][EVENT] RFCOMM callback : RFEVENT_PACKET_HANDLED")   
    TRC_MSG(BT_HF_INFO_SDP_QUERY_COMPLETE, "[HF][INFO] SDP query complete. Channel no=%d")
    TRC_MSG(BT_HF_INFO_REGISTER_SUCCESS, "[HF][INFO] HF_Register success : pChannel=0x%08X")
    TRC_MSG(BT_HF_INFO_REGISTER_SDP, "[HF][INFO] HfSdpRegisterService(bHeadset=%d)")
    TRC_MSG(BT_HF_INFO_DEREGISTER_SDP, "[HF][INFO] HfSdpDeregisterService(bHeadset=%d)")
    TRC_MSG(BT_HF_INFO_ALLOC_RF_CHANNEL, "[HF][INFO] HfAllocRfChannel()")
    TRC_MSG(BT_HF_INFO_FREE_RF_CHANNEL, "[HF][INFO] HfFreeRfChannel(pRfChannel=0x%08X)")    
    TRC_MSG(BT_HF_INFO_REGISTER_SERVER_CHANNEL, "[HF][INFO] HfRegisterRfServerChannel(pChannel=0x%08X)")
    TRC_MSG(BT_HF_INFO_DEREGISTER_SERVER_CHANNEL, "[HF][INFO] HfDeregisterRfServerChannel(pChannel=0x%08X)")
    TRC_MSG(BT_HF_INFO_OPEN_ACL_CHANNEL, "[HF][INFO] HfOpenCmgrChannel(pChannel=0x%08X, Addr=0x%02X%02X%02X%02X%02X%02X)")
    TRC_MSG(BT_HF_INFO_CLOSE_ACL_CHANNEL, "[HF][INFO] HfCloseCmgrChannel(pChannel=0x%08X)")
    TRC_MSG(BT_HF_INFO_START_SERVICE_QUERY, "[HF][INFO] HfSdpStartQuery(pChannel=0x%08X, mode=%d) : bHeadset=%d")
    TRC_MSG(BT_HF_INFO_AUDIO_EXIST, "[HF][INFO] Audio connection already exists")
    TRC_MSG(BT_HF_INFO_NO_AUDIO_CONNECTION, "[HF][INFO] No audio connection exist")
    TRC_MSG(BT_HF_INFO_AUDIO_CONENCTING, "[HF][INFO] Connecting audio is on going")
    TRC_MSG(BT_HF_INFO_AUDIO_DISCONENCTING, "[HF][INFO] Disconnecting audio is on going")
    TRC_MSG(BT_HF_INFO_CALLBACK_TO_ADP, "[HF][INFO] HfAppCallback(pChannel=0x%08X, event=%d, status=%d, data=0x%08X)")
    TRC_MSG(BT_HF_INFO_CHANNEL_STATE, "[HF][INFO] state=0x%02X, linkFlags=0x%02X, queryFlags=0x%02X")
    TRC_MSG(BT_HF_INFO_SUSPEND_CONTEXT, "[HF][INFO] suspend context 0x%08X")
    TRC_MSG(BT_HF_INFO_RESUME_CONTEXT, "[HF][INFO] resume context 0x%08X")  
    TRC_MSG(BT_HF_INFO_DEFER_DEREGISTERATION, "[HF][INFO] State is not closed, defer deregisteration : pChannel=0x%08X, state=%02X")    
    TRC_MSG(BT_HF_FLOW_AUDIO_CONNECTING, "[HF][FLOW] Create Audio channel : %d")  
    TRC_MSG(BT_HF_FLOW_AUDIO_DISCONNECTING, "[HF][FLOW] Close Audio channel : %d")   
    TRC_MSG(BT_HF_FLOW_SDP_ADD_RECORD, "[HF][FLOW] SDP add record : %d")   
    TRC_MSG(BT_HF_FLOW_SDP_REMOVE_RECORD, "[HF][FLOW] SDP remove record : %d")      
    TRC_MSG(BT_HF_FLOW_SDP_SERVICE_SEARCH, "[HF][FLOW] SDP service search : %d")  
    TRC_MSG(BT_HF_FLOW_RF_REGISTERED, "[HF][FLOW] Server channel registered : %d")     
    TRC_MSG(BT_HF_FLOW_RF_REGISTERING, "[HF][FLOW] Register server channel : %d")  
    TRC_MSG(BT_HF_FLOW_RF_DEREGISTERING, "[HF][FLOW] Deregister server channel : %d") 
    TRC_MSG(BT_HF_FLOW_RF_CONNECTING, "[HF][FLOW] Open RFCOMM channel : %d")  
    TRC_MSG(BT_HF_FLOW_RF_DISCONNECTING, "[HF][FLOW] Close RFCOMM channel : %d")  
    TRC_MSG(BT_HF_FLOW_ACL_ASSOCIATED, "[HF][FLOW] ACL associated : %d")    
    TRC_MSG(BT_HF_FLOW_ACL_ASSOCIATING, "[HF][FLOW] Associate ACL link : %d")        
    TRC_MSG(BT_HF_FLOW_ACL_DISASSOCIATING, "[HF][FLOW] Disassociate ACL link : %d")  
    TRC_MSG(BT_HF_FLOW_SCO_REGISTERING, "[HF][FLOW] Register SCO handler : %d")      
    TRC_MSG(BT_HF_FLOW_SCO_DEREGISTERING, "[HF][FLOW] Deregister SCO handler : %d")   
    TRC_MSG(BT_HF_FLOW_CHANGE_STATE, "[HF][FLOW] Change state from 0x%02X to 0x%02X")     
    TRC_MSG(BT_HF_AT_BYTES_TO_SEND, "[HF][AT] Bytestosend=0x%04X")        
    TRC_MSG(BT_HF_WARN, "[HF][WARN] file=%d, line=%d") 
    TRC_MSG(BT_HF_WARN_1, "[HF][WARN] file=%d, line=%d, p1=%d") 
    TRC_MSG(BT_HF_WARN_CHANNEL_SUSPENDED, "[HF][WARN] Channel is suspended : pChannel=0x%08X")
    TRC_MSG(BT_HF_WARN_CHANNEL_NOT_REGISTERED, "[HF][WARN] Channel not registered : pChannel=0x%08X")
    TRC_MSG(BT_HF_WARN_CHANNEL_ALREADY_REGISTERED, "[HF][WARN] Channel already registered : pChannel=0x%08X")
    TRC_MSG(BT_HF_WARN_NO_RESOURCES, "[HF][WARN] No resources : pChannel=0x%08X")
    TRC_MSG(BT_HF_WARN_NO_CONNECTION, "[HF][WARN] No connection : pChannel=0x%08X")
    TRC_MSG(BT_HF_WARN_NO_INCOMING_CONNECTION, "[HF][WARN] No incoming connection : pChannel=0x%08X")    
    TRC_MSG(BT_HF_WARN_ACCEPT_CHANNEL_FAIL, "[HF][WARN] RF_AcceptChannel failed : status=%d")    
    TRC_MSG(BT_HF_WARN_REJECT_CHANNEL_FAIL, "[HF][WARN] RF_RejectChannel failed : status=%d") 
    TRC_MSG(BT_HF_WARN_SEND_RF_PACKET_FAILED, "[HF][WARN] RF_SendData failed : status=%d")   
    TRC_MSG(BT_HF_WARN_UNRECOGNIZED_SDP_ATTR, "[HF][WARN] Unrecognized SDP attribute : %d")
    TRC_MSG(BT_HF_ERROR_UNEXPECTED_STATE, "[HF][ERR] Unexpected state : file=%d, line=%d, pChannel=0x%08X, state=0x%02X")
    TRC_MSG(BT_HF_ERROR, "[HF][ERR] file=%d, line=%d") 
    TRC_MSG(BT_HF_ERROR_1, "[HF][ERR] file=%d, line=%d, p1=%d")                                                          
    TRC_MSG(BT_HF_ADP_DEFERED_DEREGISTERATION, "[HF][ADP] Defered deregistration pChannel=0x%08X")
    TRC_MSG(BT_HF_ADP_CONNECT_TIMER_STARTED, "[HF][ADP] HF connection timeout started")      
    TRC_MSG(BT_HF_ADP_CONNECT_TIMER_TRIGGERED, "[HF][ADP] HF connection timeout triggered")    
    TRC_MSG(BT_HF_ADP_CONNECT_TIMER_CANCELED, "[HF][ADP] HF connection timer canceled") 
    TRC_MSG(BT_HF_ADP_SEND_AT_CMD, "[HF][ADP] Send AT cmd : %s")     
    TRC_MSG(BT_HF_ADP_RECV_AT_RESULT_CODE, "[HF][ADP] Receive AT result code : %s")    
    TRC_MSG(BT_HF_ADP_WARN_CHANNEL_NOT_FOUND, "[HF][ADP][WARN] Channel not found : pAdpChannel=0x%08X")     
    TRC_MSG(BT_HF_ADP_UNSUPPORTED_EVENT, "[HF][ADP] hfa_app_callback: Unsupported event")       
/* HF & HS TRACE END */
...
/* GAVDP TRACE START */
    ...
    TRC_MSG( BT_GAVDP_AVDTP_CBK_PACKET_SENT, "[GAVDP][GavdpAvdtpEnventCallback][GVDTP_EVENT_STREAM_PACKET_SENT] packet sent completed")
    TRC_MSG( BT_GAVDP_AVDTP_CBK_PACKET_RECV, "[GAVDP][GavdpAvdtpEnventCallback][GVDTP_EVENT_STREAM_PACKET_RECV] packet recv completed")
