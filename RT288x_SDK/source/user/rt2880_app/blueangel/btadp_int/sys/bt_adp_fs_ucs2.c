...
FHANDLE btmtk_fs_open_ucs2(const U16 *FileName, btmtk_file_mode_enum Flag)
{
    ...
    OS_Report("btmtk_fs_open_ucs2: flag[%d]", Flag);
   	/*
	if (Flag & BTMTK_FS_READ_ONLY)
   	...
   	else
   	{
		...
		Flag |= BTMTK_FS_CREATE_ALWAYS;
   		Flag |= BTMTK_FS_READ_WRITE;
   	}*/

   	if((fh = open((const char*)buffer, translateFileMode(Flag), /*S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH*/0666)) == -1)
   	{
...
//20101004 mtk03036 end
        {
            ...
            if ( fd_src != -1 )    // Klockwork warning.
                close(fd_src);
            if ( fd_dst != -1 )    // Klockwork warning.
                close(fd_dst);
        }
    }
    if(l_ret != 0)
...
FS_STATUS btmtk_fs_delete_dir_ucs2(const U16 *pwsFilename){
	...
	if( FALSE == RemoveDirectoryW( pwsFilename ) ){
		OS_Report("[fs] delete folder '%s' fail=%d\n", buffer, GetLastError());
		return GetLastError();
	}
...
BT_BOOL btmtk_fs_is_file_exist_ucs2(const U16 *pwsFilename){
    ...
    OS_Report("[fs] is fileexit '%s' ret=%d\n", buffer, l_ret);
    return l_ret;
#endif
...
BT_BOOL btmtk_fs_is_dir_exist_ucs2(const U16 *pwsFilename){
    ...
    if (!fOk){
		l_ret = FALSE;
		OS_Report("[fs] is folder exit check fail l_ret=%d !\n", GetLastError());
		return FALSE;
   	}else{
	...
	OS_Report("[fs] is folder exit '%s' l_ret=%d\n", buffer, l_ret );
	return l_ret;
#endif
    ...
    if (!dir) {
	OS_Report("[fs] is folder exit check fail!\n");
	return FALSE;
    }
    OS_Report("[fs] is folder exit '%s'", buffer);
    closedir(dir);
    return TRUE;
#endif
}
