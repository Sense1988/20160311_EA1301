...
S32 btmtk_fs_open(const U8 *fileName, U32 mode)
{
    ...
    {
        bt_file_desc[i].handle = (int)(uintptr_t)fp;
        bt_file_desc[i].used = KAL_TRUE;
...
#else
    if (fclose((FILE*)(uintptr_t)handle) != 0)
    {
...
S32 btmtk_fs_read(int fd, U8 *buffer, U32 size)
{
    ...
    return translateFileError(err);
#else
    byte_read = fread(buffer, 1, size, (FILE *)(uintptr_t)handle);
    if (byte_read == size || feof((FILE *)(uintptr_t)handle))
    {
...
S32 btmtk_fs_write(int fd, U8 *buffer, U32 size)
{
    ...
    bytes_written = fwrite(buffer, 1, size, (FILE *)(uintptr_t)handle);
...
#else
    if (fseek((FILE *)(uintptr_t)handle, offset, translateSeekMode(seek_mode)) == 0)
    {
        pos = ftell((FILE *)(uintptr_t)handle);
        return pos;
    }
    return BTMTK_FS_ERR;
#endif
