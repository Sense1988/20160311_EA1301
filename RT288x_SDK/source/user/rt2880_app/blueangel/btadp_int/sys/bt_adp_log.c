...
U32 btmtk_log_get_timestamp(void)
{
    ...
    /*----------------------------------------------------------------*/
    struct timeval tv;
    int secofday = 0, hours = 0, minutes = 0, seconds = 0;

    gettimeofday(&tv, NULL);

    secofday = (tv.tv_sec % 86400); 
    hours = secofday / 3600;
    minutes = (secofday % 3600) / 60 ;
    seconds = secofday % 60 ;  

    return (U32)(hours*10000000 + minutes*100000 + seconds*1000 + tv.tv_usec / 1000 );
}
...
//    kal_uint8   buf[MAX_ILM_BUFFER_SIZE] = {0}, buf2[MAX_ILM_BUFFER_SIZE] = {0};
    ...
    {
        LOGE("[ERR] btmtk_log_fill_and_send_frame : lenTraceHeader=%lu, lenData=%lu", pArgs->lenTraceHeader, pArgs->lenData);
        LOGE("frameMessageType=%lu", pArgs->frameMessageType);
        if(pArgs->frameMessageType == PS_TRACE_INDEX_TRACE_TYPE)
        {
            ...
            LOGE("msg_index=%lu, trace_class=%d", pHeader->msg_index, pHeader->trace_class);
        }
          ...
          const char *arg_type,...)
{
    ...
    vsnprintf(msg_buffer+7, 290, MOD_BT_MSG_FMT[msg_index-1], args);
    va_end(args);
  #ifdef ANDROID
...
void bt_prompt_trace(module_type mod_id, const char *fmt,...)
{
    ...
    msg_buffer[299] = 0;
    va_start(args, fmt);
    vsnprintf(msg_buffer+7, 290, fmt, args);
    va_end(args);
...
void bt_log_primitive(ilm_struct* ilm_ptr)
{
    ...
    (void)btmtk_log_fill_and_send_frame;
#endif // __PRJ_MT7650_SONY_BDP__
}
...
static int g_file_created=0;
static int g_LogFD = -1;
int g_Terminal_Log_RunTime_Config = 1;
int log_file_create(void)
{
    ...
    snprintf(fileName, sizeof(fileName), "/tmp/blueangel_terminal_log_%d", (int)pTm->tm_year+1900);
#else
    if(0 == access("/mnt/sda1", F_OK))
    {
        snprintf(fileName, sizeof(fileName), "/mnt/sda1/blueangel_terminal_log_%d", (int)pTm->tm_year+1900);
    }
    else if(0 == access("/mnt/sdb1", F_OK))
    {
        snprintf(fileName, sizeof(fileName), "/mnt/sdb1/blueangel_terminal_log_%d", (int)pTm->tm_year+1900);
    }
    else if(0 == access("/mnt/sda", F_OK))
    {
        snprintf(fileName, sizeof(fileName), "/mnt/sda/blueangel_terminal_log_%d", (int)pTm->tm_year+1900);
    }
    else if(0 == access("/mnt/sdb", F_OK))
    {
        snprintf(fileName, sizeof(fileName), "/mnt/sdb/blueangel_terminal_log_%d", (int)pTm->tm_year+1900);
    }
    else
    {
        fprintf(stderr, "USB drive path not exist!\n");
        fflush(stderr);
    	return -1;
    }
#endif
    ...
    snprintf(fileName_tmp, sizeof(fileName_tmp), "%s%02d.log", fileName, (int)pTm->tm_sec);
    memcpy(fileName, fileName_tmp, sizeof(fileName));

    fprintf(stdout, "\033[1;32mTerminal Log : open file(%s) \033[m\n", fileName);
    fflush(stdout);

    g_LogFD = open(fileName, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if ( g_LogFD < 0 )
    {
...
void bt_prompt_trace_to_file(char *str)
{
    if ( g_Terminal_Log_RunTime_Config == 0 )
    {
        return;
    }

    // Since BlueAngel process will never be stopped, we don't do file close.
    if ( g_file_created != 1 )
    {
        if ( log_file_create() < 0 )
        {
            g_file_created = -1;
            return;
        }
    ...
    if ( g_LogFD > 0 )
    {
        ...
        if ( len < 298 )
        {
            str[len]=0x0A;  // line feed
            str[len+1]=0x0D;  // carriage return
            if ( write(g_LogFD, str, len+1) < 0 )
            {
                close(g_LogFD);
                g_file_created = -1;
            }
