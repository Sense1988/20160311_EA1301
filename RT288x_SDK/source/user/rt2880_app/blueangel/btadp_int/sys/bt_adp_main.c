...
#endif
extern U32 bt_dbg_manual_dump_req_hdlr(ilm_struct *ilm_ptr);
extern void bt_em_test_cmd_handler(ilm_struct *ilm_ptr);
...
kal_bool bt_main(ilm_struct *ptr)
{
	...
	case MSG_ID_BT_ENGINEER_MODE_LOG_MASK_REQ:
	{
		...
		cnf_p->result = (logToFile((S8*) req_p->name)<0) ? 0 : 1;
        ...
        default:
            ...
            else if ((ptr->msg_id >= MSG_ID_BT_HF_GROUP_START) && (ptr->msg_id <= MSG_ID_BT_HF_GROUP_END))
            {
                /* HF handler */
                hf_adp_handler(ptr);
            }
