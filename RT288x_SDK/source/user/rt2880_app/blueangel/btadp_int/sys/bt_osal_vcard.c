...
static void mmi_vcard_retrive_address(S8* string, U8* pobox, U8* extension, U8* street, U8* city, U8* state, U8* postalcode, U8* country, U16 max_len, U8 default_charset)
{
    ...
    while(*cursor)
    {
        ...
        while ((*cursor != ';') && (*cursor != '\0'))
        {
            ...
            if (i < VCARD_WRITER_FIELD_BUFFER)
            {
                temp_buf[i++] = *cursor;
            }
            cursor++;
        ...
        {
            strncpy(temp_value_buf, temp_buf, VCARD_WRITER_FIELD_BUFFER);
        }
...
static S32 mmi_vcard_uti_get_single_line(S8 *strOutput, BTMTK_FS_HANDLE fh)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    snprintf(strCRLF, sizeof(strCRLF), "%c%c", VOBJ_CR_CHAR, VOBJ_LF_CHAR);
    snprintf(strLF, sizeof(strLF), "%c", VOBJ_LF_CHAR);
    snprintf(str_photo_end, sizeof(str_photo_end), "//Z%s%s", strCRLF, strCRLF);
...
static S32 mmi_vcard_uti_get_line(S8 *strOutput, BTMTK_FS_HANDLE fh)
{
    ...
    for (i = VCARD_FIELD_FN; i < VCARD_FIELD_TOTAL; i++)
    {
        ...
        else if (((S32)(uintptr_t)cursor > (S32)(uintptr_t)curr_cursor) && (*(curr_cursor - 1) == 0x0A))
        {
            cursor = curr_cursor;
        }
    }
    ...
    strLen = (S32)(uintptr_t)cursor - (S32)(uintptr_t)buffer;
