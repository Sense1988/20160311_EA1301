...
kal_bool bt_setProfileAddress(PROFILE_enum profile, struct sockaddr_un *addr, socklen_t addrlen)
{
    ...
    bt_prompt_trace(MOD_BT,"bt_setProfileAddress(%d, %s, %d)", profile, &addr->sun_path[0], addrlen);
...
void initProfileAddress(void)
{
	...
	/* PROFILE_HF */
    setProfileAddress(PROFILE_HF, BT_SOCK_NAME_EXT_ADP_HF);    
    /* PROFILE_HFG */
    setProfileAddress(PROFILE_HFG, BT_SOCK_NAME_EXT_ADP_HFG);
    /* PROFILE_SPP */
...
//    socket_local_server(BT_SOCK_NAME_INT_ADP, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_DGRAM);
    ...
    /* Init server socket */
    g_soaddr_int_adp.sun_family = AF_UNIX;
    memset(g_soaddr_int_adp.sun_path, 0, sizeof(g_soaddr_int_adp.sun_path));
    strncpy (g_soaddr_int_adp.sun_path, BT_SOCK_NAME_INT_ADP, strlen(BT_SOCK_NAME_INT_ADP));
    g_soaddr_int_adp_len = (offsetof (struct sockaddr_un, sun_path) + strlen (g_soaddr_int_adp.sun_path) + 1);
    ...
    memset(g_soaddr_ext_adp.sun_path, 0, sizeof(g_soaddr_ext_adp.sun_path));
    strncpy (g_soaddr_ext_adp.sun_path, BT_SOCK_NAME_EXT_ADP, strlen(BT_SOCK_NAME_EXT_ADP));
    ...
    memset(g_soaddr_a2dp_stream.sun_path, 0, sizeof(g_soaddr_a2dp_stream.sun_path));
    strncpy (g_soaddr_a2dp_stream.sun_path, BT_SOCK_NAME_A2DP_STREAM, strlen(BT_SOCK_NAME_A2DP_STREAM));
...
//            if((ilm->msg_id >= MSG_ID_BT_GAP_GROUP_START) && (ilm->msg_id <= MSG_ID_BT_GAP_GROUP_END))
            ...
            else
            {
                ...
                if((ilm->msg_id >= MSG_ID_BT_HF_GROUP_START) && (ilm->msg_id <= MSG_ID_BT_HF_GROUP_END))
                { 
                    profile = PROFILE_HF; 
                }else 
                #endif                
...
void *bt_deQueue_message(module_type mod_type)
{
    ...
    OS_MemSet((U8 *)&g_cmdaddr, 0x0 ,sizeof(g_cmdaddr));	
	...
	else if (hdp_adp_check_socket_address(g_cmdaddr.sun_path + 1))
	{
		...
		hdp_deQueue_data(g_cmdaddr.sun_path + 1, &g_rec_ilm, ret);
		return NULL;
	}
#endif
    else
