...
//    OS_Report("External platform, free memory:%d\n",counter_memory_alloc);
#ifdef __WIN_QUEUE__    
	bt_win_free(ptr);
#endif
}
...
void win_messagehdl_init(void)
{
	...
	for(i=0;i<BT_MAX_TASK_NO; i++)
	{
        ...
        for(j=0; j<2; j++)
        {
            ...
            { 
               OS_Report("Could not create file mapping object (%d).\n", GetLastError());
               return;
            }
...
void win_message_putToSharedMemory(U8 index, U8 txrx, U8 *ptr, U16 length)
{
    ...
    if(index == 0x00)
    {
        if(txrx == 0x00)
        {
            OS_Report("Server Put data to MOD_BT, length:%d\n",length);
        }
        else
        {
            OS_Report("MOD_BT Put data to Server, length:%d\n",length);
        }   
    }
    else if(index == 0x01)
    {
        if(txrx == 0x00)
        {
            OS_Report("Server Put data to MOD_Timer, length:%d\n",length);
        }
        else
        {
            OS_Report("MOD_Timer Put data to Server, length:%d\n",length);
        }        
    }
    else if(index == 0x02)
    {
        if(txrx == 0x00)
        {
            OS_Report("Server Put data to MOD_MMI, length:%d\n",length);
        }
        else
        {
            OS_Report("MOD_MMI Put data to Server, length:%d\n",length);
        }        
    }
#endif    
    switch (dwWaitResult) 
...
U16 win_message_GetFromSharedMemory(U8 index, U8 txrx, U8 *ptr, U16 length)
{
    ...
    if(bytesToCopy !=0)
    {
        ...
        if(index == 0x00)
        {
            if(txrx == 0x00)
            {
                OS_Report("MOD_BT Get data from Server, length:%d\n",bytesToCopy);
            }
            else
            {
                OS_Report("Server Get data from MOD_BT, length:%d\n",bytesToCopy);
            }
        }
        else if(index == 0x01)
        {
            if(txrx == 0x00)
            {
                OS_Report("MOD_Timer Get data from Server, length:%d\n",bytesToCopy);
            }
            else
            {
                OS_Report("Server Get data from MOD_Timer, length:%d\n",bytesToCopy);
            }
        ...
        else if(index == 0x02)
        {
            if(txrx == 0x00)
            {
                OS_Report("MOD_MMI Get data from Server, length:%d\n",bytesToCopy);
            }
            else
            {
                OS_Report("Server Get data from MOD_MMI, length:%d\n",bytesToCopy);
            }        
        }
#endif
