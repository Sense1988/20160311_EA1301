...
{ 
    ...
    sockfd = socket_local_server(SERV_SOCK_NAME, 
                                                   ...
                                                   SOCK_STREAM);
#else
    struct sockaddr_un g_soaddr_tst;
    socklen_t   g_soaddr_tst_len;
    ...
    g_soaddr_tst.sun_family = AF_UNIX;
    strncpy (g_soaddr_tst.sun_path, SERV_SOCK_NAME, sizeof(SERV_SOCK_NAME));
    g_soaddr_tst_len = (offsetof (struct sockaddr_un, sun_path) + strlen (g_soaddr_tst.sun_path) + 1);
    /* remove the bt.int.adp first */
    
    unlink(g_soaddr_tst.sun_path);
    if (bind (sockfd, (struct sockaddr *) &g_soaddr_tst, g_soaddr_tst_len) < 0)
    {
...
#else
        res = pthread_cond_wait(&tstcond, &mtx);
#endif
        g_tstWaitFlag = 0;
        if(res != 0)
        {
...
int logToFile(char *name){
    char filename[128]={0};
    snprintf(filename, sizeof(filename), "%s_%ld", name, time(NULL));
