...
{
	LOCAL_PARA_HDR
	U8 				pbap_data_file[BT_PBAP_MAX_FILEPATH_NAME_LEN];	// UCS-2
	U8				objectName[PBAP_MAX_NAME_LEN/2+1];				// UCS-2
	PbapVcardFilter	filter;
	PbapVcardFormat	format;
...
typedef struct
{
	LOCAL_PARA_HDR
	U8 				pbap_data_file[BT_PBAP_MAX_FILEPATH_NAME_LEN];	// UCS-2
	U8				folderName[PBAP_MAX_NAME_LEN/2+1];				// UCS-2
	U8				searchAttribute;
	U8				searchValue[MAX_PBAP_SEARCH_VALUE_LENGTH + 1]; // UTF-8
	U8				searchValueLength;
	U8				order;
	U16				maxListCount;
	U16				listStartOffset;
...
typedef struct
{
    LOCAL_PARA_HDR
    U8 dev_name[BT_PBAP_MAX_DEV_NAME_LEN];
    U8 auth_option;/*don't use these two paramters*/
