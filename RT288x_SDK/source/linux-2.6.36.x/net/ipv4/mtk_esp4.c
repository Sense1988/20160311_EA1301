...
)
{
	...
	if (in_atomic())
		gfp_flags |= GFP_ATOMIC;    // non-sleepable
	else
		gfp_flags |= GFP_KERNEL;    // sleepable
...
#ifdef MTK_EIP97_DRIVER
	spin_lock(&cryptoLock[0]);
#else
	spin_lock(&cryptoLock[0]);
#endif	
	while (ipsec_eip93CmdResCnt_check(0))
	{	
#ifdef MTK_EIP97_DRIVER
		ipsec_packet_put(addrsPreCompute->cmdHandler, NULL, 0); //mtk_packet_put()
#else
...
)
{
	...
	if (i == IPESC_EIP93_ADAPTERS)
	{
		...
		//currAdapterPtr = (ipsecEip93Adapter_t *) kzalloc(sizeof(ipsecEip93Adapter_t), gfp_flags);	
...
#endif		
		while (ipsec_eip93CmdResCnt_check(rdx) && ((pSkb = skb_dequeue(&currAdapterPtr->skbQueue)) != NULL))
		{
#ifdef MTK_EIP97_DRIVER
...
)
{
	...
	for (j = 0; j < 2; j++)
	{
		for (i = 0; i < IPESC_EIP93_ADAPTERS; i++)
		{
			...
			currAdapterPtr = kzalloc(sizeof(ipsecEip93Adapter_t), GFP_KERNEL);
			addrsPreCompute = kzalloc(sizeof(addrsDigestPreCompute_t), GFP_KERNEL);
#if defined (MTK_EIP97_DRIVER)			
