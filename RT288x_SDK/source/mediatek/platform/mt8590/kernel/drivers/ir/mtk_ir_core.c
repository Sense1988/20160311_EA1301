...
#define MT_EINT_IRRX_IRQ_ID              (GIC_PRIVATE_SIGNALS + 87) // irrx irq number 119
...
static DEVICE_ATTR(clock, 0222, NULL, mtk_ir_core_store_debug);

static DEVICE_ATTR(swirq, 0222, NULL, mtk_ir_core_store_debug);

static DEVICE_ATTR(hwirq, 0222, NULL, mtk_ir_core_store_debug);
...
{    
	...
	mt_set_gpio_mode(GPIO18,GPIO_MODE_06); // enable irrx pin function
	...
	ret = platform_device_register(&(mtk_ir_dev_rc5.dev_platform)); //register  rc6 device
	#endif
