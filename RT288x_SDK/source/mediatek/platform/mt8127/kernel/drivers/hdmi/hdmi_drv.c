...
void hdmi_irq_impl(void)
{
    ...
    if((hdmi_hotplugstate == HDMI_STATE_HOT_PLUGIN_AND_POWER_ON)&&(bCheckPordHotPlug(PORD_MODE|HOTPLUG_MODE)==FALSE))
    {
	   hdmi_hotplugstate = HDMI_STATE_HOT_PLUG_OUT;
	   vSetSharedInfo(SI_HDMI_RECEIVER_STATUS, HDMI_PLUG_OUT);
       vPlugDetectService(HDMI_STATE_HOT_PLUG_OUT);
    ...
    else if((hdmi_hotplugstate == HDMI_STATE_HOT_PLUG_OUT)&&(bCheckPordHotPlug(PORD_MODE|HOTPLUG_MODE)==TRUE))
    {
       hdmi_hotplugstate = HDMI_STATE_HOT_PLUGIN_AND_POWER_ON;
	   vSetSharedInfo(SI_HDMI_RECEIVER_STATUS, HDMI_PLUG_IN_AND_SINK_POWER_ON);
       vPlugDetectService(HDMI_STATE_HOT_PLUGIN_AND_POWER_ON);
	...
	else if((hdmi_cec_on==1)&&(bCheckPordHotPlug(HOTPLUG_MODE)==TRUE))
	{
	  if(hdmi_cec_isrprocess(hdmi_rxcecmode))
	  {
		 vNotifyAppHdmiState(HDMI_PLUG_IN_CEC);
	  }
	  HDMI_PLUG_LOG("cec return\n");
	}
