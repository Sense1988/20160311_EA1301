...
void pmic_low_power_setting(void)
{
...
#if 0 // tmp rollback, resume after enabling 26M BB control by hw 
    //VTCXO control
    if(chip_version > PMIC6397_E2_CID_CODE)
    {
        //power off when suspend
        //printk("@@@@@@@@0x400 0x446 chip_version=0x%x\n", chip_version);
        ret = pmic_config_interface(0x400  , 0x4C00 , 0x6c01 ,0); 
        ret = pmic_config_interface(0x446  , 0x0200 , 0x0200 ,0);
    }    
    else
#endif 
    if(chip_version > PMIC6397_E1_CID_CODE)
    {
        ...
        //printk("@@@@@@@@0x400 0x446 chip_version=0x%x\n", chip_version);
        ret = pmic_config_interface(0x400  , 0x4401 , 0x6c01 ,0); 
