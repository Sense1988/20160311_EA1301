...
bool mtk_nand_exec_read_sector(struct mtd_info *mtd, u32 u4RowAddr,  u32 u4ColAddr, u32 u4PageSize, u8 * pPageBuf, u8 * pFDMBuf)
{
    u8 *buf;
    int bRet = ERR_RTN_SUCCESS;
    struct nand_chip *nand = mtd->priv;
    ...
    if (mtk_nand_ready_for_read(nand, u4RowAddr, u4ColAddr, u4SecNum, true, buf))
    {
        ...
        if (g_bHwEcc)
        {
            if (!mtk_nand_check_bch_error(mtd, buf, pFDMBuf,u4SecNum - 1, u4RowAddr))
            {
            	...
            	MSG(INIT, "mtk_nand_check_bch_error fail retry:%d\n",retryCount);
                bRet = ERR_RTN_BCH_FAIL;
            }
        }
        }
...
static int mtk_nand_read_subpage(struct mtd_info *mtd, struct nand_chip *chip, u8 * buf, int page, int subpage)
{
//	int block_size = 1 << (chip->phys_erase_shift);
    int page_per_block = 1 << (chip->phys_erase_shift - chip->page_shift);
//	int page_per_block1 = page_per_block;
...
//	int retryCount = 0;
	...
	bRet = mtk_nand_exec_read_sector(mtd, page_in_block + mapped_block * page_per_block, coladdr, devinfo.sectorsize, buf, chip->oob_poi);
	if (bRet == ERR_RTN_SUCCESS)
    {
        do_gettimeofday(&etimer);
...
int mtk_nand_block_bad_hw(struct mtd_info *mtd, loff_t ofs)
{
	...
	ret = mtk_nand_read_subpage(mtd,chip,temp_buffer_16_align,(int)(ofs >> chip->page_shift),0);
	page_addr = mtk_nand_page_transform(mtd, chip, page_addr, &block, &mapped_block);
