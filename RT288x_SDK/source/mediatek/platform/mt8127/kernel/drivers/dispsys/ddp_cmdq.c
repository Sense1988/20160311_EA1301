...
static long cmdq_proc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case DISP_IOCTL_EXEC_COMMAND_SECURE:
            ...
            if (cmdqSubmitTaskSecure(&command))
            {
    ...
    *pEOF   = 0x1;
    ...
    for (;numRec > 0; numRec--, index++)
    {
        ...
        length += sprintf(&pPage[length], "Index: %d, Scenario: %d, Priority: %d, Reorder: %d, trigger: %d:%d IRQ Time: %d, Exec Time: %d, Total Time: %d\n",
            index, pRecord->scenario, pRecord->priority, pRecord->reorder, (uint32_t)pRecord->trigger.tv_sec, (uint32_t)pRecord->trigger.tv_usec, IRQTime, execTime, totalTime);
                            ...
                            bool copyCmdFromUserSpace)
{
    ...
    CMDQ_MSG("copy_from_user[%d], pDst[0x%08x], pSrc[0x%08x], size[%d]\n", copyCmdFromUserSpace, (unsigned int)pDst, (unsigned int)pSrc, size);
...
TaskStruct* cmdq_acquire_task(cmdqCommandStruct *pCommandDesc)
{
    ...
    CMDQ_MSG("Got task flag 0x%08x, CMD 0x%08x, size %d\n", (unsigned int)pCommandDesc->engineFlag, (uint32_t)(pCommandDesc->pVABase), (pCommandDesc->blockSize));

    CMDQ_MSG("[CMDQ] cmdq_acquire_task, addrList[%d][0x%08x], portList[%d][0x%08x]\n", 
        pCommandDesc->secData.addrListLength, (unsigned int)pCommandDesc->secData.addrList, pCommandDesc->secData.portListLength, (unsigned int)pCommandDesc->secData.portList);
    ...
    CMDQ_MSG("[CMDQ] Source: 0x%08x, target: 0x%08x, size: %d\n", (uint32_t)pCommandDesc->pVABase, (uint32_t)pTask->pVABase, pCommandDesc->blockSize);
    CMDQ_MSG("[CMDQ] Source: 0x%08x, target: 0x%08x, size: %d\n", (uint32_t)pCommandDesc->secData.addrList, (uint32_t)pTask->secData.addrList, secAddrListLength_byte);        
    CMDQ_MSG("[CMDQ] Source: 0x%08x, target: 0x%08x, size: %d\n", (uint32_t)pCommandDesc->secData.portList, (uint32_t)pTask->secData.portList, secPortListLength_byte);
                                   ...
                                   int32_t    thread)
{
    ...
    while(index < gCmdqContext.warningNum && index < CMDQ_MAX_WARNING_COUNT)
    {
        CMDQ_ERR("warning task[%d][0x%08x], THR[%d], taskCount[%d], CurrCookie[%d], irqFlag[%d], gotIRQ[%d:%d]", 
                    index, 
                    (unsigned int)gCmdqContext.warning[index].taskAddr, 
                    ...
                    (int)gCmdqContext.warning[index].gotIRQ.tv_sec, 
                    (int)gCmdqContext.warning[index].gotIRQ.tv_usec); 
        index ++; 
    }    
}
            ...
            struct timeval gotIRQ)
{
    ...
    {
        CMDQ_ERR("warning task[%d][0x%08x], THR[%d], taskCount[%d], CurrCookie[%d], irqFlag[%d], gotIRQ[%d:%d]", 
            index, (unsigned int)pTask, thread, taskCountInThread, cookie, irqFlag, (int)gotIRQ.tv_sec, (int)gotIRQ.tv_usec); 
...
{    
    if((NULL == pThread) || (CMDQ_MAX_TASK_COUNT < index) || (0 > index))
    {
        CMDQ_ERR("remove task, invalid param. THR[0x%08x], task_slot[%d], targetTaskStaus[%d]\n", 
            (unsigned int)pThread, index, targetTaskStaus); 
        return; 
    }
    ...
    if(TASK_STATE_BUSY != pThread->pCurTask[index]->taskState)
    {
        CMDQ_ERR("remove task, taskStatus err[%d]. THR[0x%08x], task_slot[%d], targetTaskStaus[%d]\n",
            pThread->pCurTask[index]->taskState, (unsigned int)pThread, index, targetTaskStaus);
        return; 
    }
            ...
            uint32_t predump_retry_count)
{
    ...
    else if (0 > waitQ)  // Task be killed
    {
        ...
        CMDQ_ERR("should not go here. waitQ[%d], thread[%d], taskCount[%d], pTask[0x%08x], pTask->MVABase[0x%08x]\n",
            waitQ, thread, pThread->taskCount, (unsigned int)pTask, pTask->MVABase);
    }
...
int32_t cmdqSubmitTask(cmdqCommandStruct *pCommandDesc)
{
    ...
    thread = cmdq_acquire_thread(pCommandDesc->engineFlag, false);
    if (CMDQ_INVALID_THREAD == thread)
    {
...
#if 1
int32_t cmdqSubmitTaskSecure(cmdqCommandStruct *pCommandDesc)
{
    struct timeval  start;
    struct timeval  done;
    TaskStruct      *pTask;
...
//    printk("[CMDQ] --> cmdqSubmitTaskSecure, totalSecureFd[%d], pSecureFdIndex[0x%08x], pSecurePortList[0x%08x]\n", totalSecureFd, pSecureFdIndex, pSecurePortList);
    ...
    thread = cmdq_acquire_thread(pCommandDesc->engineFlag, isSecureTask);
    if (CMDQ_INVALID_THREAD == thread)
    {
        cmdq_release_task(pTask);
        return -EFAULT;
