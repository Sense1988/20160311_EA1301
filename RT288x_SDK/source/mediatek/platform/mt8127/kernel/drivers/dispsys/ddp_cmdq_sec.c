            ...
            CmdqSecFillIwcCB iwcFillCB )
{
    ...
    if(NULL != pTask && CMDQ_INVALID_THREAD != thread)
    {
        ...
        //printk("[CMDQ][SEC][v2] cmdq_sec_fill_iwc_buffer, fdCound:%d\n", secFdCount);
        printk("[CMDQ][SEC] addrList[%d][0x%08x], portList[%d][0x%08x]\n", 
            pTask->secData.addrList, pTask->secData.addrListLength, pTask->secData.portListLength, pTask->secData.portList);
        ...
        uint32_t* pDstCMD = pIwc->pVABase;

        printk("[CMDQ][SEC] pTask->pVABase = 0x%08x, pIwc->pVABase = 0x%08x, cmdBlockSize\n", pTask->pVABase, pIwc->pVABase, (pTask->blockSize));
            ...
            CmdqSecFillIwcCB iwcFillCB )
{
    ...
    do
    {
        ...
        {
            uint32_t* pSrcCMD = pTask->pVABase;
            uint32_t* pDstCMD = gIwc->pVABase;

            printk("[CMDQ][SEC] after fill_iwc, pTask= 0x%08x, pIwc= 0x%08x\n", pTask, gIwc);
            printk("[CMDQ][SEC] after fill_iwc, pTask->pVABase = 0x%08x, pIwc->pVABase = 0x%08x\n", pTask->pVABase, gIwc->pVABase);
