...
//    unsigned int reg_addr;
    ...
    if(pOvlConfig->layer_en!=0)
    {
        ...
        if (1 == debug_svp)
#else
        if (OVL_LAYER_SECURE_BUFFER == is_ovl_secured())
#endif // MANUAL_DEBUG
        {
        ...
        else if (OVL_LAYER_PROTECTED_BUFFER == is_ovl_secured())
        {
...
int disp_path_config_(struct disp_path_config_struct* pConfig, int mutexId)
{
	...
	/*mode 0 from single mode,1 from dsi,2 from dpi0,3 from dpi1*/
		...
		/* 0 from bls, 1 from rdma1*/
		 ...
		 #else
                DISP_REG_SET(DISP_REG_CONFIG_DISP_OUT_SEL, 0x9); // rdma1_mout to dpi1
