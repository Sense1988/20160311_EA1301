...
static int hdmi_rdma_update_kthread(void *data)
{
    struct sched_param param = { .sched_priority = RTPM_PRIO_SCRN_UPDATE };
    int buf_sequence;
    hdmi_video_buffer_list *pUpdateList;
...
int hdmi_rdma_address_config(bool enable, hdmi_video_buffer_info buffer_info)
{
	...
	config.dstModule = DISP_MODULE_DPI0;
        ...
        {
            if (DPI1_IS_TOP_FIELD()) // top & bottom exchange for shadow register update in next vsync
...
/*static*/ void hdmi_dpi_power_switch(bool enable)
{
    ...
    else
    {
        ...
        if (p->is_clock_on == false)
        {
            ...
            #if MTK_HDMI_MAIN_PATH
            /*if hdmi main path, force disable pll*/
	...
	case MTK_HDMI_COLOR_DEEP:
       ...
       case MTK_HDMI_AUDIO_VIDEO_ENABLE:
       {
		...
		HDMI_LOG(" hdmi not enabled,change resolution fail\n");
            ...
            if(IS_HDMI_NOT_ON())
            {
		HDMI_LOG(" hdmi power state is not on,change resolution fail\n");
            }
...
long hdmi_handle_cmd(unsigned int cmd, unsigned long arg) {
	...
	HDMI_LOG("[HDMI] %s = %s(%d), arg = %lu\n", __func__,_hdmi_ioctl_spy(cmd), cmd & 0xff, arg);


	switch (cmd) {
		case MTK_HDMI_AUDIO_VIDEO_ENABLE:/*1*/
	       {
	            hdmi_video_buffer_info temp;
	        ...
	        case MTK_HDMI_VIDEO_CONFIG:/*6*/
	        {
	            ...
	            hdmi_reschange = HDMI_VIDEO_720x480p_60Hz;
		...
		case MTK_HDMI_IS_FORCE_AWAKE:/*16*/
	        {
	            if (!hdmi_drv_init_context())
	            {
	                printk("[hdmi]%s, hdmi_drv_init_context fail\n", __func__);
		...
		default :
		r = -1;
	        printk("%s Not support, cmd=0x%08x, arg=0x%08x\n", __func__, (unsigned int)cmd, (unsigned int)arg);
